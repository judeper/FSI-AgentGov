...
### Step 3a: Standardized Agent Card Template

**Purpose:** Agent Cards provide a standardized, machine-readable format for documenting AI agent capabilities, limitations, training sources, and governance status. This template is derived from ML Model Card best practices (Mitchell et al., 2019) and adapted for Copilot Studio agents and M365 Copilot deployments.

#### Agent Card Overview

An Agent Card serves as the definitive reference document for each AI agent, capturing:
- What the agent does and doesn't do
- How it was trained/configured
- Known limitations and failure modes
- Performance benchmarks
- Governance and compliance status

#### Agent Card Template (Markdown Format)
...
| Field | Value |
|-------|-------|
| **Agent ID** | [Unique identifier from registry] |
| **Agent Name** | [Display name] |
| **Version** | [Current version number] |
| **Model Tier** | [1 (High) / 2 (Medium) / 3 (Low)] |
| **Governance Zone** | [Zone 1 / Zone 2 / Zone 3] |
| **Status** | [Active / Suspended / Retired] |
| **Last Updated** | [Date] |
| **Card Version** | [Agent card revision] |
...
| **Designated Supervisor** | [Name] | [Email] |
| **Model Risk Manager** | [Name] | [Email] |

## Agent Description

### Intended Use
[Describe the primary purpose and intended use cases for this agent]

### Target Users
[Describe who should use this agent]

### Out-of-Scope Uses
[List use cases that are explicitly NOT supported or prohibited]

## Underlying Model

| Field | Value |
|-------|-------|
| **Base Model** | [GPT-5 / GPT-4o / Custom] |
| **Model Version** | [Version ID from Copilot Studio] |
| **Last Model Update** | [Date of last model change] |
| **Model Change Impact** | [None / Minor / Material - requires MRM notification] |

## Knowledge Sources

### Grounding Data

| Source Type | Source Name | Data Classification | Last Refresh |
|-------------|-------------|---------------------|--------------|
| SharePoint | [Site/Library name] | [Confidential/Internal] | [Date] |
| Dataverse | [Table name] | [Classification] | [Date] |
| Custom API | [API name] | [Classification] | [Date] |
| Web Search | [Enabled/Disabled] | N/A | N/A |

### Training/Fine-tuning
[Note: Copilot Studio agents use grounding rather than fine-tuning. Document any topic-specific training data if applicable.]

- **Custom Topics:** [Number of custom topics]
- **Sample Utterances:** [Approximate count]
...
|-------------|-----------|--------|------------|
| [Failure description] | [Low/Med/High] | [Description] | [Mitigation] |
...
| Resolution Rate | >90% | [%] | [↑/↓/→] |
| Escalation Rate | <10% | [%] | [↑/↓/→] |
| Average Response Time | <2s | [s] | [↑/↓/→] |
| User Satisfaction (CSAT) | >4.0 | [/5.0] | [↑/↓/→] |

### Bias Testing Results

| Protected Class | Disparity Measure | Result | Pass/Fail |
|-----------------|-------------------|--------|-----------|
| [Per Control 2.11] | [Measure] | [Value] | [Status] |

## Governance Status

### Validation

| Validation Type | Date | Validator | Result | Next Due |
|-----------------|------|-----------|--------|----------|
| Initial Validation | [Date] | [Name/Firm] | [Pass/Conditional] | N/A |
| Annual Validation | [Date] | [Name/Firm] | [Pass/Conditional] | [Date] |
| Bias Testing | [Date] | [Name] | [Pass/Fail] | [Date] |

### Compliance

| Requirement | Status | Evidence |
|-------------|--------|----------|
| MRM Documentation Complete | [Yes/No] | [Link to docs] |
| HITL Configured (if required) | [Yes/No/N/A] | [Config reference] |
| Audit Logging Enabled | [Yes/No] | [Purview policy] |
| Retention Policy Applied | [Yes/No] | [Policy name] |

### Change History

| Version | Date | Change Description | Approved By |
|---------|------|-------------------|-------------|
| [X.Y.Z] | [Date] | [Change summary] | [Approver] |

## Disclosures and Warnings

### Required Disclosures
[List any disclosures the agent must make to users]

### User Warnings
[List warnings that should be communicated to users]
...
For automated agent card management, use this JSON schema:
...
```json
{
  ...
  "type": "object",
  "required": ["agentId", "name", "version", "modelTier", "zone", "status"],
  "properties": {
    "agentId": { "type": "string" },
    "name": { "type": "string" },
    "version": { "type": "string" },
    "modelTier": { "type": "integer", "enum": [1, 2, 3] },
    ...
    "ownership": {
      ...
      "properties": {
        ...
        "designatedSupervisor": { "type": "string" },
        "modelRiskManager": { "type": "string" }
      }
    },
    "underlyingModel": {
      "type": "object",
      "properties": {
        ...
        "lastModelUpdate": { "type": "string", "format": "date" }
      }
    },
    "knowledgeSources": {
      "type": "array",
      "items": {
        ...
        "properties": {
          ...
          "lastRefresh": { "type": "string", "format": "date" }
        }
      }
    },
    "performanceMetrics": {
    ...
    "validation": {
      "type": "object",
      "properties": {
        "lastValidationDate": { "type": "string", "format": "date" },
        "nextValidationDue": { "type": "string", "format": "date" },
    ...
    "metadata": {
      ...
      "properties": {
        ...
        "cardGeneratedDate": { "type": "string", "format": "date" },
...
#### Agent Card Requirements by Model Tier
...
#### Agent Card Management Process

1. **Creation:** Agent Card created during agent development (Control 3.1)
2. **Review:** Card reviewed at each validation cycle
3. **Update:** Card updated with any configuration or model change
4. **Storage:** Cards stored in agent registry with version control
5. **Access:** Cards accessible to supervisory personnel and auditors
...
### Step 6b: Agent Definition (Manifest) Version Control

**Purpose:** Beyond individual prompt changes, this step establishes versioning requirements for the complete agent definition (manifest), including system instructions, knowledge sources, capabilities, topic configurations, and action authorizations. This comprehensive versioning enables point-in-time reconstruction of agent behavior for regulatory examination and incident investigation.

#### Agent Manifest Components

An agent manifest includes all configuration elements that define agent behavior:

| Component | Description | Versioning Requirement |
|-----------|-------------|----------------------|
| **System Instructions** | Core agent instructions and persona | Every change |
| **Topic Definitions** | Conversation flows and trigger phrases | Every change |
| **Knowledge Sources** | Connected SharePoint sites, files, URLs | Add/remove events |
| **Actions/Plugins** | Enabled connectors and actions | Every change |
| **Authentication Config** | User authentication requirements | Every change |
| **Grounding Settings** | Citation, web search, scope limits | Every change |
| **Fallback Behaviors** | Escalation and error handling | Every change |
| **Security Settings** | Content moderation, input filters | Every change |

#### Manifest Versioning Requirements

**Version Numbering Convention:**

```
Format: [Major].[Minor].[Patch]-[Environment]
Example: 2.3.1-prod

Major: Significant capability changes or new knowledge domains
Minor: Topic additions, prompt updates, action changes
Patch: Bug fixes, minor wording adjustments
Environment: dev, test, uat, prod
```

**Mandatory Version Triggers:**

| Change Type | Version Increment | Approval Required |
|-------------|------------------|-------------------|
| New capability or knowledge domain | Major | AI Governance + Business |
| System instruction change | Minor | AI Governance |
| Topic addition/modification | Minor | Business Owner |
| Action/plugin enablement | Minor | Security + AI Governance |
| Bug fix or minor adjustment | Patch | Developer |
| Production deployment | N/A (environment suffix) | Change Management |

#### Manifest Storage and Retrieval

**Recommended Approaches:**

1. **Git-based Version Control (Recommended):**
   - Store agent manifests in Git repository
   - Use branch protection for production manifests
   - Require pull request approval for changes
   - Tag releases with version numbers
   - Enable full history and diff capabilities

2. **SharePoint Document Library:**
   - Enable versioning on library
   - Store manifest exports as JSON/YAML
   - Apply retention policy (7+ years for Zone 3)
   - Restrict edit permissions

3. **Azure DevOps Artifacts:**
   - Publish manifest as versioned artifact
   - Integrate with CI/CD pipeline
   - Enable artifact feeds for retrieval
...
# Connect to Dataverse/Power Platform
# Note: Actual implementation depends on available APIs
...
For regulatory examination or incident investigation, be able to answer: "What was this agent's exact configuration on [date]?"

**Reconstruction Requirements:**

| Zone | Reconstruction Capability | Retention |
|------|--------------------------|-----------|
| Zone 1 | Not required | N/A |
| Zone 2 | Within 48 hours | 3 years |
| Zone 3 | Within 24 hours | 7 years |

**Reconstruction Procedure:**

```markdown
# Agent Configuration Reconstruction Request

## Request Details
- **Request Date:** [Date]
- **Requestor:** [Name/Role]
- **Agent ID:** [Agent ID]
- **Target Date:** [Date for which configuration is needed]
- **Purpose:** [Regulatory examination / Incident investigation / Other]

## Reconstruction Steps
1. [ ] Identify manifest version active on target date
2. [ ] Retrieve manifest from version control
3. [ ] Export current configuration (for comparison)
4. [ ] Document differences between target and current
5. [ ] Provide reconstruction evidence package

## Deliverables
- [ ] Manifest file as of target date
- [ ] Change log from target date to present
- [ ] Summary of material differences
- [ ] Attestation of reconstruction accuracy

## Sign-Off
Reconstructed by: _________________ Date: _________
Verified by: _________________ Date: _________
```

#### Rollback Procedures

When agent behavior issues require reverting to a previous version:

**Rollback Decision Matrix:**

| Scenario | Rollback Authority | Max Time to Rollback |
|----------|-------------------|---------------------|
| Security vulnerability | Security Admin | 1 hour |
| Regulatory violation | Compliance Officer | 4 hours |
| Functionality regression | AI Governance Lead | 24 hours |
| Performance degradation | Platform Admin | 24 hours |

**Rollback Procedure:**

1. **Initiate rollback request** with justification
2. **Identify target version** in manifest history
3. **Validate target version** is stable and tested
4. **Execute rollback** in test environment first
5. **Verify functionality** post-rollback
6. **Deploy to production** following change management
7. **Document rollback** with root cause analysis
8. **Update manifest** to reflect rollback state

**Post-Rollback Requirements:**

- Document reason for rollback
- Identify root cause of issue
- Update agent testing to prevent recurrence
- Report significant rollbacks to governance committee

#### Manifest Version Control Compliance Checklist
...
### Step 8a: Third-Party Compliance Attestation

<a id="third-party-attestation"></a>

For organizations requiring independent compliance assessment, consider engaging third-party assessors with expertise in financial services recordkeeping and AI governance.

#### Cohasset Associates Compliance Assessment

[Cohasset Associates](https://www.cohasset.com/) provides independent compliance assessments for electronic records management systems, including AI-powered platforms. Their assessments evaluate compliance with:

- **SEC Rule 17a-4** - Electronic recordkeeping requirements
- **CFTC Rule 1.31** - Commodity trading records
- **FINRA Rules** - Broker-dealer recordkeeping
- **DOL/ERISA** - Retirement plan records

**When to Consider Cohasset Assessment:**

| Scenario | Recommendation |
|----------|---------------|
| Deploying customer-facing AI agents in broker-dealer operations | Recommended |
| AI agents processing or generating transaction records | Recommended |
| Regulatory examination identified recordkeeping concerns | Strongly recommended |
| New AI platform deployment for regulated functions | Consider for Tier 1 agents |
| Annual compliance review for Zone 3 agents | Consider as best practice |

**Assessment Scope for AI Agents:**

A Cohasset-type assessment for AI agent platforms should cover:

1. **Recordkeeping Compliance:**
   - Are AI agent interactions captured in compliant format?
   - Do records meet WORM (Write Once Read Many) requirements?
   - Is the retention period configurable and enforced?
   - Can records be produced for regulatory examination?

2. **Data Integrity:**
   - Are AI outputs and inputs preserved without modification?
   - Is there an audit trail of all agent activities?
   - Are timestamps accurate and tamper-evident?

3. **Accessibility:**
   - Can records be searched and retrieved efficiently?
   - Are records accessible for the required retention period?
   - Can records be exported in standard formats?

4. **Business Continuity:**
   - Are records protected against loss?
   - Is there a disaster recovery plan for agent records?
   - Are backup and restoration processes documented?

**Documentation for Third-Party Assessment:**

Prepare the following for a compliance assessment engagement:

```
Third-Party Assessment Preparation Checklist

PLATFORM DOCUMENTATION:
[ ] AI agent architecture documentation
[ ] Data flow diagrams showing record capture
...
EVIDENCE:
[ ] Sample audit logs
[ ] Sample agent interaction records
[ ] Retention enforcement evidence
[ ] Search/retrieval demonstration
[ ] Disaster recovery test results
```

**Annual Assessment Recommendation:**

For Zone 3 agents handling customer-facing or transaction-related functions, consider annual third-party compliance assessments to:

- Validate ongoing compliance with recordkeeping requirements
- Identify gaps before regulatory examination
- Demonstrate due diligence to regulators
- Benchmark against industry best practices

**Cost-Benefit Consideration:**

While third-party assessments involve cost, they provide:

- Independent validation of compliance posture
- Defensible evidence for regulatory examinations
- Identification of gaps before they become findings
- Industry benchmark comparison
- Expert guidance on emerging requirements
