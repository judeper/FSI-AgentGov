...
### Agent-Specific Versioning and Rollback

**Purpose:** AI agents require specialized versioning and rollback procedures beyond standard solution management. Agent behavior can change due to prompt modifications, knowledge source updates, connector changes, and underlying model updates. This section establishes agent-specific procedures for version control and rapid rollback.

#### Agent Version Components

An agent's "version" encompasses multiple components that can change independently:

| Component | Version Tracking | Change Impact | Rollback Complexity |
|-----------|-----------------|---------------|-------------------|
| **System Prompt** | Explicit versioning | High - affects all responses | Low - prompt text restore |
| **Topic Prompts** | Per-topic versioning | Medium - affects specific flows | Low |
| **Knowledge Sources** | Connection versioning | High - affects grounding | Medium - source restore |
| **Connectors/Actions** | Configuration versioning | High - affects capabilities | Medium |
| **Underlying Model** | Microsoft-controlled | Variable - model dependent | N/A (Microsoft managed) |
| **Solution Package** | Standard ALM | Comprehensive | Standard ALM rollback |

#### Agent Version Numbering Convention

Adopt a semantic versioning approach for agents:

```
Format: [Major].[Minor].[Patch]+[Config]-[Environment]
Example: 2.3.1+C5-prod

Major (2): Significant capability changes, new knowledge domains, or topic additions
Minor (3): Prompt updates, connector changes, action modifications
Patch (1): Bug fixes, minor wording adjustments, typo corrections
Config (+C5): Configuration snapshot number (for non-code changes)
Environment (-prod): dev, test, uat, prod
```

#### Agent Configuration Snapshot

Before any change, capture a complete configuration snapshot:
...
```markdown
# Agent Immediate Rollback Procedure
...
- [ ] Testing completed per governance tier
