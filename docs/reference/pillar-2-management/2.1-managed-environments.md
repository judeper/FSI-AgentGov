...
When an environment is managed, administrators gain access to enhanced governance features. Microsoft documentation lists **24 distinct capabilities** for Managed Environments ([Source](https://learn.microsoft.com/en-us/power-platform/admin/managed-environment-overview)):
...
### Full Capabilities Reference (24 Features)
...
| 15 | **Virtual Network support** | Private network connectivity (see [Control 1.20](../pillar-1-security/1.20-network-isolation-private-connectivity.md)) |
...
| 19 | **Cross-tenant inbound/outbound restrictions** | Control connector access across tenant boundaries |

**Advanced Features:**

| # | Capability | Description |
|---|------------|-------------|
| 20 | Extended backup | Longer backup retention |
| 21 | Data policies for desktop flow | RPA-specific DLP |
| 22 | Export data to Azure Application Insights | Telemetry integration |
| 23 | Create app description with Copilot | AI-assisted documentation |
| 24 | System administration capabilities | Enhanced admin controls |

!!! tip "FSI Priority Features"
    For regulated financial services, prioritize: **IP Firewall**, **VNet support** (see [Control 1.20](../pillar-1-security/1.20-network-isolation-private-connectivity.md)), **CMK**, **Lockbox**, **data masking**, **enhanced auditing**, and **cross-tenant restrictions**. These features address common examination expectations around data protection and access control.
...
!!! note
    Sharing limits do not apply when agent authentication is set to "No authentication". Always
    enable authentication for shared/enterprise agents.

!!! note
    Sharing limits govern **Power Platform sharing** for apps/flows/agents. They do not replace
    strong agent authentication and do not control access granted through any anonymous/public
    endpoint. For regulated use, avoid “No authentication” and validate controls with negative
    tests in `## Verification & Testing`.
...
|---------|-------------|
| [Control 1.20: Network Isolation and Private Connectivity](../pillar-1-security/1.20-network-isolation-private-connectivity.md) | VNet support implementation guidance |
