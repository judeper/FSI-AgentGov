...
!!! danger "Important - Routing Does NOT Block Default Environment Access"
    **When environment routing is enabled but no routing rule matches a user, the user is routed to the Default environment.** Per Microsoft Learn documentation: *"If no rule matches, or if environment routing isn't turned on, the maker is routed to the default environment."* ([Source](https://learn.microsoft.com/en-us/power-platform/admin/default-environment-routing))

**This means:**

1. Environment routing alone is **insufficient** to prevent access to the Default environment
2. You must either:
...
See [Control 1.1: Restrict Agent Publishing](../pillar-1-security/1.1-restrict-agent-publishing-by-authorization.md) for the "Sterile Default Environment Strategy" including DLP channel blocking.
