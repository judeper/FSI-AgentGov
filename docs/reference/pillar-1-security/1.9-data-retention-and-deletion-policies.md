...
**Primary Owner Admin Role:** Purview Records Manager
...
### Agent Memory Retention Considerations

Modern AI agents may utilize persistent memory beyond conversation logs. This memory enables context retention across sessions but introduces additional retention compliance requirements.

**Types of Agent Memory:**

| Memory Type | Description | Retention Consideration |
|-------------|-------------|------------------------|
| **Session Memory** | Context within a single conversation | Cleared at session end; included in conversation logs |
| **Persistent Memory** | User preferences, prior interactions stored across sessions | Subject to SEC 17a-4 as customer records |
| **RAG/Knowledge Memory** | Retrieved document context and embeddings | Inherits source document retention |
| **Vector Database Memory** | Semantic representations of interactions | Treat as derivative of source data |

**Agent Memory Retention Requirements:**

1. **Inventory Agent Memory Sources:** Identify all locations where agents store persistent information (Dataverse, Azure AI services, third-party memory providers)

2. **Apply Retention Policies:** Ensure agent memory stores are covered by retention policies matching the underlying data type:
   - Customer interaction memory → 7-year retention (SEC 17a-4)
   - Internal operational memory → 6-year retention
   - Session-only memory → No extended retention required

3. **Memory Reset Capability:** Maintain documented procedures to:
   - Purge individual user memory on request (CCPA/GDPR where applicable)
   - Reset agent memory for compliance investigations
   - Clear memory during agent decommissioning

4. **Audit Trail for Memory Operations:**
   - Log memory writes (what data was stored)
   - Log memory reads (what data was retrieved)
   - Log memory deletions (what was purged and why)

**Memory Scope Boundaries:**

| Boundary | Description | Zone Applicability |
|----------|-------------|-------------------|
| **Per-User** | Memory isolated to individual user | All zones |
| **Per-Agent** | Memory shared by all users of one agent | Zone 2-3 only |
| **Cross-Agent** | Memory shared across multiple agents | Zone 3 with explicit approval |

For Zone 3 agents, document the memory scope in the agent inventory (Control 3.1) and ensure memory boundaries align with data classification requirements.
...
!!! warning "Broker-Dealers"
    SEC Rule 17a-4(f) requires electronic records to be preserved in non-rewritable, non-erasable
    (WORM) format. Standard Microsoft 365 retention policies do not meet this requirement. See
    [Control
    1.7](./1.7-comprehensive-audit-logging-and-compliance.md#sec-17a-4-worm-requirements-broker-dealers)
    for WORM compliance options.
