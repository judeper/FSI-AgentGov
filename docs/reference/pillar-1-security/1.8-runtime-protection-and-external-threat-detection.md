...
- Microsoft Defender for Cloud (recommended for SIEM integration)
- Microsoft Sentinel (optional, for advanced threat detection)
...
| Setting | Recommended Value | Zone 3 Value |
|---------|-------------------|--------------|
...
### Step 5: Set Up External Threat Detection (Optional)
...
3. For Microsoft Defender integration:
   - Connect to Microsoft Defender for Cloud Apps
   - Enable Cloud Discovery for Power Platform
...
### Step 7: Configure SIEM Integration (Recommended for Zone 3)
...
if ($Threats.Count -gt 0) {
    Write-Host "`n⚠️ POTENTIAL THREATS DETECTED:" -ForegroundColor Red
    $Threats | Format-Table
} else {
    Write-Host "`n✅ No threat indicators found in period" -ForegroundColor Green
}
...
5. **Validate SIEM Integration (Zone 3):**
