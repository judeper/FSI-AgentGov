...
### Step 6: Set Up External Threat Detection

!!! warning "Preview Feature"
    External threat detection via custom webhooks is currently in **preview**. For regulated FSI environments, evaluate feature stability and data handling before production deployment. Monitor [Microsoft documentation](https://learn.microsoft.com/en-us/microsoft-copilot-studio/external-security-provider) for GA announcement and any changes to data sharing behavior.

External threat detection enables real-time evaluation of agent tool invocations through third-party security providers or Microsoft Defender. When configured, Copilot Studio sends tool invocation requests to an external webhook endpoint, which returns allow/block decisions before execution.

#### Step 6a: External Threat Detection Architecture

**How It Works:**

```
User Prompt → Agent Evaluates Tool Use → External Threat Detection Endpoint
                                                    ↓
                                        Provider Evaluates Request
                                                    ↓
                              ┌─────────────────────┴─────────────────────┐
                              ↓                                           ↓
                          ALLOW                                        BLOCK
                              ↓                                           ↓
                    Tool Executes Normally                    Tool Blocked + User Notified
```
...
| Aspect | Detail |
|--------|--------|
| **Timeout** | 1 second maximum response time |
| **Default Behavior** | Configurable: allow or block on timeout/error |
| **Authentication** | Federated Identity Credentials (FIC) - secret-less |
| **Scope** | Evaluates tool invocations before execution |

**Data Shared with External Provider:**

| Data Element | Description | FSI Sensitivity |
|--------------|-------------|-----------------|
| User prompts | Current and historical prompts in session | High |
| Chat history | Conversation context | High |
| Tool invocations | Tool name, reasoning, input parameters | Medium |
| Conversation metadata | Agent ID, user identity (UPN), tenant ID | Medium |
| Session context | Conversation ID, timestamps | Low |

#### Step 6b: Configure Entra App Registration for Webhook

**Portal Path:** [Microsoft Entra Admin Center](https://entra.microsoft.com) → **Applications** → **App registrations** → **New registration**

To enable secret-less authentication between Copilot Studio and your external threat detection endpoint, create an Entra app registration with Federated Identity Credentials (FIC).
...
3. Configure:
   ...
   - **Redirect URI:** Leave blank (not required for FIC)
4. Click **Register**
5. Note the **Application (client) ID** - you'll need this for Power Platform configuration
...
4. Configure:
   ...
   - **Subject identifier:** `/eid1/c/pub/t/{base64_tenantId}/a/m1WPnYRZpEaQKq1Cceg--g/{base64_endpoint}`
   - **Name:** `CopilotStudio-FIC`
   - **Audience:** `api://AzureADTokenExchange`

> **Note:** The subject identifier format is specific to Power Platform. Replace `{base64_tenantId}` with your tenant ID encoded in base64, and `{base64_endpoint}` with your webhook endpoint URL encoded in base64.

**PowerShell Alternative:**

```powershell
# Create-CopilotWebhookApp.ps1
# Creates Entra app registration with FIC for Copilot Studio external threat detection
...
#### Step 6c: Configure External Threat Detection in Power Platform Admin Center

**Portal Path:** [Power Platform Admin Center](https://admin.powerplatform.microsoft.com) → **Security** → **Threat detection** → **Additional threat detection**

1. Navigate to **Power Platform Admin Center**
2. Go to **Security** → **Threat detection**
3. Select **Additional threat detection** (or similar section for external providers)
4. Configure:

| Setting | Value | Notes |
|---------|-------|-------|
| **Enable external threat detection** | On | Enables webhook integration |
| **Enable data sharing** | On | Required - see data sharing table in Step 6a |
| **Application ID** | [From Step 6b] | Entra app registration client ID |
| **Endpoint URL** | Your webhook URL | Must be HTTPS, publicly accessible |
| **Default behavior on error** | Block (recommended for FSI) | Fail-closed for regulated environments |

5. Click **Save**

**Important Configuration Decisions:**

| Decision | Recommended for FSI | Rationale |
|----------|-------------------|-----------|
| **Default on timeout** | Block | Fail-closed prevents unvetted tool execution |
| **Default on error** | Block | Assume malicious until verified |
| **Logging** | Verbose | Required for audit trail and incident investigation |

#### Step 6d: FSI Data Sharing Considerations

External threat detection requires sharing sensitive data with the webhook endpoint provider. For FSI organizations, consider:

**Risk Assessment Requirements:**

- [ ] **Vendor risk assessment completed** - Third-party providers handling user prompts and conversation data must undergo vendor due diligence (see [Control 2.7](../pillar-2-management/2.7-vendor-and-third-party-risk-management.md))
- [ ] **Data residency verified** - Confirm webhook endpoint location meets data residency requirements
- [ ] **Data retention policy reviewed** - Understand how long provider retains shared data
- [ ] **Encryption in transit confirmed** - HTTPS required; verify TLS 1.2+ enforcement
- [ ] **SOC 2 / ISO 27001 attestation** - Third-party providers should provide security certifications

**Regulatory Considerations:**

| Regulation | External Threat Detection Implication |
|------------|--------------------------------------|
| **GLBA 501(b)** | User prompts may contain NPI - provider must meet GLBA safeguards |
| **FINRA 4511** | Conversation data may be a "communication" subject to retention |
| **SEC 17a-4** | Tool invocations involving customer data may require preservation |
| **SOX 302/404** | Provider access to financial systems data requires controls assessment |

**Data Minimization Recommendations:**

1. If using a third-party provider (not Microsoft Defender), evaluate whether the provider offers:
   - Data anonymization options
   - Configurable data sharing scope
   - Regional endpoint options for data residency

2. Document the business justification for external threat detection, including:
   - Threat categories addressed
   - Alternative controls considered
   - Risk acceptance for data sharing

#### Step 6e: Microsoft Defender Integration Option

For organizations preferring a Microsoft-first approach, integrate with Microsoft Defender for Cloud Apps instead of (or in addition to) a custom webhook.

**Portal Path:** [Microsoft Defender Portal](https://security.microsoft.com) → **Cloud apps** → **Connected apps** → **App connectors**

1. Navigate to **Microsoft Defender Portal**
2. Go to **Cloud apps** → **Connected apps** → **App connectors**
3. Click **+ Connect an app** → **Microsoft Power Platform**
4. Follow connection wizard to authorize Defender access
5. Enable relevant policies:
   - **Anomalous activity detection** - Unusual agent behavior patterns
   - **Session policies** - Real-time controls during agent interactions
   - **Access policies** - Conditional access for agent usage

**Defender Integration Benefits for FSI:**

| Benefit | Description |
|---------|-------------|
| **Microsoft data handling** | Data stays within Microsoft ecosystem |
| **Unified security view** | Correlates with identity, endpoint, and email signals |
| **Built-in compliance** | Microsoft compliance certifications apply |
| **No custom webhook maintenance** | Managed service, no infrastructure to operate |
...
### Step 8: Configure SIEM Integration (Recommended for higher-risk tiers; SOC-dependent)
...
External Threat Detection: Not applicable (preview feature)
...
External Threat Detection: Optional (requires vendor risk assessment)
Response SLA: 15 minutes
Incident Playbook: Required
```

> **Note:** External threat detection via custom webhooks is a preview feature. For Zone 3 regulated environments, evaluate GA status before deployment. If using third-party webhook providers, complete vendor risk assessment per [Control 2.7](../pillar-2-management/2.7-vendor-and-third-party-risk-management.md) before enabling.
...
7. **Verify External Threat Detection Configuration (Zone 3, if enabled):**
   - Power Platform Admin Center → Security → Threat detection → Additional threat detection
   - **EXPECTED:** External threat detection shows as enabled with configured endpoint
   - Test webhook connectivity by triggering an agent tool invocation
   - **EXPECTED:** Webhook receives request and returns allow/block decision within 1 second

8. **Verify Entra App Registration for Webhook:**
   - Microsoft Entra Admin Center → App registrations → CopilotStudio-ThreatDetection-Webhook
   - **EXPECTED:** App registration exists with Federated Identity Credential configured
   - Verify FIC subject identifier matches expected format
...
- [ ] Screenshot: External threat detection configuration (Zone 3, if enabled)
- [ ] Screenshot: Entra app registration with FIC configured (if using custom webhook)
- [ ] Documentation: Vendor risk assessment for third-party threat detection provider (if applicable)
...
**Symptoms:** Agent tool invocations timing out or defaulting to fallback behavior
...
4. Verify Federated Identity Credential is configured correctly:
   ...
   - Subject identifier format is correct (base64 encoding)
   - Audience is `api://AzureADTokenExchange`
5. Test endpoint manually using curl/Postman to verify connectivity
6. Review webhook endpoint logs for authentication errors

### Issue: External Threat Detection Blocking Legitimate Tool Calls

**Symptoms:** Valid agent tool invocations being blocked by external provider

**Solutions:**

1. Review external provider's evaluation criteria and thresholds
2. Check if specific tool names or parameters are triggering false positives
3. Temporarily set default behavior to "Allow" to isolate webhook vs. built-in protection
4. Work with provider to tune detection rules for your use case
5. Consider using Microsoft Defender integration for better built-in tuning
...
1. Regenerate the FIC subject identifier with correct base64 encoding
2. Verify tenant ID is correct in both issuer URL and subject
3. Confirm webhook endpoint URL in subject matches exactly (including trailing slashes)
4. Check app registration has not been modified or deleted
5. Review Entra sign-in logs for detailed authentication error messages
...
- [External Threat Detection for Copilot Studio](https://learn.microsoft.com/en-us/microsoft-copilot-studio/external-security-provider) (Preview)
- [Power Platform Security Best Practices](https://learn.microsoft.com/en-us/power-platform/admin/security/security-overview)
- [Microsoft Sentinel Power Platform Connector](https://learn.microsoft.com/en-us/azure/sentinel/business-applications/solution-overview)
...
| [Control 2.7](../pillar-2-management/2.7-vendor-and-third-party-risk-management.md) | Third-party webhook provider risk assessment |
