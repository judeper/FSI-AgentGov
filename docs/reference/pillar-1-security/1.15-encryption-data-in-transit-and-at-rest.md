...
### Step 8: Customer Managed Keys for Copilot Studio (Preview)

!!! warning "Preview Feature"
    Customer Managed Keys (CMK) for Copilot Studio environments is currently in **Preview** (April 2025). Features and configuration may change before general availability. Monitor Microsoft documentation for updates.

**Purpose:** Customer Managed Keys for Copilot Studio enables organizations to control encryption keys for agent data stored in Dataverse, including conversation logs, knowledge base configurations, and agent definitions. This is a common FSI requirement for data sovereignty and regulatory compliance.

**Prerequisites:**

- Azure subscription with Key Vault Premium (HSM) access
- Power Platform environment configured as Managed Environment
- Microsoft 365 E5 or E5 Compliance license

**Configuration:**

1. **Create Azure Key Vault (if not existing):**
   - Navigate to Azure Portal → Key Vaults → Create
   - Select **Premium** SKU for HSM support
   - Enable **Soft Delete** and **Purge Protection** (required)

2. **Configure Copilot Studio CMK:**
   - Navigate to **Power Platform Admin Center**
   - Select **Environments** → [Your Copilot Studio Environment]
   - Click **Settings** → **Encryption**
   - Select **Customer-managed key**
   - Provide Azure Key Vault URI and select key

3. **Verify CMK Status:**
   ```powershell
   # Check environment encryption status
   $env = Get-AdminPowerAppEnvironment -EnvironmentName "<Environment-ID>"
   $env.Properties.encryptionKeySource
   # Should return: "Microsoft.Keyvault"
   ```

**Zone Configuration:**

| Zone | CMK Requirement | Key Storage |
|------|-----------------|-------------|
| **Zone 1** | Not required | Microsoft-managed |
| **Zone 2** | Recommended | Azure Key Vault Standard |
| **Zone 3** | **Required** | Azure Key Vault Premium (HSM) |

**FSI Regulatory Alignment:**

- **GLBA 501(b)** - Organization control of encryption for customer NPI
- **SOX 404** - Internal control over encryption key management
- **OCC Data Security** - Data sovereignty requirements

---

### Step 9: Configure Key Rotation Schedule
