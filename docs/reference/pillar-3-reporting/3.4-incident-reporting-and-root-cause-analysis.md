...
**Regulatory Reference:** GLBA 501(b), SOX 404, FINRA 4511, SEC Rule 17a-4, FFIEC IT Handbook, SEC Regulation S-P (2024 Amendments)
...
### Step 5a: RAG Citation Logging for Incident Investigation

**Purpose:** When investigating AI agent incidents (especially data quality or hallucination issues), understanding which knowledge sources the agent cited is critical for root cause analysis. Citation logging provides source attribution for agent responses.

**Citation Data Fields:**

| Field | Description | Use in RCA |
|-------|-------------|------------|
| **Source URL** | Knowledge source location | Verify source accuracy |
| **Source Title** | Document/page title | Quick identification |
| **Citation Timestamp** | When source was accessed | Check for staleness |
| **Confidence Score** | Model's confidence in citation | Assess reliability |
| **Snippet Used** | Text passage cited | Validate interpretation |
| **Response Context** | How citation was used | Understand agent reasoning |

**Citation Audit Query (KQL):**

```kql
// Retrieve citations for a specific incident
let incidentTimeRange = datetime(2026-01-15T10:00:00Z);
let incidentAgent = "AGT-001";
CopilotInteraction
| where TimeGenerated between ((incidentTimeRange - 1h) .. (incidentTimeRange + 1h))
| where AgentId == incidentAgent
...
| project
    ...
    ResponseSnippet = substring(ResponseText, 0, 500)
| order by TimeGenerated asc
```

**Enhanced RCA Template Fields for Citation Investigation:**

Add these fields to Section 3 (Root Cause Analysis) for AI agent incidents:

```plaintext
3a. SOURCE ATTRIBUTION ANALYSIS
    Sources Cited in Incident Response:
    | Source | URL | Confidence | Issue Found |
    |--------|-----|------------|-------------|
    | [Source 1] | [URL] | [Score] | [Yes/No - Details] |

    Citation Issues Identified:
    - [ ] Source was outdated (last modified: [date])
    - [ ] Source contained incorrect information
    - [ ] Agent misinterpreted source content
    - [ ] Source conflicted with other sources
    - [ ] Agent fabricated citation (source doesn't exist)
    - [ ] Low confidence citation presented as certain

    Knowledge Source Remediation:
    - Source requiring update: [URL]
    - Corrective action: [Description]
    - Owner: [Content Owner]
```

**PowerShell Script for Citation Investigation:**

```powershell
function Get-IncidentCitations {
    param(
        [Parameter(Mandatory=$true)]
        [string]$IncidentId,
        [datetime]$IncidentTime,
        [string]$AgentId,
        [string]$OutputPath = ".\Citation-Investigation-$IncidentId"
    )

    Write-Host "Investigating citations for incident: $IncidentId" -ForegroundColor Cyan

    # Query audit logs for the incident timeframe
    $startTime = $IncidentTime.AddHours(-1)
    $endTime = $IncidentTime.AddHours(1)

    $events = Search-UnifiedAuditLog -StartDate $startTime -EndDate $endTime `
    ...
    foreach ($event in $events) {
        ...
        if ($data.AgentId -eq $AgentId -and $data.Citations) {
    ...
    $citations | Export-Csv -Path "$OutputPath\Citations.csv" -NoTypeInformation

    Write-Host "Found $($citations.Count) citations" -ForegroundColor Green
    Write-Host "Exported to: $OutputPath\Citations.csv"

    # Analyze for common issues
    Write-Host "`nCitation Analysis:" -ForegroundColor Yellow

    # Check for low confidence citations
    $lowConfidence = $citations | Where-Object { $_.Confidence -lt 0.7 }
    if ($lowConfidence.Count -gt 0) {
        Write-Host "  WARNING: $($lowConfidence.Count) low-confidence citations" -ForegroundColor Red
    }

    # Group by source
    $sourceGroups = $citations | Group-Object SourceUrl | Sort-Object Count -Descending
    Write-Host "  Most cited sources:"
    $sourceGroups | Select-Object -First 5 | ForEach-Object {
        Write-Host "    - $($_.Name): $($_.Count) citations"
    }
...
# Usage example:
# Get-IncidentCitations -IncidentId "INC-2026-0115-001" -IncidentTime "2026-01-15 10:30:00" -AgentId "AGT-001"
```

**Integration with Incident Tracking:**

Update the incident tracking SharePoint list (Step 2) to include citation fields:

| Column | Type | Purpose |
|--------|------|---------|
| CitationIssueDetected | Yes/No | Flag if citation was problematic |
| ProblematicSource | Hyperlink | Link to incorrect source |
| CitationRootCause | Choice | Outdated/Incorrect/Misinterpreted/Fabricated/Conflicting |
| SourceRemediationOwner | Person | Content owner for fix |
...
| **SEC Regulation S-P (2024)** | Customer breach notification | Unauthorized access to customer information | 30 days to affected customers |

### Notification SLA Framework

!!! warning "SEC Regulation S-P Amendments (2024) - Customer Notification"
    The SEC's 2024 amendments to Regulation S-P establish a **mandatory 30-day customer notification requirement** for incidents involving unauthorized access to customer information. Covered institutions (broker-dealers, investment advisers, investment companies, transfer agents) must notify affected customers within 30 days of discovering an incident. Compliance deadline varies by institution type (2025-2026).

**Three-Tier Notification SLA Model:**

Organizations must distinguish between three types of notification requirements with different timelines:

| Notification Type | Target | SLA | Regulatory Basis |
|-------------------|--------|-----|------------------|
| **Internal Response** | Security/Incident team | 15 min - 24 hours (by severity) | Internal policy |
| **Regulatory Notification** | Regulators/Supervisors | 24-72 hours | GLBA, FFIEC, OCC, Fed |
| **Customer Notification** | Affected customers | **30 days** | **SEC Regulation S-P** |

**Internal Response SLAs (Existing):**

| Severity | Response SLA | Escalation |
|----------|--------------|------------|
| Critical (P1) | 15 minutes | Immediate - CISO/CCO |
| High (P2) | 1 hour | 4 hours - Director |
| Medium (P3) | 4 hours | 24 hours - Manager |
| Low (P4) | 24 hours | 48 hours - Team Lead |

**Regulatory Notification SLAs:**

| Regulator | Notification Window | Trigger |
|-----------|--------------------| --------|
| OCC (National Banks) | 36 hours | Incident affecting customer data or operations |
| Federal Reserve | 36 hours | Security incident at supervised institution |
| FDIC | 36 hours | Computer-security incident |
| State Banking Regulators | 24-72 hours (varies) | State breach notification laws |
| FINRA (Broker-Dealers) | Prompt notification | Material incident affecting operations |
| SEC (Investment Advisers) | Varies | Material incident requiring Form ADV update |

**Customer Notification SLAs (SEC Regulation S-P):**

| Requirement | Timeline | Details |
|-------------|----------|---------|
| Discovery determination | Prompt | Institution must reasonably determine incident scope |
| Customer notification deadline | **30 days from discovery** | Written notice to affected customers |
| Notice content | Required elements | What happened, data involved, customer steps, contact info |
| Service provider incidents | Extended coordination | May coordinate with service provider but institution remains responsible |
| Senior management notification | Immediate | Internal escalation upon discovery |

### SEC Regulation S-P Incident Classification

For incidents potentially triggering Regulation S-P notification requirements:

| Classification | Description | Notification Required |
|----------------|-------------|----------------------|
| **Confirmed Breach** | Unauthorized access to customer information confirmed | Yes - within 30 days |
| **Probable Breach** | Evidence strongly suggests unauthorized access occurred | Yes - within 30 days of confirmation |
| **Potential Breach** | Incident under investigation, access uncertain | Prepare notification; issue upon confirmation |
| **Near Miss** | Attempted access prevented; no customer data exposed | No customer notification; document internally |
| **Non-Customer Data** | Incident involved non-customer data only | No S-P notification; other requirements may apply |

**Regulation S-P Incident Response Integration:**

```plaintext
INCIDENT INVOLVING CUSTOMER INFORMATION
========================================
T+0:00    - Incident detected
T+0:15    - Initial severity assessment (P1/P2/P3/P4)
T+1:00    - Determine if customer information potentially affected
T+4:00    - Preliminary scope assessment
T+24:00   - Regulatory notification (if required by threshold)
T+48:00   - Customer impact assessment complete
T+72:00   - Senior management briefing on S-P applicability
...
T+30 days - DEADLINE: Customer notification (if S-P applies)
...
**Updated:** Jan 2026
**Version:** v1.1 (Jan 2026) - Added SEC Regulation S-P 30-day notification requirements
