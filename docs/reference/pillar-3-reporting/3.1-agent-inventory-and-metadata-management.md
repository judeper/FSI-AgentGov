...
!!! info "Microsoft Agent 365 - Future Consolidated Registry"
    **Microsoft Agent 365** (currently in Frontier preview) will provide a **centralized agent registry** that consolidates agent discovery across Copilot Studio, M365 Agent Builder, and partner agents. Key capabilities include:

    - **Unified agent inventory** - Single pane of glass for all organizational agents
    - **Entra Agent ID integration** - Automatic identity assignment for agents
    - **Lifecycle management** - Publish, version, assign ownership, retire from one location
    - **Observability dashboards** - Real-time monitoring and alerting

    When Agent 365 reaches general availability, it will reduce the need for manual reconciliation between M365 Admin Center and Power Platform Admin Center inventories. FSI organizations participating in the Frontier program should evaluate early. Monitor for GA announcements.
...
| Business Continuity | [Control 2.4](../pillar-2-management/2.4-business-continuity-and-disaster-recovery.md) | RTO/RPO definitions for NYDFS Part 500 compliance fields |
...
### NYDFS Part 500 Compliance Fields (Tier 2-3)

These fields support NYDFS Part 500 §500.13 (Asset Inventory and Device Management) requirements, effective November 1, 2025.

| Field | Description | Format | Required |
|-------|-------------|--------|----------|
| `recovery_time_objective_minutes` | Maximum acceptable downtime (RTO) | Integer (minutes) | Yes |
| `recovery_point_objective_minutes` | Maximum acceptable data loss (RPO) | Integer (minutes) | Yes |
| `support_expiration_date` | Vendor support end-of-life date | Date (YYYY-MM-DD) | Yes |
| `criticality_tier` | Business criticality classification | 1-4 (1=Critical, 4=Low) | Yes |
| `backup_compliance_status` | Backup and recovery compliance | compliant/non-compliant/pending | Yes |
| `operational_state` | Current operational status | active/disabled/decommissioned | Yes |

!!! info "Cross-Reference: Business Continuity"
    RTO and RPO values must align with [Control 2.4: Business Continuity and Disaster Recovery](../pillar-2-management/2.4-business-continuity-and-disaster-recovery.md) definitions and testing requirements.

**NYDFS Part 500 Inventory Example:**

```yaml
# Agent Inventory Record - NYDFS Compliance Fields
agent_id: "AGT-NYC-2026-001"
agent_name: "Customer Account Inquiry Bot"
nydfs_compliance:
  recovery_time_objective_minutes: 60      # 1-hour RTO
  recovery_point_objective_minutes: 15     # 15-minute RPO
  support_expiration_date: "2027-12-31"    # Platform support end date
  criticality_tier: 2                      # Important but not critical
  backup_compliance_status: "compliant"    # Tested and verified
  operational_state: "active"              # Currently in production
  last_backup_test_date: "2026-01-10"
...
| NYDFS Part 500 | Maintain asset inventory with cybersecurity classification, RTO/RPO, and support status |
...
| **NYDFS Part 500** | Asset inventory and cybersecurity (§500.13) | Track RTO/RPO, support expiration, criticality tier, backup compliance status for all information systems including AI agents |
...
**Updated:** Jan 2026
**Version:** v1.1 (Jan 2026) - Added NYDFS Part 500 compliance fields
