...
### Step 8: Token-Level Cost Monitoring

**Purpose:** For agents using Azure OpenAI or other consumption-based AI services, tracking costs at the token level provides granular visibility into usage patterns and enables early detection of runaway queries or unexpected cost spikes.
...
├─────────────────────────────────────────────────────────────────┤
│  Agent Request → Azure OpenAI → Token Meter → Cost Attribution  │
│                         ↓                           ↓           │
│               Usage Metrics API          Per-Agent Aggregation  │
│                         ↓                           ↓           │
│               Log Analytics            Chargeback/Reporting     │
└─────────────────────────────────────────────────────────────────┘
```

#### Azure OpenAI Token Tracking Configuration

**Portal Path:** Azure Portal → Azure OpenAI Resource → **Metrics**

1. Navigate to your Azure OpenAI resource
2. Select **Metrics** from the left menu
3. Configure token tracking metrics:

| Metric | Description | Recommended Alert |
|--------|-------------|-------------------|
| `Processed Inference Tokens` | Total tokens processed | >100,000/hour |
| `Processed Prompt Tokens` | Input tokens (prompts) | >50,000/hour |
| `Processed Completion Tokens` | Output tokens (responses) | >50,000/hour |
| `Token Transaction Count` | Number of API calls | >1,000/hour |

**Enable Diagnostic Logging:**

1. Navigate to Azure OpenAI resource → **Diagnostic settings**
2. Click **+ Add diagnostic setting**
3. Configure:
   - **Name:** `aoai-token-logs`
   - **Logs:** RequestResponse (includes token counts)
   - **Destination:** Log Analytics workspace
4. Save

#### Per-Agent Token Attribution

To attribute token usage to specific agents, implement request tagging:

**Custom Dimensions for Agent Attribution:**

| Dimension | Purpose | Implementation |
|-----------|---------|----------------|
| `AgentId` | Link usage to agent inventory | Custom header or user field |
| `SessionId` | Track conversation costs | Copilot Studio session ID |
| `Environment` | Environment-level attribution | Resource tags |
| `CostCenter` | Business unit allocation | Custom metadata |

**Log Analytics Query for Per-Agent Token Usage:**

```kusto
// Per-agent token consumption (last 7 days)
AzureDiagnostics
| where ResourceProvider == "MICROSOFT.COGNITIVESERVICES"
| where Category == "RequestResponse"
| where TimeGenerated > ago(7d)
| extend AgentId = tostring(parse_json(properties_s).agentId)
...
| summarize
    ...
    RequestCount = count()
    by AgentId
| extend EstimatedCost = TotalTokens * 0.00002  // Adjust rate per model
| order by TotalTokens desc
```

#### Token Anomaly Detection

Configure alerts for unusual token consumption patterns:

**Anomaly Detection Scenarios:**

| Scenario | Detection Method | Alert Action |
|----------|-----------------|--------------|
| **Runaway Query** | Single request >10,000 tokens | Immediate alert to platform team |
| **Usage Spike** | >200% of baseline in 1 hour | Alert to agent owner |
| **Cost Threshold** | Daily spend exceeds $100/agent | Alert to cost center owner |
| **Off-Hours Usage** | Significant usage outside business hours | Security review trigger |

**Azure Monitor Alert Rule for Token Spike:**

```json
{
  "alertRuleName": "TokenUsageSpike",
  "condition": {
    "allOf": [
      {
  ...
  "actions": [
    {
      "actionGroupId": "/subscriptions/{sub}/resourceGroups/{rg}/providers/Microsoft.Insights/actionGroups/ai-ops-alerts"
    }
  ]
}
```
...
**Current Azure OpenAI Pricing Reference (subject to change):**
...
!!! note
    Pricing varies by region and agreement. Consult your Azure agreement for current rates.

**Token Cost Attribution Formula:**

```
Agent Monthly Cost = (Prompt Tokens × Prompt Rate) + (Completion Tokens × Completion Rate)
```

#### Cost-Per-Interaction Reporting

Track and report average cost per agent interaction:

**Cost-Per-Interaction Dashboard Metrics:**

| Metric | Calculation | Target |
|--------|-------------|--------|
| Avg Tokens/Interaction | Total tokens / Total sessions | <2,000 |
| Avg Cost/Interaction | Total token cost / Total sessions | <$0.05 |
| Cost Efficiency Trend | Month-over-month cost per interaction | Declining or stable |
| High-Cost Interactions | Sessions exceeding $0.50 | <1% of total |

**Power BI Measures for Token Reporting:**

```dax
// Average cost per interaction
Avg Cost Per Interaction =
    DIVIDE(
        SUM('TokenUsage'[TotalCost]),
        DISTINCTCOUNT('TokenUsage'[SessionId])
    )

// Token efficiency score
Token Efficiency =
    DIVIDE(
        SUM('TokenUsage'[CompletionTokens]),
        SUM('TokenUsage'[PromptTokens])
    )
```

#### Token Governance by Zone

| Zone | Token Monitoring | Budget Control | Alert Sensitivity |
|------|-----------------|----------------|-------------------|
| **Zone 1** | Aggregate only | Soft cap | Weekly summary |
| **Zone 2** | Per-agent tracking | Per-agent budget | Daily alerts at 80% |
| **Zone 3** | Per-interaction tracking | Real-time enforcement | Immediate alerts |

#### Token Monitoring Checklist

```markdown
# Token Monitoring Implementation Checklist

## Azure OpenAI Configuration
- [ ] Diagnostic logging enabled
- [ ] Metrics configured for token tracking
- [ ] Log Analytics workspace receiving data
- [ ] Data retention set per compliance requirements

## Per-Agent Attribution
- [ ] Agent ID tagging implemented
- [ ] Cost center mapping configured
- [ ] Session-level tracking enabled

## Alerting
- [ ] Runaway query alert configured
- [ ] Usage spike detection enabled
- [ ] Budget threshold alerts active
- [ ] Alert recipients documented

## Reporting
- [ ] Per-agent token report created
- [ ] Cost-per-interaction dashboard built
- [ ] Monthly token summary automated
- [ ] Chargeback integration tested
