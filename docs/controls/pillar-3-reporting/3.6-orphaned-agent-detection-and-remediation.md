...
!!! tip "Agent 365 + Entra Agent ID Architecture"
    Where licensing, region, and tenant rollout permit, Agent 365 lifecycle governance (GA May 1, 2026) and Microsoft Entra Agent ID lifecycle workflows (GA, with the Lifecycle Workflows agent-sponsor task type still labelled **Preview** in Microsoft Learn as of the verification date) reduce reliance on manual PowerShell-based orphan scans. Both surfaces require Microsoft Agent 365 or Microsoft 365 E7 ("Frontier Suite") licensing. Until tenant parity is confirmed, treat the automated path as a supplement to — not a replacement for — the multi-surface PowerShell and reconciliation workflows in this control. See Controls [2.25](../pillar-2-management/2.25-agent-365-admin-center-governance-console.md) and [2.26](../pillar-2-management/2.26-entra-agent-id-identity-governance.md) for current availability and prerequisites.
