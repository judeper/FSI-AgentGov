...
!!! danger "⚠️ Action Required by July 1, 2026 — Two Changes Affecting Agent Discovery"
    Two licensing and discovery changes take effect on **2026-07-01**:

    1. **AI Agent Inventory** — After 2026-07-01, Defender for Cloud Apps alone no longer provides AI Agent Inventory. **Agent 365** (or Microsoft 365 E7 / Frontier Suite) is required. Organizations without it will lose AI Agent Inventory visibility entirely. See [Control 3.7](3.7-ppac-security-posture-assessment.md) for details.
    2. **Third-party cloud agent discovery** — Third-party and multicloud cloud agents are no longer discoverable through Microsoft Defender for Cloud connectors. Transition to **Registry sync** (currently Preview) in the Microsoft 365 agent registry before this date. See the [discovery warning](#agent-365-admin-center-ownerless-agents-governance-card) within this control's Control Description.
...
|---|---|
| FINRA Rule 3110 (Supervision) | A documented orphan-detection and remediation workflow supports — but does not replace — registered-principal supervisory review... |
| FINRA Rule 4511 / FINRA RN 24-09 | Unowned agents may generate records without proper supervision; orphan registers and remediation evidence are records of business... |
| SEC Rules 17a-3 / 17a-4 | Orphan registers, detection-run logs, remediation tickets, and reassignment approvals are records of business and must be... |
| SOX Sections 302 / 404 | Orphan remediation evidence is part of the IT general-controls population that supports management's quarterly and annual... |
...
| NYDFS 23 NYCRR 500 | Section 500.7 (access privileges) and 500.13 (asset management) require periodic review and termination of access for departed... |
