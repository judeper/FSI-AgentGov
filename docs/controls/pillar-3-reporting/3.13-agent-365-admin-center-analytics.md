...
!!! info "Frontier Preview — Hero Metrics Require Enrollment"
    The **Agent 365 hero metrics** displayed on the Overview page (Active Users, Total Sessions, Exception Rate, Agent Runtime) require enrollment in the **Microsoft 365 Frontier program**. The **Agent Registry** (agent inventory listing) is **Generally Available** and accessible to all tenants. Financial institutions not yet enrolled in Frontier should implement the GA Agent Registry capabilities immediately and plan Frontier enrollment to unlock full analytics. Metric availability and UI layout are subject to change during the Preview period. Last UI verification: March 2026.
...
| **Hero Metric: Active Users** | Count of unique users who interacted with at least one agent during the last 30-day period. Measures adoption scope and identifies departments with high agent engagement. (Frontier Preview) |
| **Hero Metric: Total Sessions** | Count of complete agent invocations in which an agent performed a task or answered a query. Provides volumetric baseline for workload assessment and capacity planning. (Frontier Preview) |
| **Hero Metric: Exception Rate** | Percentage of agent sessions that completed without errors (i.e., the agent successfully finished its intended task). A declining exception rate is a leading indicator of degraded agent reliability. (Frontier Preview) |
| **Hero Metric: Agent Runtime** | Total agent-assisted time, computed as the sum of session durations (end time minus start time) across all agents in the measurement window. Quantifies the operational dependency on agent automation. (Frontier Preview) |
...
- **Role Requirements**: Global Administrator or Microsoft 365 Copilot Admin role required for full Analytics access. Usage Reports Reader provides limited read-only access.
...
| Role | Responsibility |
|---|---|
| **Global Administrator / M365 Copilot Admin** | Maintain access to Agent 365 Admin Center. Perform or delegate weekly/monthly analytics reviews. Disposition pending agent deployment requests within defined SLA. Assign owners to ownerless agents. |
...
| **3.2 — Usage Analytics** | Operational usage detail complement. 3.2 covers granular per-agent usage data; 3.11 provides the tenant-level supervisory dashboard aggregating usage across all agents. |
...
| **2.25 — Agent 365 Admin Center Governance Console** | Governance actions companion. 2.25 covers deployment approval workflows and policy enforcement; 3.11 covers the analytics and reporting layer of the same Admin Center. |
| **3.12 — Agent 365 Observability SDK** | Custom agent telemetry feeder. Agents instrumented with the 3.12 Observability SDK contribute session, exception rate, and runtime data to the 3.11 hero metrics dashboard. Without 3.12 implementation, custom agent data is absent from 3.11 metrics. |
...
1. Global Administrator or M365 Copilot Admin can successfully navigate to M365 Admin Center > Agents > Overview and view the Agent Registry count.
...
- [Microsoft Learn: Microsoft Agent 365 overview](https://learn.microsoft.com/en-us/microsoft-agent-365/overview) *(Explains Frontier preview prerequisites and hero-metric availability)*
...
!!! note "Documentation Currency"
    Agent 365 Admin Center UI and feature availability are subject to change during the Frontier Preview period. Verify all navigation paths and feature availability against current Microsoft Learn documentation before implementing. This control was last UI-verified in March 2026.
