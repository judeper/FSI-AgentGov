...
Copilot Agent Kit (formerly Copilot Studio Kit) is a Power CAT-maintained toolkit that supplements, rather than replaces, native Power Platform admin center and Agent 365 governance:

- **Agent Inventory:** Provides tenant-wide, feature-level visibility for Copilot Studio agents.
...
Treat these components as optional accelerators. Verify current licensing, dependencies, security roles, connector policy, and supportability before deployment. Keep the authoritative registry and regulatory evidence in the firm's approved systems; do not characterize Copilot Agent Kit, Power Shield, or any Kit dashboard as Agent 365 itself or as a runtime interception layer.
