---
description: "Centralized Agent Inventory Enforcement: Establish enforcement mechanisms for centralized agent inventory completeness through automated detection, mandatory…"
---
# Control 3.11: Centralized Agent Inventory Enforcement

**Control ID:** 3.11<br>
**Pillar:** Reporting<br>
**Regulatory Reference:** FINRA 4511, FINRA RN 24-09, FINRA 3110, SEC 17a-4, SOX 404, OCC Bulletin 2026-13 (formerly OCC 2011-12), Fed SR 26-2 (formerly Fed SR 11-7)<br>
...
!!! info "Agent Inventory Feature Status (source-checked September 2026)"
    Power Platform Inventory in PPAC (which surfaces Microsoft Copilot Studio agents alongside apps, flows, environments, and environment groups) reached **General Availability on February 9, 2026** (MC1223778). Inventory data typically refreshes within 20 minutes ([Microsoft Copilot Studio Agent inventory](https://learn.microsoft.com/microsoft-copilot-studio/admin-agent-inventory)) and supports filtering, sorting, column customization, and Excel export. The **Power Platform for Admins V2** connector lets Power Automate flows query the same inventory data without a custom API. Microsoft Agent 365 became generally available for the Commercial segment on **May 1, 2026** as a per-user licensed control plane. Microsoft documents human administrative access through Microsoft 365 admin roles; govern agent identity and access separately through the applicable Agent 365 and Entra controls. Microsoft Entra Agent ID reached general availability in April 2026 ([Entra Agent ID — What's new](https://learn.microsoft.com/en-us/entra/agent-id/whats-new-agent-id)); verify per-tenant availability in the Entra portal.

    **Feature Status Tracking:**

    | Capability | Status / availability | Implementation Approach |
    |------------|-------------------|-------------------------|
    | Power Platform Inventory (PPAC) | **GA** (Feb 9, 2026) | Primary discovery interface; ~20-minute refresh |
    | Power Platform for Admins V2 connector | GA | Workflow-driven inventory queries in Power Automate |
    | Microsoft Agent 365 control plane | GA May 1, 2026 for the Commercial segment; per-user licensing | Unified discovery, lifecycle, and governance control plane |
    ...
    | Copilot Agent Kit (formerly Copilot Studio Kit) | Component-specific availability | Supporting Agent Inventory and Compliance Hub toolkit; verify component supportability |
...
|---|---|
| FINRA 4511 / FINRA RN 24-09 | Books and records requirements necessitate a complete and accurate inventory of all systems and tools used in securities... |
| SEC 17a-4 | Records-retention obligations apply to communications and decisions produced by AI agents; the firm must be able to enumerate... |
| SOX 404 | IT general controls over financial reporting systems require a documented inventory of all applications with financial-data... |
| OCC Bulletin 2026-13 (formerly OCC 2011-12) | Model Risk Management guidance requires inventory and ongoing monitoring of all automated decision-making systems. |
| Fed SR 26-2 (formerly Fed SR 11-7) | Supervisory guidance on model risk management calls for a comprehensive model inventory with ownership, purpose, and risk ratings. |
...
??? note "Regulatory mapping — click to expand"
    ...
    - **Fed SR 26-2 (formerly Fed SR 11-7):** Supervisory guidance on model risk management calls for a comprehensive model inventory with ownership, purpose, and risk ratings. Unmanaged agents introduce unquantified operational risk
...
Organizations adopting Agent 365 should align inventory enforcement processes with the generally available control plane while retaining the established reconciliation and evidence sources required by this control.

### Copilot Agent Kit: Supporting Inventory and Compliance Evaluation

Copilot Agent Kit (formerly Copilot Studio Kit) is a separate Power CAT-maintained toolkit for Copilot Studio that can complement governance processes using native Power Platform admin center and Agent 365 surfaces:

- **Agent Inventory:** Provides tenant-wide, feature-level visibility for Copilot Studio custom agents and declarative agents, subject to the connection account's access across environments.
- **Compliance Hub:** Evaluates collected agent configurations against administrator-defined controls and can apply configured risk levels, SLA timers, and enforcement actions such as manual review, quarantine, or delete.
- **Maker notifications:** Can guide remediation through Microsoft Teams or Outlook notifications when Compliance Hub findings require action.

Treat these components as optional accelerators. Verify current licensing, dependencies, security roles, connector policy, and environment coverage before deployment. Microsoft Support covers the underlying platform capabilities, while accelerator-specific issues are handled through the Kit's GitHub repository. Keep the authoritative registry and regulatory evidence in the firm's approved systems; do not characterize Copilot Agent Kit, Power Shield, or any Kit dashboard as Agent 365 itself or as a runtime interception layer.
