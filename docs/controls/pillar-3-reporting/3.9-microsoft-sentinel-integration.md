...
!!! warning "Connector Availability (January 2026)"
    **No dedicated Copilot Studio connector exists for Microsoft Sentinel.** Copilot Studio administrative events are captured via the **Power Platform Admin Activity** connector, which logs to the `PowerPlatformAdminActivity` table in Log Analytics. For comprehensive agent telemetry (conversation logs, CSAT, custom events), organizations must implement custom integration paths through Application Insights → Log Analytics → Sentinel.
...
| **Defender for Cloud Apps** | Defender for Cloud Apps | `CloudAppEvents` | Cloud app activity, shadow IT detection |
