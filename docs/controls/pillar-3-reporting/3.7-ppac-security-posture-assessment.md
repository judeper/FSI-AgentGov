...
| **Connected Agents** | Inter-agent connectivity disabled unless approved | Copilot Studio > Agent > Settings > Connected agents (verify the literal menu label at apply-time — may surface as 'Agents' depending on UI release ring; A2A reached GA in April 2026 but per-agent toggle may still show Preview — [Learn](https://learn.microsoft.com/en-us/microsoft-copilot-studio/authoring-connect-agents)) | [1.18](../pillar-1-security/1.18-application-level-authorization-and-role-based-access-control-rbac.md) | Monthly |
