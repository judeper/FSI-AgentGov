...
- Scope Endpoint DLP policies to specific **device groups** using **dynamic device groups defined in Microsoft Entra ID** — for example, enforce a policy only when users access data from Windows devices and not from macOS. Recommended for Zone 3 to apply differentiated controls by device platform or posture. See [Create a DLP policy that uses device scoping](https://learn.microsoft.com/en-us/purview/endpoint-dlp-create-policy-device-scoping).
- Query Endpoint DLP device configuration and policy-sync attributes at scale via the **`DeviceInfo` table's `DlpInfo` column** in Advanced Hunting (Microsoft Defender portal), rather than relying on point-in-time exports from the Purview portal — aids fleet-wide configuration verification and audit evidence for FINRA 4511 / SEC 17a-4. See [Access Endpoint DLP device attribute data using Advanced Hunting](https://learn.microsoft.com/en-us/purview/dlp-edlp-tshoot-sync#access-device-attribute-data-using-advanced-hunting).
