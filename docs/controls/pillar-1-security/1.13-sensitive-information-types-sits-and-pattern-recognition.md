...
9. EDM rule package shows **Active**; hash refresh executed within the agreed cadence (typically daily); seeded exact-match record detected.
10. NER-backed SITs (e.g., "All full names") return matches on a seeded NER test document (E5 license required).
11. False-positive / false-negative review log maintained on a defined cadence (recommended monthly for Zone 3); findings routed back into SIT tuning and DLP policy adjustments.
12. Trainable classifiers in use have current model-risk validation evidence per OCC Bulletin 2026-13 (formerly OCC 2011-12) / Fed SR 26-2 (formerly SR 11-7) if they drive a primary control.
