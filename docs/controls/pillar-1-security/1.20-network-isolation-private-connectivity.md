...
**Control ID:** 1.20<br>
**Pillar:** Security<br>
**Regulatory Reference:** FINRA Rule 3110, FINRA Regulatory Notice 25-07, SEC Rule 17a-4, GLBA 501(b) Safeguards Rule, NY DFS 23 NYCRR 500.11(a) (vendor / third-party network access), OCC Bulletin 2011-12, Federal Reserve SR 11-7, FFIEC IT Examination Handbook (Information Security — Network Security), NIST SP 800-53 Rev. 5 SC-7 (Boundary Protection), SC-7(11) (Restrict Incoming Communications), SC-7(21) (Isolation of Information System Components), AC-4 (Information Flow Enforcement)<br>
...
Establish network-layer boundary protection for Power Platform, Copilot Studio, Microsoft 365 Copilot–dependent integrations, and Azure-hosted AI services so that agent traffic to data sources, secret stores, AI model endpoints, and telemetry sinks is constrained to trusted, named network paths. The control combines **IP firewall**, **IP-address-based cookie binding**, **virtual network (VNet) integration via subnet delegation**, **Azure Private Link / Private Endpoints** for the resources agents call (Key Vault, Application Insights, Azure SQL, Storage, Azure OpenAI, Azure AI Search, Cognitive Services), **Tenant Restrictions v2** for outbound cross-tenant access, and **NSG / Azure Firewall / Private DNS** for traffic inspection and name resolution.
...
The control is layered. As with encryption (Control 1.15), it is important to distinguish what Microsoft 365 / Power Platform provides **by default** from what requires **explicit customer network configuration** — examiners frequently probe this distinction, and FINRA 25-07 specifically calls out the AI-tooling subset.
...
1. **IP Firewall (Power Platform Managed Environments)** — Restricts Dataverse and Power Platform service access to allowlisted IPv4/IPv6 ranges (up to 200 ranges, 4,000-character limit on the address field). Available on Managed Environments in Commercial, GCC, GCC High, and DoD clouds. Two modes: **Audit only** (logs blocked attempts without blocking) and **Enforce** (blocks).
...
| **Compliance Officer** | Approves the network architecture against the firm's regulatory mapping (FINRA 25-07, NY DFS 500.11(a), GLBA 501(b)); records evidence in the supervisory file; signs the quarterly Zone 3 attestation. |
...
| [Sovereign Cloud Parity Matrix](../../reference/sovereign-cloud-parity-matrix.md) | Per-cloud (GCC, GCC High, DoD IL5, 21Vianet) availability for IP firewall, VNet integration, Private Link / Private Endpoints to AI services, and Tenant Restrictions v2. Consult before designing Zone 3 networks for sovereign-cloud tenants. |
