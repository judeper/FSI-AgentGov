---
description: "Data Loss Prevention (DLP) and Sensitivity Labels: Prevent sensitive financial data from unauthorized exposure through AI agents by implementing DLP policies…"
---
...
|---|---|
| FINRA 4511 | Requires preservation and protection of customer records — DLP rules can help block or warn on customer PII appearing in AI... |
| FINRA Rule 3110 / Regulatory Notice 24-09 | AI-specific supervisory obligations under FINRA's technology-neutral application of existing supervision rules require... |
| SEC Reg S-P (as amended 2024, 17 CFR 248) | Privacy protection — sensitivity labels and DLP help control AI access to customer information. |
| GLBA 501(b) | Safeguard customer information — DLP helps prevent exposure of financial account numbers |
| SOX 404 | IT controls over data confidentiality — DLP event logging contributes to the IT general controls evidence base when paired with... |
| NYDFS 23 NYCRR 500.11 / 500.15 / 500.16 | Third-party service-provider security (500.11), encryption of NPI in transit and at rest (500.15), and... |
| OCC Bulletin 2026-13 (formerly OCC Bulletin 2011-12) / Federal Reserve SR 26-2 (formerly SR 11-7) (Model Risk Management) | Where AI agents are treated as models, DLP forms part of the data-quality and confidentiality controls expected around model... |
| PCI DSS 4.0 (where cardholder data flows) | Requirement 3 (protect stored account data) and Requirement 7 (least privilege) — DLP rules referencing PCI SITs help detect... |
...
| CFPB UDAAP (where consumer financial data is exposed) | Inadvertent disclosure of consumer financial data via AI surfaces can become a UDAAP exposure; DLP-driven egress controls help... |
| FFIEC Guidelines | Data classification — label-based access control for AI agents |

??? note "Regulatory mapping — click to expand"
    - **FINRA 4511:** Requires preservation and protection of customer records — DLP rules can help block or warn on customer PII appearing in AI prompts and responses, but do not by themselves satisfy the books-and-records preservation requirement (see Control 1.9 — Data Retention and Deletion Policies)
...
!!! warning "Non-substitution"
    ...
    - **Records management and preservation** — SEC 17a-4(f) WORM-equivalent retention is satisfied by Control 1.9 — Data Retention and Deletion Policies, not by DLP
...
!!! tip "Automation Available"
    ...
    - [Agent Knowledge Source Scanner](https://github.com/judeper/FSI-AgentGov-Solutions/tree/main/agent-knowledge-source-scanner) — item-level permission scanning for SharePoint libraries backing agent knowledge sources
...
!!! note "Third condition — Email is received from (External users) (preview)"
    Microsoft Learn now documents a third condition for the **Microsoft 365 Copilot and Copilot Chat** DLP location: **Email is received from (External users)** (preview). It is recommended for restricting Copilot from using **external email as grounding data**, which helps reduce prompt-injection exposure from untrusted senders — relevant to the adversarial-input concerns in [Control 1.21](1.21-adversarial-input-logging.md). Treat it as **preview**: validate in a non-production environment and verify rollout status against the [Microsoft 365 Roadmap](https://www.microsoft.com/en-us/microsoft-365/roadmap) and Message Center before relying on it as a sole Zone 3 control. Source: [Learn — DLP policy reference](https://learn.microsoft.com/en-us/purview/dlp-policy-reference) ("Conditions Microsoft 365 Copilot supports").
...
- Pair this subsection with [Control 1.13 - Sensitive Information Types](1.13-sensitive-information-types-sits-and-pattern-recognition.md) so that sanctioned-country, OFAC-list, and routing-identifier SITs are defined once and reused by both Purview DLP and Power Platform data policies.
...
- [OFAC: A Framework for OFAC Compliance Commitments](https://ofac.treasury.gov/media/16331/download?inline) — the five components of an effective OFAC compliance program
- [OFAC Sanctions List Service](https://ofac.treasury.gov/sanctions-list-service) — primary OFAC sanctions-list distribution point (SDN, consolidated lists, sectoral and program lists)
...
- [FFIEC BSA/AML Examination Manual](https://bsaaml.ffiec.gov/manual) — examiner methodology referenced by federal banking regulators
- [FinCEN: BSA Resources for Financial Institutions](https://www.fincen.gov/resources/financial-institutions) — current FinCEN guidance and rule index for 31 CFR Chapter X
- [31 CFR § 1010.230 — Beneficial ownership requirements (CDD Rule)](https://www.ecfr.gov/current/title-31/subtitle-B/chapter-X/part-1010/subpart-C/section-1010.230)
- [31 CFR § 1020.320 — Suspicious activity reporting (banks)](https://www.ecfr.gov/current/title-31/subtitle-B/chapter-X/part-1020/subpart-C/section-1020.320)
- [31 CFR § 1023.320 — Suspicious activity reporting (broker-dealers)](https://www.ecfr.gov/current/title-31/subtitle-B/chapter-X/part-1023/subpart-C/section-1023.320)
