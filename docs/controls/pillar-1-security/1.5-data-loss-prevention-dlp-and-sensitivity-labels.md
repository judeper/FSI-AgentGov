...
|------|----------------------|-----------|
| **Zone 1 (Personal Productivity)** | Allow most connectors as Business; block social channels (Facebook, WhatsApp) | Enable self-service agent development while preventing external data sharing |
...
|---------|---------|--------------|-------------|
| `*.internal.yourbank.com` | Internal corporate APIs | `https://api.internal.yourbank.com/customer` | Allow (Zone 3) |
| `*.yourdomain.com` | Corporate domain APIs | `https://api.yourdomain.com/*` | Allow (Zone 3) |
...
| `*.marketdataprovider.com` | Approved market data vendors (Bloomberg, Refinitiv) | `https://api.marketdataprovider.com/v2/` | Allow (Zone 2-3, with vendor approval) |
| `https://*.social-network.com/*` | Social media APIs (LinkedIn, Twitter, Facebook) | `https://api.twitter.com/2/tweets` | Block (Zone 2-3) |
| `https://*.file-sharing.com/*` | Consumer file sharing (Dropbox, Box personal) | `https://api.dropbox.com/2/files/` | Block (Zone 3) |
| `https://api.*.free-tier.com/*` | Free-tier external APIs (no SLA, no BAA) | Various | Block (Zone 3) |
...
https://api.partner-bank.com/*   # Approved partner bank APIs (with BAA)
```
...
!!! tip "Zone 3 Best Practice"
    For customer-facing agents, configure HTTP endpoint filtering to allow only pre-approved external APIs. Combine with network isolation (Control 1.20) for defense in depth. Document all allowed external endpoints in your IT change control system per Control 2.1.
...
| AI Administrator | Manage Copilot connector delegation and provide oversight for DLP connector classification decisions |
