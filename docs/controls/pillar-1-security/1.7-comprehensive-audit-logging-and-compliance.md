...
**Control ID:** 1.7<br>
**Pillar:** Security<br>
**Regulatory Reference:** FINRA Rule 4511, SEC Rules 17a-3/17a-4, SOX Sections 302/404, GLBA Section 501(b), OCC 2011-12<br>
...
- **FINRA Rule 4511:** Requires retention of books and records per regulatory timelines. The FINRA 2026 Annual Regulatory Oversight Report emphasizes that firms must retain not just agent outputs but also prompts, model state, and reasoning chains to enable reconstruction of how the agent reached its conclusion.
- **SEC Rule 17a-4(b)(4):** Communications records require 3-year retention (first 2 years readily accessible)
- **SEC Rule 17a-4(a):** Financial/accounting records require 6-year retention
- **SEC Rules 17a-3/17a-4:** Record retention for AI-assisted communications; broker-dealers may require WORM storage
- **SOX Sections 302/404:** Internal controls over AI system logging require audit trails
- **GLBA Section 501(b):** Security safeguards including audit trails for data access
...
- Retain complete prompt/response pairs per anticipated FINRA Regulatory Notice 25-07 requirements (no summaries)
...
!!! info "Preview Feature"
    The `agentSignIn` log type and `MicrosoftServicePrincipalSignInLogs` diagnostic stream are currently in preview. Configuration and field names may change before general availability. Frontier program enrollment is required for `agentSignIn` access.

Microsoft Entra has extended audit and sign-in logging to include dedicated agent activity telemetry. The following log types and attributes should be configured to achieve comprehensive agent audit coverage in FSI-regulated environments.

#### agentSignIn Log Type (Preview)

The `agentSignIn` resource type is a new category within Entra sign-in logs dedicated to authentication events performed by AI agent identities. This log type is the authoritative source for attributing authenticated actions to a specific agent identity during audit investigations.

**Filter Path in Entra Admin Center:**
...
| Event scope | Every authentication event performed by a registered AI agent identity |
...
#### New Sign-In Attributes for Agent Correlation

The following new attributes have been added to Entra sign-in log records to support agent-specific audit correlation. These fields are populated for both `agentSignIn` records and standard sign-in records where an agent identity is involved.

| Attribute | Purpose | FSI Use Case |
|---|---|---|
| `AppOwnerTenantId` | Identifies the tenant that owns the agent application | Cross-tenant agent attribution |
| `ResourceOwnerTenantId` | Identifies the tenant that owns the resource being accessed | Data residency verification |
| `SessionID` | Correlates all events occurring within a single agent execution session | Session-level audit reconstruction |
| `SourceAppClientID` | Identifies the originating application that initiated the agent action | Agent identity chain mapping |
| `ASN` | Network-level identifier for the autonomous system from which the request originated | Geolocation and routing audit review |
...
| Entra sign-in log filter `Is Agent = Yes` verified operational | Zone 2, Zone 3 | [ ] |
...
10. `agentSignIn` logging is enabled or validated as available for Zone 2 and Zone 3 agent identities
11. Agent-specific sign-in fields (`AppOwnerTenantId`, `ResourceOwnerTenantId`, `SessionID`, `SourceAppClientID`, and `ASN`) are preserved in the SIEM ingestion schema
12. `MicrosoftServicePrincipalSignInLogs` has been evaluated and documented for Zone 3 workloads
13. Agent-specific audit records are reviewed on a recurring cadence and retained according to approved policy
