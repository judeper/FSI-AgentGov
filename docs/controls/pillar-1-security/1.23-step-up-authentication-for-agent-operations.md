---
description: "Step-Up Authentication for AI Agent Operations: Implement step-up authentication requirements when AI agents attempt sensitive operations such as financial…"
---
...
|---|---|
| GLBA 501(b) Safeguards Rule | Helps verify customer identity at the point of sensitive data access, supporting the access-control safeguards required for... |
| FINRA 4511 / RN 24-09 | Supports authorized access to financial books and records at transaction time; FINRA RN 24-09 calls for supervisory oversight of... |
| FINRA 4530 | Helps create a contemporaneous, attributable authentication record at the moment of material events that may later require... |
...
| NIST SP 800-63B | Supports AAL2 / AAL3 authenticator assurance levels at the moment of high-risk action, rather than relying solely on initial... |
| NIST SP 800-53 IA-2 (Identification and Authentication) | Aids in meeting IA-2(1) MFA for privileged accounts and IA-2(2) MFA for non-privileged accounts when paired with... |
| NYDFS 23 NYCRR 500.12 | Supports the universal MFA mandate for all individuals accessing covered information systems, including AI-mediated access... |
...
!!! note "Step-up for agent identities depends on Conditional Access for agents (verify enforcement status)"
    Step-up authentication contexts apply cleanly to **human** principals (makers, sponsors, admins) acting through an agent. Applying step-up to an **agent identity** itself depends on **Conditional Access for agents**, which rides on Conditional Access for Workload Identities (Workload Identities Premium SKU) and Microsoft Agent 365 / Microsoft 365 E7 licensing. Some CA-for-agents surfaces remain in Public Preview and license-based enforcement is still maturing — **note this dependency rather than assuming agent-identity step-up is fully enforced**, and verify current preview/GA and enforcement status against Microsoft Learn before relying on it as a live Zone 3 control. Pre-Agent-ID Copilot Studio agents must be migrated to Agent ID before any agent-identity Conditional Access (including step-up) applies (see [Control 2.26](../pillar-2-management/2.26-entra-agent-id-identity-governance.md)).
