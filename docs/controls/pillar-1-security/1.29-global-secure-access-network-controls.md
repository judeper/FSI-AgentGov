---
description: "Global Secure Access: Network Controls for Copilot Studio Agents: Extend Zero Trust network security principles to Copilot Studio agent outbound traffic by…"
---
...
|---|---|
| Data exfiltration | An agent processing customer PII or NPI could be manipulated through prompt injection or misconfigured tool definitions to POST... |
| Unapproved AI destinations | FINRA, OCC, and SEC guidance increasingly scrutinizes use of third-party generative AI services. |
| Malicious infrastructure access | Agents connecting to known command-and-control (C2) infrastructure through misconfigured connectors or malicious MCP servers can... |
| Unauthorized file transfers | Agents with file handling capabilities could download malicious payloads or upload sensitive documents to unauthorized... |
...
Universal tenant restrictions use Global Secure Access signaling to apply tenant restrictions v2 policy information to Microsoft Entra ID and Microsoft Graph traffic. Microsoft documents the configuration path as **Microsoft Entra admin center > Global Secure Access > Settings > Session Management > Universal Tenant Restrictions**, after tenant restrictions v2 policies are defined in cross-tenant access settings. See [Turn on universal tenant restrictions](https://learn.microsoft.com/en-us/entra/global-secure-access/how-to-universal-tenant-restrictions) and [Set up tenant restrictions v2](https://learn.microsoft.com/en-us/entra/external-id/tenant-restrictions-v2).
...
!!! note "Relationship to Control 1.20 — Network Isolation"
    Control 1.29 is **complementary to, not overlapping with**, Control 1.20 (Network Isolation and Private Connectivity). Control 1.20 governs **inbound** network controls — restricting which clients and networks can reach your agents. Control 1.29 governs **outbound** network controls — filtering what external destinations your agents can access. Both controls are required for a complete Zero Trust network posture for Copilot Studio agent environments.
...
!!! warning "Preview feature — Zone 3 obligation is conditional on GA"
    This entire control governs a **Preview feature**. Preview features are not intended for production use and may have restricted functionality. The Zone 3 requirements below apply **when the feature reaches general availability (GA)**. Until GA, organizations must assess regulatory risk acceptance before enabling this control in regulated production environments, and must document that decision in their change management record. Monitor the [Power Platform release plans](https://learn.microsoft.com/en-us/power-platform/released-versions/) for GA status.

| Requirement | Zone 1 — Personal | Zone 2 — Team | Zone 3 — Enterprise / Regulated |
|---|---|---|---|
| **GSA Agent Traffic Forwarding** | Optional — recommended when agents access external web sources | Mandatory for all environments with external connector usage | **Mandatory when GA; currently Preview** — validate regulatory risk acceptance and document approval before enabling in regulated environments |
...
- [Microsoft Learn: Network content filtering in Global Secure Access](https://learn.microsoft.com/en-us/entra/global-secure-access/how-to-network-content-filtering)
