...
!!! tip "Agent 365 Architecture Update"
    ...
    **Agent Registry Visibility (Updated May 2026):** Microsoft Copilot Studio agents and Microsoft Copilot agents (e.g., declarative agents) are covered by the Microsoft Entra Agent ID registry per the [Agent Registry convergence](https://learn.microsoft.com/en-us/entra/agent-id/agent-registry-convergence) documentation. Declarative agents appear in the registry but lack org-wide deployment capability — export/import is required for broader distribution, and direct publish is under consideration. Admins can block or delete declarative agents but cannot deploy them org-wide from the registry.

    **Coverage note — Azure AI Foundry agents:** Microsoft documentation explicitly states that Azure AI Foundry agents are **not currently included** in the Microsoft Entra Agent ID registry. Foundry Agent Applications are documented separately at the [Azure AI Foundry agent applications](https://learn.microsoft.com/en-us/azure/foundry/agents/how-to/agent-applications) page. Until Microsoft confirms convergence, organizations using Foundry agents must apply Control 1.11 compensating controls (separate registration, separate audit, parallel identity governance) — they cannot rely on the Entra Agent ID registry to enumerate or attest Foundry agents.
...
!!! info "License Requirements"
    ...
    | **Microsoft Entra Agent ID** | Microsoft Agent 365 or Microsoft 365 E7 licensing (GA May 2026) | Rides on top of CA WID — Workload Identities Premium is a prerequisite |
...
!!! info "Sovereign Cloud Considerations"
    ...
    | Microsoft Entra Agent ID | GA (May 2026) — requires Microsoft Agent 365 or M365 E7 ("Frontier Suite") licensing | Verify Agent 365 / E7 SKU availability with FastTrack | Not yet announced | Not yet announced |
...
!!! info "Generally Available — May 2026"
    Microsoft Entra Agent ID reached general availability in May 2026 and is included with Microsoft Agent 365 or Microsoft 365 E7 licensing. Entra Agent ID rides on top of **CA for Workload Identities** (above) — the Workload Identities Premium SKU is a prerequisite. Some related surfaces (e.g., the `agentSignIn` resource type in Entra logs) remain in Public Preview; verify current preview/GA status against Microsoft Learn before relying on specific behaviors in production.
...
Agent-specific CA policies extend the standard Entra CA framework to workload identities representing AI agents. Capabilities expanded alongside Entra Agent ID GA (May 2026); some specific CA-for-agents surfaces may remain in Public Preview — verify current status against Microsoft Learn before relying on a specific surface in production policy. The capabilities below are documented on Microsoft Learn for Entra Agent ID and require **Microsoft Agent 365 or Microsoft 365 E7** licensing alongside the Workload Identities Premium SKU prerequisite for CA WID:
...
|---|---|---|
| Microsoft Agent 365 or Microsoft 365 E7 licensing provisioned (Entra Agent ID GA) | Zone 2, Zone 3 | [ ] |
