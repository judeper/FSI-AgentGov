---
description: "Application-Level Authorization and Role-Based Access Control (RBAC): Implement role-based access control for Power Platform and Microsoft Copilot Studio…"
---
...
|---|---|
| SOX 302/404 | Supports segregation of duties and internal control over financial reporting (ICFR) by limiting who can change configurations... |
...
| FINRA 4511 / 3110 | Aids in maintaining books-and-records integrity (4511) and supervisory accountability (3110) by restricting who can publish,... |
| FINRA RN 24-09 / Rule 3110 | Supports the access-control and supervisory-procedure expectations for AI systems, including documented role-based restrictions... |
...
| NIST SP 800-53 AC-2/AC-3/AC-5/AC-6 | Aligns with account management, access enforcement, separation of duties, and least-privilege control families for federal... |
...
!!! tip "Automation Available"
    ...
    - [Credential Oversharing Detector](https://github.com/judeper/FSI-AgentGov-Solutions/tree/main/credential-oversharing-detector) — detection of overprivileged connectors, excessive OAuth scopes, and unauthorized service accounts against zone policy
...
| [1.23 - Step-Up Authentication](1.23-step-up-authentication-for-agent-operations.md) | Conditional Access templates targeting Entra Agent ID identities are documented alongside the April 2026 Agent ID GA release (verify the current preview/GA status of specific Conditional Access for agents surfaces against Microsoft Learn) ([Entra Agent ID — What's new](https://learn.microsoft.com/en-us/entra/agent-id/whats-new-agent-id)) — enables CA policies scoped to agent identities for agent-specific RBAC enforcement |
...
> **Note:** The Microsoft Entra Agent ID **platform** is generally available (platform GA, April 2026 — [Entra Agent ID — What's new](https://learn.microsoft.com/en-us/entra/agent-id/whats-new-agent-id)); the distinct **Microsoft Agent 365** licensing bundle became generally available on **May 1, 2026**. Conditional Access templates for agent identities are documented alongside the April 2026 Agent ID GA release, though some **Conditional Access for agents** surfaces may still carry a Public Preview label. A few Agent ID surfaces — notably the identity **creation wizard** (blueprint and agent-identity creation in the Entra admin center) — also remain in **Public Preview**. Verify per-tenant rollout and current preview/GA status against Microsoft Learn before relying on specific Agent ID features.
