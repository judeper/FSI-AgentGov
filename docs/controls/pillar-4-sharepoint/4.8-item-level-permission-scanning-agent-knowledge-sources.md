---
description: "Item-Level Permission Scanning for Agent Knowledge Sources: Require item-level permission scanning of all SharePoint libraries connected as knowledge sources…"
---
# Control 4.8: Item-Level Permission Scanning for Agent Knowledge Sources

**Control ID:** 4.8<br>
**Pillar:** SharePoint<br>
**Regulatory Reference:** GLBA §314.4(c)(3), SEC Rule 17a-4, NIST AI RMF, Fed SR 26-2 (formerly SR 11-7)<br>
...
Require item-level permission scanning of all SharePoint libraries connected as knowledge sources to Microsoft Copilot Studio agents before deployment and on a recurring cadence. This control addresses the amplified data exposure risk that occurs when an AI agent actively retrieves and returns document content to users — making overshared items in knowledge source libraries an active threat rather than a passive risk.
...
|---|---|
| GLBA §314.4(c)(3) | Requires access controls limiting access to customer information to authorized users only — AI agents accessing customer data... |
| SEC Rule 17a-4 | Electronic records access must be documented and controlled — agent access to broker-dealer records through SharePoint knowledge... |
| Fed SR 26-2 (Model Risk Management, formerly SR 11-7) — applied to AI retrieval | Federal Reserve model-risk-management principles require institutions to govern the data scope used by models. |
| NIST AI RMF (GOVERN 1.2, MANAGE 2.2) | AI system access controls and risk monitoring require organizations to validate that AI agents access only authorized content at... |
| NIST SP 800-53 (AC-3, AC-6) | Access Enforcement and Least Privilege principles require that file-level permissions in agent knowledge sources align with the... |
| Fed SR 26-2 (formerly SR 11-7) (Model Risk Management) | Data governance requirements applicable to AI models using SharePoint as a data source — item-level permissions in knowledge... |
...
??? note "Full regulatory context"
    ...
    - **Fed SR 26-2 (Model Risk Management, formerly SR 11-7) — applied to AI retrieval:** Federal Reserve model-risk-management principles require institutions to govern the data scope used by models. Applied to AI knowledge retrieval systems, these principles support scoping agent knowledge sources to only data the agent is authorized to access. *(Derived application — FFIEC has not published guidance that directly names "knowledge source scoping" as a control concept for AI retrieval systems as of this version. The framework's regulatory-alignment hedging conventions apply.)*
    ...
    - **Fed SR 26-2 (formerly SR 11-7) (Model Risk Management):** Data governance requirements applicable to AI models using SharePoint as a data source — item-level permissions in knowledge sources must be validated to support model data integrity
...
| SharePoint unstructured-data knowledge source limits: **1,000 files, 50 folders, 10 levels of subfolders, 512 MB per file** | A library exceeding these limits may have content that is silently not indexed. Inventory the *full* library — unindexed items remain a sharing risk and may become indexed if limits change or content is moved. |
