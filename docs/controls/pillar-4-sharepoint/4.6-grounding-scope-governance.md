...
| **Sync Frequency** | Varies by source type: Salesforce and ServiceNow connectors sync every 4–6 hours; SharePoint knowledge sources require manual sync via the Copilot Studio portal | Content updates not immediately available for connector sources; SharePoint sources must be manually refreshed |
