---
description: "Multi-Agent Orchestration Limits: Establish governance limits and monitoring for multi-agent orchestration scenarios where agents call other agents, delegate…"
---
# Control 2.17: Multi-Agent Orchestration Limits

**Control ID:** 2.17<br>
**Pillar:** Management<br>
**Regulatory Reference:** FINRA RN 24-09, FINRA Rule 3110, FINRA 2026 Annual Regulatory Oversight Report, OCC Bulletin 2026-13 (formerly OCC 2011-12), Fed SR 26-2 (formerly SR 11-7), SOX 302/404<br>
**Last UI Verified:** May 2026<br>
**Governance Levels:** Baseline / Recommended / Regulated<br>

---

## Objective

Establish governance limits and monitoring for multi-agent orchestration scenarios where agents call other agents, delegate tasks, or operate in coordinated workflows — supporting oversight and helping reduce the risk of cascade failures, unbounded delegation, and combined model risk.

---

## Why This Matters for FSI

| Regulation | How this control supports it |
|---|---|
| FINRA RN 24-09 / Rule 3110 and FINRA 2026 Annual Regulatory Oversight Report | Identify AI and agentic systems as a 2025–2026 supervisory priority; firms are expected to extend supervisory procedures (FINRA... |
| OCC Bulletin 2026-13 (formerly OCC 2011-12) | Combined model risk assessment required for interacting systems |
| Fed SR 26-2 (formerly SR 11-7) | Multi-agent systems must be validated as composite models |
| SOX 302/404 | Financial controls must address automated decision chains |

??? note "Regulatory mapping — click to expand"
    - **FINRA RN 24-09 / Rule 3110 and FINRA 2026 Annual Regulatory Oversight Report:** Identify AI and agentic systems as a 2025–2026 supervisory priority; firms are expected to extend supervisory procedures (FINRA Rule 3110) and recordkeeping (FINRA Rule 4511) to multi-agent workflows
    - **OCC Bulletin 2026-13 (formerly OCC 2011-12):** Combined model risk assessment required for interacting systems
    - **Fed SR 26-2 (formerly SR 11-7):** Multi-agent systems must be validated as composite models
    - **SOX 302/404:** Financial controls must address automated decision chains

---

!!! tip "Automation Available"
    Companion solutions in [FSI-AgentGov-Solutions](https://github.com/judeper/FSI-AgentGov-Solutions):

    - [Agent Communication Restriction Detector](https://github.com/judeper/FSI-AgentGov-Solutions/tree/main/agent-communication-restriction-detector) — detection of unauthorized agent-to-agent communication, zone boundary violations, cross-tenant communication, and maker/checker violations in multi-agent orchestration
    - [HITL Workflow Governance](https://github.com/judeper/FSI-AgentGov-Solutions/tree/main/hitl-workflow-governance) — validation that agent flows include the human-in-the-loop checkpoints required by zone governance policy

## Control Description

This control establishes orchestration governance through design practices and custom implementation. Microsoft Copilot Studio does not provide built-in platform-enforced orchestration limits; organizations must implement these governance patterns.

!!! note "Expanded Multi-Agent Connectivity (April 2026)"
    Copilot Studio now supports expanded connectivity protocols for multi-agent and multi-tool orchestration:

    - **Agent-to-Agent (A2A) Protocol (GA, April 2026)**: Open standard for inter-agent communication; in Copilot Studio, A2A enables Copilot Studio agents and Microsoft 365 Copilot agents to call remote agents that implement the A2A standard. See [Connect agents using A2A](https://learn.microsoft.com/en-us/microsoft-copilot-studio/authoring-add-other-agents).
    - **Model Context Protocol (MCP)** — Open protocol for standardized tool discovery and invocation
    - **Microsoft Foundry** — Native integration for orchestrating agents built in Microsoft Foundry
    - **Microsoft Fabric** — Direct connectivity to Fabric data agents and analytics workloads
    - **128 tool limit** — Each Copilot Studio agent supports up to 128 tools (connectors, plugins) per agent; plan multi-agent architectures with this constraint in mind
    - **Tool groups (Public Preview, November 2025) and Component Collections:** Copilot Studio now supports Tool groups (Public Preview, November 2025) for organizing agent capabilities and Component Collections for reusable agent building blocks, enabling standardized governance patterns across agent deployments. As a Preview feature, do not depend on Tool Groups for production change-gate enforcement; status may change. See [Add tools to a custom agent](https://learn.microsoft.com/en-us/microsoft-copilot-studio/add-tools-custom-agent).
    - **Organization policy blocking for tools:** Copilot Studio can show tools disabled in the **Add tool** panel when an organization's governance or data policies block them. This is a platform capability note for interpreting allowed-tool and blocked-tool evidence; it does not replace the custom orchestration limits and reviews required by this control. See [Add tools to a custom agent](https://learn.microsoft.com/en-us/microsoft-copilot-studio/add-tools-custom-agent#tools-blocked-by-your-organizations-policies).

    **Feature GA Status Tracking:**

    | Feature | Status | GA Date | Governance Impact |
    |---------|--------|---------|-------------------|
    | Multi-Agent Orchestration | **GA** | November 2025 | Full platform support for agent-to-agent delegation |
    | MCP Server Support | **GA** | October 2025 | Standardized tool discovery and invocation |
    | Code Interpreter | **GA** | August 2025 | Agent code execution requires sandbox governance |

    These expanded connectivity options increase the attack surface and complexity of multi-agent systems. Update orchestration governance policies to account for cross-protocol delegation chains.

!!! warning "Design Patterns, Not Platform Enforcement"
    The capabilities below are **governance design patterns** that organizations implement through custom logic, not built-in platform constraints. Copilot Studio does not enforce delegation depth limits, circuit breakers, or financial stop-loss controls natively.

1. **Delegation Depth Limits** - Maximum levels of agent-to-agent calls (2-3 recommended) - *custom orchestration logic*
2. **Circuit Breaker Patterns** - Automatic failsafe when cascading errors occur - *custom error handling*
3. **Human-in-the-Loop Triggers** - Conditions requiring human review in chains - *custom workflow logic*
4. **Financial Stop-Loss Controls** - Cumulative transaction limits across agent chains - *custom business logic*
5. **Orchestration Monitoring** - Track multi-agent interactions for audit - *Application Insights custom telemetry*
6. **Combined Risk Assessment** - Evaluate multi-agent systems as single models - *organizational process*

---

## Key Configuration Points

### Governance Policy Decisions (Organization-Defined)

- Define maximum delegation depth per zone (Zone 1: 1, Zone 2: 2, Zone 3: 3)
- Define circuit breaker thresholds (e.g., 3 consecutive failures)
- Define cumulative financial limits across agent chains
- Define HITL trigger conditions for critical actions in delegation chains
- Schedule quarterly review of orchestration patterns

### Custom Implementation Required

- Implement delegation depth tracking in orchestration logic (custom code)
- Implement circuit breaker pattern in agent error handling (custom Power Automate or code)
- Implement financial stop-loss validation in transaction processing (custom business logic)
- Enable comprehensive logging via Application Insights custom events (custom instrumentation)
- Create combined Agent Card for multi-agent systems (documentation artifact)

!!! info "Implementation Approach"
    These controls are enforced through custom orchestration logic (Power Automate, custom connectors, or code plugins) rather than platform configuration settings. The platform provides the capability to build agents that call other agents, but does not enforce limits on that capability.

---

## Zone-Specific Requirements


!!! info "Zone cross-reference"
    **Baseline** = Zone 1 (Personal Productivity) · **Recommended** = Zone 2 (Team Collaboration) · **Regulated** = Zone 3 (Enterprise Managed). Zone requirements below correspond to the Governance Level in the control header.

| Zone | Requirement | Implementation | Rationale |
|------|-------------|----------------|-----------|
| **Zone 1** (Personal) | Single-agent only; no delegation allowed | Design restriction (not platform-enforced) | Simplicity reduces complexity risks |
| **Zone 2** (Team) | Max depth 2; circuit breaker required; logged interactions | Custom orchestration logic | Controlled orchestration for team use |
| **Zone 3** (Enterprise) | Max depth 3; full monitoring; HITL triggers; financial controls; combined validation | Custom orchestration logic with comprehensive telemetry | Customer-facing requires maximum orchestration controls |

!!! note "Enforcement Mechanism"
    These requirements are enforced through organizational design standards and custom implementation, not through platform configuration. Organizations should document orchestration patterns and validate compliance through code review and testing.

---

## Roles & Responsibilities

| Role | Responsibility |
|------|----------------|
| AI Governance Lead | Define orchestration limits, approve multi-agent patterns |
| Power Platform Admin | Configure technical controls, monitor orchestration |
| Entra Security Admin | Review circuit breaker events, validate controls |
| Compliance Officer | Approve financial limits, validate regulatory alignment |

---

## Related Controls

| Control | Relationship |
|---------|--------------|
| [2.6 - Model Risk Management](2.6-model-risk-management-sr-26-2.md) | Multi-agent systems require combined MRM |
| [2.12 - Supervision](2.12-supervision-and-oversight-finra-rule-3110.md) | HITL requirements in delegation chains |
| [1.7 - Audit Logging](../pillar-1-security/1.7-comprehensive-audit-logging-and-compliance.md) | Orchestration events captured in audit logs |
| [1.24 - AI-SPM](../pillar-1-security/1.24-defender-ai-security-posture-management.md) | Coordinator agent visibility and attack path analysis |
| [3.4 - Incident Reporting](../pillar-3-reporting/3.4-incident-reporting-and-root-cause-analysis.md) | Circuit breaker events trigger incident process |

---

## Implementation Playbooks

!!! info "Step-by-Step Implementation"

    This control has detailed playbooks for implementation, automation, testing, and troubleshooting:

    - [Portal Walkthrough](../../playbooks/control-implementations/2.17/portal-walkthrough.md) — Step-by-step portal configuration
    - [PowerShell Setup](../../playbooks/control-implementations/2.17/powershell-setup.md) — Automation scripts
    - [Verification & Testing](../../playbooks/control-implementations/2.17/verification-testing.md) — Test cases and evidence collection
    - [Troubleshooting](../../playbooks/control-implementations/2.17/troubleshooting.md) — Common issues and resolutions

---

## March 2026 Documentation Update

### Human-in-the-loop approval boundaries

Microsoft Agent Framework now documents HITL workflows with **request and response** handling. In orchestration terms, this gives teams a standard way to pause a workflow, capture a human decision, and resume execution without losing the original request context.

| Workflow capability | Multi-agent governance use |
|---------------------|----------------------------|
| RequestPort / `request_info()` | Insert human approval before high-risk agent-to-agent delegation or tool execution |
| Response handler routing | Bind the human decision to the correct agent step and approval request |
| Checkpoint persistence | Resume long-running orchestrations without dropping outstanding approvals |
| Event streaming | Export approval events to observability and supervision tooling for chain-of-custody evidence |

For FSI use cases, treat human approval checkpoints as explicit orchestration boundaries for customer-impacting actions, regulated data access, or material financial decisions.

---

## Verification Criteria

Confirm control effectiveness by verifying:

1. Delegation depth limits enforced by custom orchestration logic (test exceeding limit is blocked)
2. Circuit breaker pattern triggers on consecutive errors (custom implementation)
3. Financial stop-loss controls prevent cumulative limit breach (custom business logic)
4. HITL triggers function at configured conditions (custom workflow)
5. Orchestration logs capture complete agent-to-agent interaction chain (Application Insights custom events)
6. HITL approval checkpoints preserve request IDs and reviewer decisions for escalated orchestration steps
7. If custom MCP servers are used, server-level and tool-level DLP/access controls are validated before production approval

!!! warning "Testing Note"
    These verification tests validate custom implementations, not built-in platform features. If your organization has not implemented custom orchestration controls, these tests will not be applicable.

---

## MCP Server Governance

Model Context Protocol (MCP) servers enable cross-platform agent interoperability, allowing agents to access tools and resources from non-Microsoft platforms. This creates additional governance requirements for organizations using multi-vendor agent ecosystems.

!!! info "Custom MCP servers in Copilot Studio"
    Microsoft documents **custom MCP servers** in the 2026 Wave 1 release plan, with public preview in **March 2026** and general availability targeted for **April 2026**. The release-plan text states that custom MCP servers provide enterprise-grade governance, including DLP policy enforcement and access controls at both the MCP server and individual tool level.

### MCP Governance Requirements

| Requirement | Zone 2 | Zone 3 | Implementation |
|-------------|--------|--------|----------------|
| **MCP Server Approval** | IT approval required | AI Governance Committee approval | Maintain approved MCP server registry |
| **Tool Access Review** | Quarterly | Monthly | Document which tools each agent can access via MCP |
| **Data Flow Mapping** | Required | Required + DLP validation | Map data paths through MCP connections |
| **Authentication Standards** | OAuth 2.0 minimum | OAuth 2.0 + certificate pinning | Validate MCP server authentication |
| **Audit Logging** | Tool invocations logged | Full request/response capture | Configure MCP-aware logging |

### MCP Server Approval Workflow

Organizations should establish an approval process for MCP servers:

1. **Request Submission** - Agent developer requests MCP server connection with business justification
2. **Security Assessment** - Security team evaluates:
   - MCP server vendor reputation and security posture
   - Data classification of accessible resources
   - Authentication and encryption standards
   - Audit logging capabilities
3. **Compliance Review** - Compliance validates:
   - Regulatory implications of cross-platform data flow
   - Vendor contract terms for AI usage
   - Data residency requirements
4. **Approval Decision** - AI Governance Committee (Zone 3) or IT (Zone 2) approves/denies
5. **Registration** - Approved MCP servers added to organizational registry

### MCP Server Risk Categories

| Category | Examples | FSI Considerations |
|----------|----------|-------------------|
| **Internal MCP Servers** | Self-hosted database connectors, internal APIs | Lower risk; standard internal controls apply |
| **Vendor MCP Servers** | Salesforce, ServiceNow, Bloomberg connectors | Evaluate vendor security; data residency; contract terms |
| **Open Source MCP Servers** | Community-maintained tool providers | Higher risk; requires security audit before use |
| **AI Provider MCP Servers** | Claude MCP, OpenAI tool servers | Model risk management applies; evaluate prompt injection risks |

### Cross-Platform Agent Governance

When agents interact with non-Microsoft AI platforms via MCP:

| Concern | Mitigation |
|---------|------------|
| **Prompt injection via MCP** | Validate MCP responses before agent processing |
| **Data leakage to external AI** | Validate DLP enforcement and tool-level access controls across MCP data flows, especially when the same server is reused outside Microsoft platforms |
| **Audit trail gaps** | Ensure MCP interactions are captured in unified audit log |
| **Model risk accumulation** | Combined risk assessment for multi-platform agent chains |
| **Regulatory jurisdiction** | Validate data processing locations for MCP servers |

### MCP Monitoring and Telemetry

Extend observability to capture MCP interactions:

```kql
// Query MCP server interactions
customEvents
| where timestamp > ago(24h)
| where name == "MCP.ToolInvocation"
| extend
    mcpServer = tostring(customDimensions.mcpServer),
    toolName = tostring(customDimensions.toolName),
    agentId = tostring(customDimensions.agentId),
    dataClassification = tostring(customDimensions.dataClassification)
| summarize
    InvocationCount = count(),
    UniqueTools = dcount(toolName)
    by mcpServer, agentId
| order by InvocationCount desc
```

### Related Industry Guidance

- [FINOS AI Governance Framework](https://github.com/finos/ai-governance-framework) - Addresses authorization bypass and privilege escalation risks in agentic AI
- [NIST AI RMF 1.0](https://www.nist.gov/itl/ai-risk-management-framework) - Calls for incorporating trustworthiness considerations into the design, development, use, and evaluation of AI systems, supporting explicit oversight controls for MCP governance

---

## Additional Resources

- [Microsoft Learn: Copilot Studio Agent Orchestration](https://learn.microsoft.com/en-us/microsoft-copilot-studio/add-tools-custom-agent)
- [OWASP Agentic AI Threats and Mitigations](https://genai.owasp.org/resource/agentic-ai-threats-and-mitigations/)
- [OCC 2026-13: Model Risk Management (formerly OCC 2011-12)](https://www.occ.gov/news-issuances/bulletins/2026/bulletin-2026-13.html)
- [MITRE ATLAS: AI Threat Framework](https://atlas.mitre.org/)
- [Microsoft Learn: MCP Server Support](https://learn.microsoft.com/en-us/microsoft-copilot-studio/agent-extend-action-mcp)
- [MCP Server Governance for FSI — Three flavors of MCP](../../playbooks/advanced-implementations/mcp-server-governance/index.md#three-flavors-of-mcp) — See also for governance differences among Copilot Studio custom MCP, Microsoft-published MCP servers, and Agent 365 BYO MCP when orchestration designs rely on MCP tools.
- [Work IQ Governance Reference](../../reference/work-iq-governance.md) — Governance considerations for Work IQ MCP tools, business skills, admin consent, and audit expectations
- [Microsoft Learn: Copilot Studio Planned Features (2026 Wave 1)](https://learn.microsoft.com/en-us/power-platform/release-plan/2026wave1/microsoft-copilot-studio/planned-features)
- [Microsoft Learn: Human-in-the-Loop workflows in Microsoft Agent Framework](https://learn.microsoft.com/en-us/agent-framework/workflows/human-in-the-loop)
- [Model Context Protocol Specification](https://modelcontextprotocol.io/)
- [Architecting Agent Solutions — Microsoft Guidance](https://learn.microsoft.com/en-us/agents/architecture/)

---

*Updated: September 2026 | Version: v1.6.2 | UI Verification Status: Current (content update only; UI last verified May 2026)*
