...
!!! warning "Sovereign Cloud Availability — GCC, GCC High, DoD, China"
    ...
    - **Microsoft Foundry evaluation harness** — verify Foundry availability and evaluator parity in Azure Government / DoD; some built-in safety evaluators rely on commercial-cloud-hosted judge models.
...
!!! info "Copilot Studio Analytics Retention Windows (May 2026)"
    Analytics data is available for up to **180 days**; session details and transcript information is available for the last **28 days** (per Microsoft Learn). Ongoing-monitoring evidence — session-level data, conversation transcripts — required beyond 28 days must be exported to Log Analytics or a WORM-capable store before expiry. Validation memos and MRM Committee minutes do not live in Copilot Studio Analytics; route those to the 17a-4(f) retention path described in the [Control 2.6 portal walkthrough §5](../../playbooks/control-implementations/2.6/portal-walkthrough.md#5-validation-evidence-retention-path).
