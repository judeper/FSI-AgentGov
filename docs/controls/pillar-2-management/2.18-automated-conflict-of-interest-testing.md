---
description: "Automated Conflict of Interest Testing: Implement automated testing to detect potential conflicts of interest in AI agent recommendations, particularly for…"
---
...
|---|---|
| SEC Regulation Best Interest (Reg BI) / FINRA Rule 2111 (Suitability) | Broker-dealers must act in the best interest of retail customers when making recommendations. |
| SEC Rule 10b-5 (Anti-Fraud) | Prohibits material misstatements, omissions, and self-serving recommendations; helps ensure AI agents do not steer clients toward... |
| FINRA Rule 3110 (Supervision) | Requires written supervisory procedures (WSPs) and reasonable supervision of recommendations. |
| FINRA RN 24-09 / Rule 3110 (AI Supervision) | Reinforces that firms must implement testing, monitoring, and documentation of AI tools used in recommendations to retail... |
...
!!! tip "Copilot Studio Evaluation Framework"
    ...
    - **Enterprise audit trail** — Microsoft Purview's [Copilot Studio audit log](https://learn.microsoft.com/en-us/microsoft-copilot-studio/admin-logging-copilot-studio) captures agent lifecycle events (`BotCreate`, `BotUpdate`, `BotPublish`, `BotDelete`) and user-agent conversations (`CopilotInteraction`), providing evidence of configuration changes and runtime interactions. Test-execution metadata (which test set ran, which evaluator graded each turn, and the resulting pass/fail decision) is **not currently emitted as Purview audit events**; that evidence must be retained from the [agent evaluations REST API](https://learn.microsoft.com/en-us/microsoft-copilot-studio/analytics-agent-evaluation-rest-api) or Power Automate evaluation connector pipeline outputs. For SEC 17a-4 evidence chains, organizations should architect a pipeline that captures evaluation-API outputs into WORM-compliant storage in addition to the Purview audit feed. (See control 3.4 for the M365 Backup / Storage-account retention pattern.)
