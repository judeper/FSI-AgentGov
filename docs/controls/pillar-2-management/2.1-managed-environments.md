...
- **Sovereign cloud caveat:** Lockbox availability and operational behavior may differ in 21Vianet (China) and certain GCC High / DoD sub-services — re-verify per the [Sovereign Cloud Availability](#agent-365-blueprint-lifecycle-preview) guidance above.

### Data Residency Configuration (per Sub-Control above)

- **Microsoft 365 Multi-Geo:** Enable the Multi-Geo add-on (separate license SKU) and assign each user to the appropriate `PreferredDataLocation` (PDL) at the time of license assignment. Verify Multi-Geo placement via Microsoft 365 Admin Center → **Setup** → **Multi-Geo Capabilities** and per-user via `Get-MsolUser -UserPrincipalName ... | Select PreferredDataLocation`. Document PDL assignment rules in the firm's data-residency policy and re-verify annually.
