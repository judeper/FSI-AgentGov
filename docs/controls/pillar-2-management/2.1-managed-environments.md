...
**Regulatory Reference:** FINRA Rule 4511, SEC Rule 17a-4, GLBA Section 501(b), SOX Section 302<br>
...
- **FINRA Rule 4511:** Usage insights provide activity records supporting books and records requirements
- **SEC Rule 17a-4:** Solution checker enforces change control for recordkeeping systems
- **GLBA Section 501(b):** Sharing limits help protect customer information from unauthorized access
- **SOX Section 302:** Maker welcome content documents policy acknowledgment for internal controls
...
- **Restrict environment creation to authorized admins:** In Power Platform Admin Center > Tenant Settings, configure the following to "Only specific admins" for each environment type:
    ...
    This helps prevent uncontrolled environment sprawl where trial or developer environments may expose sensitive data or bypass compliance controls

- **Configure environment routing:** In PPAC > Tenant Settings > Environment Routing, configure routing rules to help direct new environments in the correct region aligned with data residency requirements and organizational governance policies. This supports compliance with data locality regulations and optimizes resource management

- **Enable tenant isolation:** In PPAC > Security > Identity and access > Tenant Isolation, enable "Restrict Cross-Tenant Connections" to help prevent data from moving into or out of the tenant via Power Platform connectors. Configure explicit exceptions (by Tenant ID and direction) only for trusted partner tenants. Unrestricted cross-tenant connectivity increases risk of unintended data exchange and regulatory non-compliance
