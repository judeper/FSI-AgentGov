...
!!! info "Metered Copilot consumption billing — licensing context"
    This control governs who may incur **metered Copilot spend** (Copilot Credits and pay-as-you-go consumption). Two policy objects back metered spend: a **PAYG billing policy** (Azure subscription, tenant ceiling of **50**, budget alerts only — not a hard-stop) and a **prepaid credit policy** (no Azure subscription, ceiling of **10**, standalone hard-stop, Chat-only). Per the **June 2026 Copilot Studio Licensing Guide (footnotes 6 & 7)**: Copilot Studio agents surfaced on **Teams, SharePoint, and Microsoft 365 Copilot** are included with the Microsoft 365 Copilot user license at no additional charge when invoked under the licensed user's own identity, subject to fair-usage limits. Verify current ceilings and credit rates against Microsoft licensing documentation before wiring enforcement — these surfaces are changing rapidly.

### Scope Boundaries — How This Control Differs from Related Controls

**Control 2.27 vs. Control 2.26 (Entra Agent ID):** 2.26 governs **agent identity lifecycle** (sponsor, access packages, decommission). 2.27 governs **who is entitled to incur metered consumption** on the agent. An agent can have a fully governed identity while no one has decided who may spend Credits through it. These are separate objectives.

**Control 2.27 vs. Control 1.18 (RBAC):** 1.18 governs **functional authorization** (author/publish/invoke). 2.27 adds a **cost dimension**: a functionally-authorized user may still need to be scoped to a billing policy before incurring metered spend. 1.18 is a necessary input but does not express billing-policy scope.

**Control 2.27 vs. Control 3.5 (Cost Allocation):** 3.5 provides cost **visibility** (chargeback, budget alerting). 2.27 provides the runtime **entitlement decision** that governs whether spend is permitted. Pair them — reporting without entitlement enforcement observes uncontrolled spend; enforcement without reporting cannot evidence results to examiners.
...
- [Microsoft Copilot Studio — billing rates and management](https://learn.microsoft.com/en-us/microsoft-copilot-studio/requirements-messages-management) — *per-feature credit rates confirmed here; pricing is time-sensitive — re-confirm against current Microsoft licensing documentation as it changes*
- [Microsoft Copilot Studio capacity packs for Microsoft Copilot Chat and SharePoint agents](https://learn.microsoft.com/microsoft-365/copilot/pay-as-you-go/copilot-capacity-packs) — *prepaid capacity pack quantity (25,000 Copilot Credits per month per pack) and credit-policy tenant limit (10)*
