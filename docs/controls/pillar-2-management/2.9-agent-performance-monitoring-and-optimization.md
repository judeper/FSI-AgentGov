...
!!! note "Read-only Analytics Access via the Analytics Viewer Sharing Role"
    Operations, AI Governance, and Model Risk reviewers often need to consult an agent's Analytics page (sessions, response time, CSAT, escalation rate) without holding edit rights on the agent. The Copilot Studio **Analytics Viewer** sharing role helps meet this separation-of-duties requirement by granting read-only access to the agent's Analytics page; pair it with the **Bot Transcript Viewer** role to also expose conversation transcripts for performance root-cause analysis. The role is shared by the agent owner via the agent's three-dots menu → **Share** → **Analytics viewer** and **must be assigned to individual users — security groups are not supported**, so maintain a named-individual attestation log to support OCC 2011-12 / Fed SR 11-7 ongoing-monitoring evidence. See [Share an agent](https://learn.microsoft.com/en-us/microsoft-copilot-studio/admin-share-bots).
