...
*Updated: May 2026 | Version: v1.4.0 | UI Verification Status: Post-GA (verified against Microsoft Learn `entra/agent-id`, `entra/agent-id/whats-new-agent-id`, and `entra/id-governance/agent-id-governance-overview`, May 2026). The Lifecycle Workflows agent-sponsor tasks and the "All agents (preview)" assignment-policy toggle remain labelled "(Preview)" on Microsoft Learn — verify exact UI strings before wiring supervisory cadences to specific names.*
