---
description: "Entra Agent ID — Identity Governance for Agents: Establish identity lifecycle governance for all Microsoft 365 Copilot and Microsoft Copilot Studio agent…"
---
# Control 2.26: Entra Agent ID — Identity Governance for Agents

**Control ID:** 2.26
**Pillar:** Management
**Regulatory Reference:** SOX 404, GLBA 501(b), OCC Bulletin 2026-13 (formerly OCC 2011-12), FFIEC Access Management, FINRA 3110, FINRA RN 24-09
...
!!! info "Generally Available — Microsoft Entra Agent ID"
    **Microsoft Entra Agent ID reached general availability in April 2026** ([Entra Agent ID — What's new](https://learn.microsoft.com/en-us/entra/agent-id/whats-new-agent-id); [Entra fundamentals — What's new](https://learn.microsoft.com/en-us/entra/fundamentals/whats-new)). The **Microsoft Agent 365** licensing bundle that packages Entra Agent ID with the Agent 365 control plane in **Microsoft 365 E7** ("Frontier Suite," which bundles E5 + Microsoft 365 Copilot + Entra Suite + Agent 365) became generally available on **May 1, 2026** ([Agent 365 admin overview](https://learn.microsoft.com/en-us/microsoft-365/admin/manage/agent-365-overview)). Access to Entra Agent ID and the agent governance capabilities described in this control requires **Microsoft Agent 365** licensing or **Microsoft 365 E7**. Some adjacent surfaces remain in Public Preview — notably the Lifecycle Workflows **agent-sponsor tasks** and elements of the agent identity governance experience labelled "(Preview)" in Microsoft Learn. Treat the core sponsor / access-package / lifecycle pattern as production-ready, but verify the specific Lifecycle Workflows task names and "All agents (preview)" assignment toggle naming against current Microsoft Learn before wiring supervisory cadences to the exact UI strings.

!!! warning "Conditional Access for agents and the pre-Agent-ID migration gap"
    **Conditional Access for agents** is the enforcement surface that applies adaptive-access policy to agent identities, but it is meaningful only once the agent identity exists and is in scope. Two caveats:

    - **Migration gap:** Copilot Studio agents created **before** Entra Agent ID was enabled in the tenant are **not** automatically protected by Conditional Access for agents or Identity Protection for agents until they are **migrated** to Agent ID ([Migrate Copilot Studio agents to Agent ID](https://learn.microsoft.com/en-us/entra/agent-id/migrate-copilot-studio-agents-to-agent-id)). Do not assume uniform agent-identity coverage across the existing agent estate; inventory pre-Agent-ID agents and track migration status during Zone 2/3 onboarding.
    - **Enforcement dependency:** Conditional Access for agents rides on **Conditional Access for Workload Identities** (Workload Identities Premium SKU) and requires Microsoft Agent 365 / Microsoft 365 E7 licensing. Some CA-for-agents surfaces remain in Public Preview, and license-based enforcement is still maturing — author policies in report-only mode and **verify current preview/GA and enforcement status against Microsoft Learn before relying on CA for agents as a live preventive control.** See [Control 1.11](../pillar-1-security/1.11-conditional-access-and-phishing-resistant-mfa.md) and [Control 1.23](../pillar-1-security/1.23-step-up-authentication-for-agent-operations.md).
