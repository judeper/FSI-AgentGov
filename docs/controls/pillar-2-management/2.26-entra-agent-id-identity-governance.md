...
!!! info "Generally Available — Microsoft Entra Agent ID"
    **Microsoft Entra Agent ID reached general availability in April 2026** ([Entra Agent ID — What's new](https://learn.microsoft.com/en-us/entra/agent-id/whats-new-agent-id)). The **Microsoft Agent 365** licensing bundle became generally available on **May 1, 2026**. Access to Entra Agent ID capabilities described in this control requires **Microsoft Agent 365** or **Microsoft 365 E7** licensing. Some adjacent surfaces remain in Public Preview — notably the Lifecycle Workflows **agent-sponsor tasks** (verify specific task names against current Microsoft Learn before wiring supervisory cadences).

!!! warning "Conditional Access for agents and the pre-Agent-ID migration gap"
    **Conditional Access for agents** is meaningful only once the agent identity exists and is in scope. Two caveats:

    - **Migration gap:** Copilot Studio agents created **before** Entra Agent ID was enabled are **not** automatically protected until **migrated** to Agent ID ([migrate guide](https://learn.microsoft.com/en-us/entra/agent-id/migrate-copilot-studio-agents-to-agent-id)). Inventory pre-Agent-ID agents and track migration status.
    - **Enforcement dependency:** Conditional Access for agents requires Workload Identities Premium SKU and Microsoft Agent 365 / E7 licensing. Some surfaces remain in Public Preview — author policies in report-only mode and verify enforcement status before relying on CA for agents as a live preventive control. See [Control 1.11](../pillar-1-security/1.11-conditional-access-and-phishing-resistant-mfa.md).

### Scope Boundaries — How This Control Differs from Related Controls

**Control 2.26 vs. Control 1.11 (Conditional Access and MFA):** Control 1.11 governs **access policies at authentication time** (a point-in-time gate). Control 2.26 governs the **ongoing identity lifecycle** — accountable sponsor, time-bound access, lifecycle workflows, and controlled decommission. Both must be present.

**Control 2.26 vs. Controls 2.3 / 2.5 (Change Management and Testing/QA):** Controls 2.3/2.5 govern the SDLC of agent development. Control 2.26 governs the **identity lifecycle** of the agent identity object in Entra ID. The SDLC can complete successfully while identity lifecycle governance is entirely absent; both must be present for a compliant deployment.
