...
> Microsoft notes that to search for Copilot/AI audit scenarios, use the **Activities – operation names** filter and properties like `Operation`, `RecordType`, and `Workload`.
...
- **Operation:** `CopilotInteraction`
- **RecordType:** `CopilotInteraction`
- **AppHost:** Teams (or other)
- **Post-filter (offline):** `AppIdentity` starts with `Copilot.Studio.`

**Use cases**
- Evidence for custom agents built in Copilot Studio, including correlation to `AgentId`/`AgentName` where present.

### 2.3 Admin activities: Copilot settings/plugins/workspaces
Microsoft notes audit logs are generated for administrator activities related to Copilot settings, plugins, promptbooks, or workspaces.
...
- Filter: Operation = CopilotInteraction
- Offline filter: `AgentId == <agent_id>` and `AgentVersion == <version>`
...
- Filter: Operation = CopilotInteraction
- Offline filter: any `AccessedResources[*].PolicyDetails` exists OR `SensitivityLabelId` in restricted label set
...
- Time window: last 30 days
- Filter: Operation = CopilotInteraction
- Offline filter: any `AccessedResources[*].XPIADetected == true`
...
- Agent Inventory Entry (`../agent-lifecycle/agent-inventory-entry.md`)
- Action Authorization Matrix (`../governance-operations/action-authorization-matrix.md`)
- Decision Logs (`../governance-operations/decision-log-schema.md`)
- Per-Agent Data Policy (`../agent-lifecycle/per-agent-data-policy.md`)

This makes it possible to assemble exam-ready evidence without bespoke work each time.
...
Microsoft notes you can identify if Copilot referenced the public web by checking `AISystemPlugin.Id` for the value `BingWebSearch` in the CopilotInteraction audit record when web search is enabled.

- Evidence query: filter CopilotInteraction and offline filter on `AISystemPlugin.Id == BingWebSearch`.
