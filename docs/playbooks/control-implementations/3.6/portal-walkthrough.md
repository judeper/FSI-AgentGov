...
| 1 | Signal #1 — Agent 365 Ownerless Agents card | Microsoft 365 admin center → Agents → Overview | VC-3 |
| 2 | Signal #2 — Entra Lifecycle Workflows leaver cascade | Entra admin center → Identity Governance → Lifecycle Workflows | VC-3, VC-4 |
| 3 | Signal #3 — PPAC maker-departed | Power Platform admin center → Resources → Power Apps | VC-3 |
| 4 | Signal #4 — PPAC environment-owner-departed | PPAC → Environments | VC-3 |
| 5 | Signal #5 — SharePoint Copilot agent author-disabled | SharePoint admin center → Content services → Copilot agents | VC-3 |
| 6 | Signal #6 — PPAC Recycle bin and deleted-environment audit | PPAC → Environments → Recycle bin | VC-5 |
| 7 | Signal #10 — Shadow-agent discovery via Defender for Cloud Apps | Defender XDR → Cloud Apps → Cloud App Catalog | VC-6 |
| 8 | Remediation walkthrough (4-step ordering) | Agent 365 → 2.26 → PPAC / Copilot Studio → bulk Graph | VC-7 |
| 9 | Evidence capture, zone cadence, and examiner packet | All portals + Purview | VC-8 |
...
Only proceed to §1 after every box is ticked.
...
## 1. Signal #1 — Agent 365 Ownerless Agents Card

The **Ownerless Agents** governance card in the Agent 365 admin center is the **primary portal-driven detection surface** for orphaned agents in commercial tenants. It aggregates signals from Entra directory state, the Control 1.2 agent registry, Power Platform maker telemetry, and SharePoint author state.
...
| "Last refreshed" timestamp | Card data freshness | Expect ≤ 24 h |
...
### 1.3 Drill-in and inline Assign Owner
...
[Screenshot anchor: `docs/images/3.6/EXPECTED.md#1-3-inline-assign-owner`]

### 1.4 Zone cadence and SLA
...
## 2. Signal #2 — Entra Lifecycle Workflows Leaver Cascade
...
[Screenshot anchor: `docs/images/3.6/EXPECTED.md#2-1-lifecycle-workflows`]

### 2.2 Reviewing execution history
...
### 2.3 Manager-transfer confirmation
...
Overrides are executed via the Agent 365 Ownerless card (§1.3) — the agent will surface there until a valid sponsor is assigned.
...
## 3. Signal #3 — PPAC Maker-Departed
...
[Screenshot anchor: `docs/images/3.6/EXPECTED.md#3-1-ppac-resources-power-apps`]

### 3.2 Filtering by maker
...
### 3.3 Cross-referencing against Entra disabled users
...
4. For each finding, note:
    - The **environment** the agent lives in (important for §4 and §6).
    - Whether the agent has a **co-owner** (if yes, co-owner auto-promotion may apply — verify in §8.3).
    - The agent's **Zone** per the 1.2 registry (drives cadence and SLA).

### 3.4 Remediation from this surface

!!! warning "PPAC reassignment is maker-scoped, not sponsor-scoped"
    Reassigning the **maker / owner** in PPAC transfers edit rights on the underlying Power Platform resource. It does **not** update the agent **sponsor** in the Control 1.2 registry. Both updates are required; see §8 for the ordering.
...
!!! example "Examiner Evidence Box — PPAC maker-departed"
    ...
    4. Confirmation that the Control 1.2 registry sponsor field was updated via §8.
...
## 4. Signal #4 — PPAC Environment-Owner-Departed
...
[Screenshot anchor: `docs/images/3.6/EXPECTED.md#4-1-ppac-environments`]

### 4.2 Filtering and export
...
### 4.4 Special handling for Default and Developer environments

| Environment type | Special handling |
|------------------|------------------|
| Default | Cannot be deleted; reassign to tenant Power Platform Admin pool. |
| Developer (per-user) | If the developer is a leaver, schedule the environment for §6 recycle-bin review after grace window; do not auto-delete during this session. |
...
!!! example "Examiner Evidence Box — Environment-owner-departed"
    ...
    4. List of Developer environments deferred to §6.
...
## 5. Signal #5 — SharePoint Copilot Agent Author-Disabled
...
[Screenshot anchor: `docs/images/3.6/EXPECTED.md#5-1-sharepoint-copilot-agents`]

### 5.2 Reviewing authorship
...
## 6. Signal #6 — PPAC Recycle Bin and Deleted-Environment Audit
...
[Screenshot anchor: `docs/images/3.6/EXPECTED.md#6-1-recycle-bin`]
...
### 6.3 Restore vs. retire decision
...
!!! warning "Retention window is not guaranteed"
    Recycle bin retention varies by environment type and tenant configuration; do not rely on the recycle bin as a primary recovery mechanism.     The §1 reconciliation workflow and the §9 evidence library provide the durable audit trail.

## 7. Signal #10 — Shadow-Agent Discovery via Defender for Cloud Apps
...
[Screenshot anchor: `docs/images/3.6/EXPECTED.md#7-1-cloud-app-catalog`]
...
### 7.4 Shadow-agent reconciliation artifact
...
## 8. Remediation Walkthrough — Four-Step Ordering
...
### 8.1 Step 1 — Agent 365 inline Assign Owner (preferred)

The Agent 365 Ownerless card's **inline Assign Owner** action (§1.3) is the **default** remediation surface because it:
...
Use Step 1 for **every** agent that surfaces in the Ownerless card and has a known replacement owner. Only escalate to Steps 2–4 when Step 1 cannot complete (e.g., the agent does not appear in the card during rollout lag, or the replacement owner requires a sponsor / manager confirmation upstream).

### 8.2 Step 2 — Confirm or override 2.26 sponsor manager-transfer

If the orphan was triggered by a sponsor leaver, the 2.26 Lifecycle Workflow may have already transferred sponsorship to the leaver's manager. Confirm via §2.3 and:

- **Accept the transfer** if appropriate (capture confirmation in the session worksheet).
- **Override** via §2.4 if the manager is not a valid sponsor (e.g., manager also disabled, or Zone 3 clearance gap).
...
### 8.3 Step 3 — PPAC / Copilot Studio maker reassignment

If the orphan involves a Power Platform maker or environment owner (§3 / §4), execute the PPAC reassignment per those sections. Specifically:
...
### 8.4 Step 4 — Bulk Graph / PowerShell
...
### 8.5 Remediation ordering decision tree
...
!!! example "Examiner Evidence Box — Remediation"
    ...
    5. Time-to-remediation timestamp (for SLA tracking, see §9).
...
### 9.1 Per-session evidence packet
...
| Remediation pre / post screenshots | §8 | PNG | 6 years |
| Defender for Cloud Apps reconciliation artifact | §7 | CSV / PDF | 6 years |
...
### 9.3 Monthly and quarterly roll-up
...
3. **Annual control-effectiveness review** — aggregate trend analysis, coverage-gap assessment, and control-design refresh recommendations. Supports independent review expectations under OCC Bulletin 2026-13 (formerly OCC Bulletin 2011-12) / Fed SR 26-2 (formerly SR 11-7).
...
- [ ] Ownerless card reviewed for the in-scope zone(s) (§1)
- [ ] Lifecycle Workflow run history reviewed (§2)
- [ ] PPAC maker sweep executed (§3)
- [ ] PPAC environment-owner sweep executed (§4)
- [ ] SharePoint Copilot author sweep executed (§5)
- [ ] PPAC recycle bin audit executed (§6)
- [ ] Defender for Cloud Apps shadow-agent sweep executed (§7)
- [ ] Remediations executed in the four-step ordering (§8)
- [ ] Per-session evidence packet assembled and stored in Purview (§9.1)
- [ ] Cadence drift, if any, documented with root-cause analysis (§9.2)
