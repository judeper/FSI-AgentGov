...
**Estimated time:** 8–14 hours initial setup across 6+ admin roles (multi-day calendar; per-matter time variable — see §9 SLAs)<br>
...
- Copilot Studio agent transcripts that persist to mailbox; Copilot Studio transcripts that persist to **Dataverse** are documented as a coverage gap with compensating control in §5.5 and §13 anti-pattern AP-06.
...
- Adversarial-input detection (Control 1.21 — its evidence is *preserved* via the §9.10 handoff into this control).
...
| Verify export hash chain | ❌ (manifest visible in portal) | ✅ `Get-FileHash -Algorithm SHA256` | PowerShell required for end-to-end verification — see §7.5 |
| Records Management Preservation Lock (SEC 17a-4(f) handoff) | See **Control 1.9** | See **Control 1.9** | Out of scope here |
| Power Platform Dataverse audit + admin export (Copilot Studio gap compensating control) | ✅ Power Platform admin center | ✅ Power Platform admin PowerShell module | See §5.5 |
...
- [ ] **Copilot Studio license** for the environments hosting Copilot Studio agents — relevant for the §5.5 Dataverse compensating control.
- [ ] **Power Platform per-app or per-user license** in environments hosting Copilot Studio agents — relevant for §5.5.
...
| **Records Manager** (Purview Records Manager role) | Records Management surface in Purview | n/a | Records / Information Governance lead | Required for the §9.1 SEC 17a-4(f) Preservation Lock handoff into Control 1.9 |
...
| **Compliance Officer** | Approves legal holds; signs off on production packages | Per matter | Named per WSP | Approves the two-admin pattern in §5.4 |
...
- [ ] **Copilot Studio agent inventory** maintained in Control 1.2 agent registry, with environment ↔ Dataverse-table mapping for the §5.5 compensating control.
...
- [ ] Copilot Studio Dataverse coverage gap disclosed in the WSP per §5.5 and AP-06.

### 1.5 Test fixtures gate
...
- [ ] **Canary string** — a unique, traceable token (e.g., `FSI-CANARY-1.19-2026-04-XXXX`) embedded in a synthetic Copilot prompt. Used in §10 quarterly drill end-to-end verification.
- [ ] **PyRIT-derived adversarial fixture** coordinated with Control 1.21 — required for the §9.10 adversarial-evidence preservation handoff verification.
- [ ] **Test export landing zone** — a non-production immutable storage account or tenant-isolated SharePoint library, used for §7 dry runs without committing to long-term retention.

---


## §2 Create the case and add the custodian

### 1.1 Open the case dashboard
...
- For Zone 3 customer-facing or recordkeeping-scope cases: confirm **eDiscovery Administrator** elevation has been activated through PIM with a ticket reference; capture the activation timestamp and the justification text into the case notes per §1.2.

### 1.3 Create the case

1. Click **Create case**.
2. **Name** the case using the firm's case-naming convention (recommended pattern: `{matter-id}-{matter-type}-{custodian-count}-{YYYYMMDD}`, e.g., `FSI-2026-0417-FINRA8210-CUST3-20260417`). Naming convention drives the §13 evidence pack indexing — see anti-pattern AP-15.
...
### 1.4 Capture pre-flight evidence
...
- [ ] Record the case ID into the §13 evidence-pack manifest under artifact #1.
...
3. **Microsoft 365 Copilot interactions toggle** — in the custodian wizard, confirm the **Microsoft 365 Copilot interactions** location is enabled for each custodian. If the toggle is absent in your cloud, document the gap per §1.1 and substitute the SubstrateHolds-direct compensating pattern in §5.3.
4. **Hold-notice (custodian acknowledgment) workflow** — for FRCP 37(e) and FINRA 8210 matters, configure the hold-notice template under **Custodians → Communications**. Acknowledgment metadata feeds the §13 evidence pack (artifact #4).
...
6. Click **Next** through the wizard, leaving the hold creation step **unchecked at this stage** (we author the hold separately in §5 to enforce the two-admin pattern).
7. Click **Submit**. The custodian appears in the Data sources list with status **Indexing** (typically 4–24 h to complete).

### 1.6 Add non-custodial data sources (agent grounding surfaces)
...
### 1.7 Capture custodian + non-custodial evidence
...
## §3 Location selection — the five surfaces that hold agent interactions

Custodian addition (§2.5) attaches the standard four locations (mailbox, OneDrive, Teams, SharePoint personal). Agent-interaction discovery requires explicit attention to **five surfaces**, only some of which are toggled by the custodian wizard.

### 2.1 Microsoft 365 Copilot interactions location
...
2. Select the in-scope custodians (must already be added per §2.5).
3. The location attaches the **SubstrateHolds container** of each custodian's mailbox to the case scope.
4. Click **Submit**.

If the **Microsoft 365 Copilot interactions** location is not yet visible, use the SubstrateHolds-direct compensating pattern in §5.3 — add the custodian mailbox with an explicit search scope that includes the SubstrateHolds folder by `itemclass` filter.

### 2.2 Custodian Exchange Online mailbox (including SubstrateHolds)

Already attached by §2.5. Confirm:
...
### 2.3 SharePoint sites and OneDrive accounts (grounding surfaces)

1. Confirm the §2.6 non-custodial sources include every SharePoint site and OneDrive account the in-scope agent could ground on.
...
### 2.4 Teams channels and chats

1. For 1:1 and group chat content, the custodian mailbox attachment (§3.2) is the substrate path.
...
### 2.5 Copilot Studio Dataverse (coverage gap)
...
**Compensating control (operationalized in §5.5):** Power Platform Admin runs Dataverse audit export per environment per matter; the export is hashed (SHA-256) and added to the §13 evidence pack as artifact #18. Document the gap to General Counsel **at hold issuance**, not at production. See anti-pattern AP-06.

### 2.6 Capture location evidence
...
## §4 Author the search — KeyQL plus the Copilot activity condition card

### 3.1 Open the search authoring pane
...
### 3.2 Choose locations

1. Select the custodians, non-custodial sources, and the Microsoft 365 Copilot interactions location attached in §2 / §3.
...
#### 4.3.3 The `from:"Copilot"` anti-pattern
...
**Correct approach:** rely on the **Microsoft 365 Copilot interactions** location (§3.1) plus the **Copilot activity** condition card (§4.4) to scope. Use KeyQL only for date, topical keywords, sensitivity labels, and SIT references — not as the primary "is this a Copilot item" filter.

#### 4.3.4 SIT and label scoping
...
### 3.4 The Copilot activity condition card

The **Copilot activity** condition card (in the search authoring UI under **Add condition → Copilot activity**, where available) lets you scope by:
...
Use this card as the **primary** "is this a Copilot item and which agent" filter — in combination with the location selection in §3.1, not in lieu of it.

### 3.5 The CopilotInteraction-content anti-pattern
...
3. Join both in the §13 evidence pack — neither alone is sufficient.

### 3.6 Run the search and review statistics
...
2. Review the **Statistics** tab:
   ...
   - **Unindexed items** count — non-zero counts must be triaged; for Zone 3, the Compliance Officer signs off on the disposition of unindexed items per §9.4.

### 3.7 Common search-statistics signals

| Signal | Likely cause | Action |

| Custodian shows zero items but other custodians return content | Indexing not complete (status "Indexing" still); or license assignment recent | Wait 24 h; verify license per §1.1 |
| High unindexed count concentrated in one custodian | Mailbox-corruption or item-class-not-recognized condition | Triage with Exchange Admin; document in §9.4 |
...
## §5 Issue the legal hold (case-scoped) — two-admin pattern

### 4.1 Conceptual model — hold preserves availability, not format
...
### 4.2 Open the hold authoring pane
...
3. **Description** must include the FRCP 37(e) reasonable-anticipation trigger date, the FINRA 8210 letter date (if applicable), the hold-author UPN, and the named hold-approver UPN (the second admin per §5.4).

### 4.3 Choose locations

1. Select the same custodians, non-custodial sources, and Copilot interactions location used in the §4 search.
...
### 4.4 The two-admin pattern (Zone 3 mandatory)
...
### 4.5 Copilot Studio Dataverse compensating control (issued in parallel with the hold)
...
3. **Export:** trigger an admin-scoped Dataverse export of the transcript table(s) filtered by the custodian and date range matching the §4 search; the export is JSONL or CSV per the firm's standard.
4. **Hash:** Power Platform Admin computes SHA-256 of the export file.
5. **Lock:** the export is placed in immutable storage (matching the §9.1 SEC 17a-4(f) pathway under Control 1.9).
...
## §6 Review set — culling, redaction, conversation reconstruction

### 5.1 Add search results to a review set
...
### 5.2 Verify review-set ingestion
...
3. The Reviewer role (separate human per §1.2) performs the substantive review and tagging.

### 5.4 Redact for privilege and NPI
...
### 5.5 Themes, near-duplicate, predictive coding (E5 only)
...
### 5.6 Capture review-set evidence
...
## §7 Export the production package — load file + native + metadata + hash chain

### 6.1 Open the export pane
...
### 6.2 Run the export and download
...
### 6.3 Verify the Microsoft-generated manifest
...
### 6.4 Build the chain-of-custody record
...
### 6.5 PowerShell — independent SHA-256 verification
...
Capture the output into the §13 evidence pack as artifact #21. The PowerShell-computed hash must match the manifest hash; any divergence is a chain-of-custody break and must be triaged before transmission. See anti-pattern AP-14.

### 6.6 Hand off to Records Management Preservation Lock (Control 1.9)
...
3. The Records Manager signs the §13 artifact #22 (Preservation Lock confirmation memo).

### 6.7 Designated Supervisor sign-off
...
4. Signs the §13 artifact #23 (supervisory sign-off memo) with name, CRD number, date, and timestamp. **Group sign-off is not acceptable** — the human supervisor must be named. See anti-pattern AP-13.

### 6.8 Capture export evidence
...
| eDiscovery role posture | Manager standing assignment OK; Reviewer optional | Manager standing OK; Reviewer mandatory; Comm Compliance separation enforced | **Two-admin pattern mandatory** (§5.4); eDiscovery Administrator PIM-only; Manager / Reviewer separation enforced (AP-08) |
...
| Quarterly drill participation | Optional | Recommended | **Mandatory** — see §10 |
| Evidence-pack completeness | Subset of §13 (artifacts #1, #2, #4, #8, #11, #14, #17, #20, #21) acceptable | Most of §13 (omit #18 if no Copilot Studio in scope) | **All 25 artifacts** of §13 |
...
## §9 FSI incident-handling pathways

Each pathway below is a regulator-facing scenario keyed to a specific clock and a specific evidence-package shape. The case-handling steps in §2–§7 produce the evidence; this section governs **timing, supervisory sign-off, and notification orchestration**.

### 8.1 SEC 17a-4(b)(4) recordkeeping retention floor + 17a-4(f) format compliance
...
**Pathway:**
1. eDiscovery hold preserves availability (this control, §5).
...
4. lock posture quarterly per §10.

### 8.2 SEC 17a-4(f) October 2022 audit-trail alternative
...
2. eDiscovery export with SHA-256 verification (§7.5) provides the cryptographic checksum.
...
### 8.3 FINRA Rule 8210 production response
...
**Pathway:**
1. **Day 0 (letter received):** General Counsel logs the matter; Compliance Officer convenes the case team; eDiscovery Manager opens the case per §2.
2. **Day 0 + 24 h:** hold issued under the two-admin pattern per §5.4.
...
4. **Day 1–7:** search authoring and statistics review (§4); Compliance Officer reviews scope.
5. **Day 7–21:** review-set ingestion, culling, tagging, redaction (§6); General Counsel reviews privilege calls.
6. **Day 21–28:** export (§7); independent SHA-256 verification; chain-of-custody assembly; Designated Supervisor sign-off.
...
### 8.4 FRCP Rule 37(e) anti-spoliation defense
...
**Pathway:**
1. **Hold issuance within 24 h of trigger** per §5 two-admin pattern.
2. **Hold notice to custodians** per §2.5 with acknowledgment workflow — the acknowledgment metadata is the affirmative-defense evidence at any future 37(e) hearing.
3. **Hold scope broadened** per §5.3 — over-preserve at the substrate level rather than rely on a narrow Copilot-only slice that may need to widen later.
...
5. **Triage unindexed items** per §4.7; Compliance Officer signs off on any non-zero unindexed disposition.

### 8.5 SOX Section 802 (18 USC §1519) anti-spoliation
...
**Pathway:**
1. Hold issuance under the two-admin pattern (§5.4) provides the procedural integrity.
2. Manager / Reviewer separation (§1.2) provides the role-segregation defense.
3. PIM elevation of eDiscovery Administrator (§1.2) prevents the standing-superuser anti-pattern (AP-09).
4. Records Management Preservation Lock (Control 1.9) provides format-immutability.
5. Designated Supervisor sign-off (§7.7) provides the named-human accountability.

### 8.6 GLBA Safeguards Rule (16 CFR Part 314) and Reg S-P 30-day customer notification (May 2024 amendment)
...
**Pathway:**
1. eDiscovery case scoped to identify the affected customer set: search by SIT (`SensitiveType:"U.S. Social Security Number (SSN)"`, etc.) per §4.3.4.
2. Review-set culling (§6.3) confirms the identification of affected customers.
...
4. Production package (§7) supports the notification narrative and any subsequent regulator inquiry.
...
### 8.7 NYDFS 23 NYCRR Part 500 §500.17(a) 72-hour notification
...
**Pathway:**
1. Hold issuance per §5 within 24 h of trigger to preserve evidence ahead of the 72-h notification.
...
### 8.8 SEC Form 8-K Item 1.05 (cybersecurity incident materiality) 4-business-day clock
...
### 8.9 FINRA Rule 4530 customer-complaint reporting
...
**Pathway:**
1. Hold issuance per §5 within 24 h of complaint receipt.
2. eDiscovery scope: the specific Copilot interactions plus any grounding sources; Conversation reconstruction (§6.1) is mandatory to reconstruct the multi-turn context the customer experienced.
3. Production package supports any subsequent FINRA arbitration or 8210 escalation.

### 8.10 Adversarial-input incident handoff from Control 1.21
...
3. Use the Copilot activity condition card (§4.4) to scope by conversation thread ID matching the Defender alert.
...
### 8.11 Communication Compliance escalation (from Control 1.10)
...
**Pathway:**
1. Comm Compliance Investigator (separate human from any case Manager / Reviewer per §1.2) escalates to Compliance Officer.
2. Compliance Officer convenes the case team; eDiscovery Manager opens the 1.19 case per §2.
...
4. Maintain Comm Compliance / eDiscovery role separation (§1.2) for the FINRA 3110 supervisory-independence defense.

---

## §10 Quarterly Zone 3 drill
...
- One named test custodian (per §1.5).
...
- One PyRIT-derived adversarial fixture per §9.10 (rotated each quarter).

### 9.2 Drill scenario

Choose one of the §9 scenarios per quarter; rotate annually:

- Q1: FINRA 8210 (~30-day production)
- Q2: FRCP 37(e) (24-h hold issuance)
- Q3: Reg S-P 30-day customer notification
- Q4: §9.10 adversarial-input handoff from Control 1.21
...
4. **Steps:** execute §2 → §7 end-to-end against the test custodian + canary; produce a synthetic export.
5. **Validate canary recovery:** the canary string must surface in the §6 review set and the §7 export. If it does not, root-cause the gap (license, location, KeyQL, hold propagation, indexing) and remediate before drill closeout.
6. **Validate Dataverse compensating control:** the Power Platform Admin runs §5.5 against the test environment; the Dataverse export hash is recorded.
7. **Validate hash chain:** §7.5 PowerShell SHA-256 matches Microsoft manifest.
...
The report is filed to the §13 evidence pack as artifact #25 and feeds the Control 1.2 risk register update.

### 9.5 Drill cadence enforcement
...
| §1 pre-flight gates | Pre-flight checklist signed by Purview Compliance Admin + Compliance Officer | Compliance Officer |
| §2 case + custodians | `CaseAdded` and `CustodianAddedToCase` UAL evidence | eDiscovery Manager |
| §3 Copilot interactions location | Location screenshot + Control 4.6 mapping spreadsheet | AI Governance Lead |
| §4 search | Search definition export + statistics screenshot + canary-recovery confirmation | eDiscovery Manager |
| §5 hold | Two-admin UAL evidence + `Get-Mailbox` `InPlaceHolds` output + Dataverse compensating-control memo | eDiscovery Manager + Power Platform Admin |
| §6 review set | Ingestion configuration + tagging summary + privilege log | Reviewer + General Counsel |
| §7 export | Microsoft manifest + PowerShell SHA-256 + chain-of-custody record + Preservation Lock + Designated Supervisor sign-off | eDiscovery Manager + Records Manager + Designated Supervisor |
| §9 incident scenarios | Per-scenario clock-compliance evidence | Compliance Officer + General Counsel |
| §10 quarterly drill | Drill report (artifact #25) | AI Governance Lead |
...
## §13 Evidence pack — 25 artifacts with SHA-256 manifest

The evidence pack is the auditable bundle that survives the matter. Build it during the case, not after; the §10 quarterly drill validates that the build process is operational.

### 12.1 Artifact inventory

| # | Artifact | Section | Format | Owner |

| 1 | Case overview screenshot + `CaseAdded` UAL record | §2.4 | PNG + CSV | eDiscovery Manager |
| 2 | Pre-flight checklist signed by Compliance Officer | §1 | PDF | Compliance Officer |
| 4 | Custodian list + hold-notice acknowledgments | §2.5 | CSV + PDF | eDiscovery Manager |
| 5 | Non-custodial source ↔ Control 4.6 grounding inventory mapping | §2.6 | XLSX | AI Governance Lead |
| 6 | Copilot interactions location screenshot | §3.1 | PNG | eDiscovery Manager |
| 7 | Mailbox / SharePoint location verification | §3.2–4.4 | PNG + CSV | eDiscovery Manager |
| 8 | Search definition export (KeyQL + condition cards) | §4.6 | JSON + PNG | eDiscovery Manager |
| 9 | Search statistics screenshot | §4.6 | PNG | eDiscovery Manager |
| 10 | `ComplianceSearchCreated` / `Started` UAL records | §4.8 | CSV | eDiscovery Manager |
| 11 | Hold definition + inactive-state screenshot + active-state screenshot + two-admin UAL evidence | §5.7 | PNG + CSV | eDiscovery Manager (author) + approver |
| 12 | `Get-Mailbox InPlaceHolds` PowerShell output | §5.6 | TXT | eDiscovery Manager |
| 13 | Review-set ingestion configuration screenshot | §6.6 | PNG | eDiscovery Manager |
| 14 | Tagging summary report | §6.6 | CSV | Reviewer |
| 15 | Privilege log with redaction reason codes | §6.6 | XLSX + PDF | General Counsel |
| 16 | Predictive-coding model summary (if used) | §6.5 | PDF | eDiscovery Manager |
| 17 | Export job summary + Microsoft `Summary.csv` | §7.8 | PNG + CSV | eDiscovery Manager |
| 18 | Copilot Studio Dataverse compensating-control memo + export hash | §5.5 | PDF + TXT | Power Platform Admin |
| 19 | Microsoft-generated export manifest | §7.3 | TXT/JSON | eDiscovery Manager |
| 20 | Export ZIP (native + extracted text + metadata + load file) | §7.2 | ZIP | eDiscovery Manager |
| 21 | Independent PowerShell SHA-256 of export ZIP | §7.5 | TXT | eDiscovery Manager |
| 22 | Records Management Preservation Lock confirmation (Control 1.9 handoff) | §7.6 | PDF | Records Manager |
| 23 | Designated Supervisor sign-off memo (named human + CRD + timestamp) | §7.7 | PDF | Designated Supervisor |
| 24 | Chain-of-custody record (transmission to receiving party + acknowledgment) | §7.4 | PDF | eDiscovery Manager + General Counsel |
| 25 | Quarterly drill report (most recent applicable) | §10.4 | PDF | AI Governance Lead |

Zone 1 evidence packs may omit artifacts marked optional in §8. Zone 2 packs may omit artifact #18 if no Copilot Studio agent is in scope. Zone 3 packs **must** contain all 25 artifacts.

### 12.2 SHA-256 manifest — PowerShell
...
### 12.3 Anti-pattern catalog (must-not-do — 15 entries)

Each anti-pattern below has been observed in real FSI matters and has documented regulatory or defensibility consequences. Train every Zone 3 case team member on this catalog at onboarding and at the §10 quarterly drill.

| # | Anti-pattern | Why it fails | Correct pattern |

| **AP-01** | Writing `from:"Copilot"` or `sender:Copilot` in KeyQL to "find Copilot messages" | Copilot prompts/responses do not have a sender of "Copilot" in the substrate; the filter returns zero | §4.3.3 — use the Copilot interactions location (§3.1) plus the Copilot activity condition card (§4.4) |
| **AP-02** | Treating `CopilotInteraction` Unified Audit Log records as content evidence | The audit record is metadata only — no prompt body, no response body | §0.3 + §4.5 — join Plane 3 metadata with Plane 1 eDiscovery content |
| **AP-03** | Substituting Content Search for a case-scoped legal hold | Content Search does not preserve — items remain subject to deletion / retention purge | §5 — always issue a case-scoped hold via the Holds tab; the two-admin pattern under §5.4 in Zone 3 |
...
| **AP-05** | Searching only "Teams" without including custodian mailbox to find Copilot interactions | Copilot prompts/responses live in mailbox SubstrateHolds, not in Teams chat threads | §3.2 — always attach the custodian mailbox; use the Copilot interactions location for clarity |
| **AP-06** | Treating Copilot Studio Dataverse `bot_transcript` content as covered by unified eDiscovery | Native unified-eDiscovery integration is in limited preview; Dataverse content is a known coverage gap | §5.5 — Power Platform Admin compensating control with weekly hashed exports |
| **AP-07** | Treating an eDiscovery hold as WORM / SEC 17a-4(f)-compliant | Hold preserves availability; format compliance requires Records Management Preservation Lock under Control 1.9 | §9.1 + §9.2 — pair the hold with the Preservation Lock, do not substitute |
| **AP-08** | Granting a single human both Reviewer and Manager rights on the same case | Collapses separation of duties; Reviewer cannot export, Manager can — combining defeats the FINRA 8210 / Reg S-P privileged-handling defense | §1.2 — strict role separation in Zone 3 |
| **AP-09** | Standing eDiscovery Administrator assignment without PIM elevation | Administrator can see and manage all cases tenant-wide; standing assignment is a privileged-access control failure under SOX 802 and OCC Bulletin 2026-13 (formerly OCC 2011-12) | §1.2 — eligible-only PIM with mandatory ticket reference and short activation window |
| **AP-11** | Skipping conversation reconstruction in review-set ingestion for Teams / Copilot matters | Reviewers see individual prompt/response items in isolation; loses multi-turn context; inflates privilege-call risk | §6.1 — conversation reconstruction on for any Teams / Copilot matter |
| **AP-12** | Single-human authoring + activating a Zone 3 hold in one session | Collapses the two-admin pattern; UAL shows single `UserId` for both `HoldCreated` and the activation `HoldUpdated` | §5.4 — distinct author UPN and approver UPN; capture both UAL records |
| **AP-13** | Group sign-off (e.g., "Compliance team approved") in lieu of a named Designated Supervisor with CRD number | FINRA 3110 requires named supervisor accountability; group sign-off is not defensible | §7.7 — named human, CRD, timestamp |
| **AP-14** | Skipping the independent PowerShell SHA-256 verification of the export ZIP and relying solely on the Microsoft-reported manifest hash | The Microsoft manifest reports the pre-zip checksum in some configurations; independent post-zip hash is the chain-of-custody anchor | §7.5 — always run `Get-FileHash -Algorithm SHA256` against the downloaded ZIP |
| **AP-15** | Ad-hoc case / search / hold / export naming that does not encode matter ID + custodian count + date | Defeats indexing of the §13 evidence pack; complicates supervisory review and any subsequent regulator inquiry | §2.3 + §4.1 + §5.2 + §7 — use the documented naming convention |
...
- **Control 1.10 — Communication compliance for AI** (Comm Compliance escalations feeding eDiscovery cases per §9.11)
- **Control 1.13 — Sensitive Information Types for AI** (SITs consumed in review-set culling per §4.3.4 and §6.3)
- **Control 1.14 — Data minimization for AI grounding** (reducing the discoverable surface upstream)
- **Control 1.21 — Adversarial input detection for AI agents** (the upstream evidence source for §9.10)
- **Control 1.24 — AI incident response** (the §9 scenario clocks and notification workflow handoff)
- **Control 4.6 — Agent grounding scope** (the non-custodial source inventory per §2.6 and §3.3)
...
5. **Feed lessons learned** into the next §10 quarterly drill agenda.
