...
!!! danger "READ FIRST — Scope, Sibling Routing, and Non-Substitution"
    ...
    **In scope (this file)**
    ...
    |-------|----------------|
    | Defender XDR Unified Incidents queue, severity rubric, MITRE ATT&CK + ATLAS tagging | §1 |
    | Microsoft Sentinel Incidents and SOAR (Logic Apps) playbook wiring | §2 |
    | Purview Insider Risk Management — Cases (pseudonymized review, AI-agent-misuse templates) | §3 |
    | Purview eDiscovery (Premium) — legal holds, custodian notices, regulator-production export | §4 |
    | Purview Communication Compliance — Copilot-generated content supervisory escalation | §5 |
    | Microsoft 365 Service Health — Microsoft-side correlation **before** firm-side root-cause declaration | §6 |
    | SharePoint IR list — case fields, regulator notification matrix, legal-hold-aware site | §7 |
    | Power Automate notification flows — per-regulator timer countdowns, Teams adaptive cards | §8 |
    | Materiality determination workflow (SEC Form 8-K Item 1.05) | §9 |
    | OFAC sanctions screening for ransom payments | §10 |
    | Tabletop exercise harness (quarterly AI-specific scenarios) | §11 |
    | Regulator portal references (NYDFS, EDGAR, FINRA Firm Gateway, FTC, CISA, state AGs) | §12 |
...
!!! warning "Non-Substitution"
    ...
    - **Legal hold and litigation preservation** — issued and revoked by the General Counsel or designee through Purview eDiscovery (Premium); automated holds described in §4 are operational support, not legal sufficiency.
    - **Regulator notification filings** — NYDFS Cybersecurity Portal, SEC EDGAR Form 8-K, FINRA Firm Gateway 4530, FTC online breach form, CISA portal (when CIRCIA effective), and state-attorney-general portals are filed by **named human filers** in Compliance, Legal, or the Privacy Office. Power Automate countdown timers (§8) **inform** filers — they do not file.
...
| 1 | Defender XDR Unified Incidents — triage, severity rubric, MITRE ATT&CK + ATLAS tagging | security.microsoft.com → Investigation & response → Incidents | VC-3 |
| 1 | Microsoft Sentinel — Incidents and SOAR (Logic Apps) playbooks | Azure portal → Sentinel workspace → Incidents | VC-4 |
| 1 | Purview Insider Risk Management — Cases | purview.microsoft.com → Insider risk management → Cases | VC-5 |
| 1 | Purview eDiscovery (Premium) — legal hold and regulator-production export | purview.microsoft.com → eDiscovery → Premium | VC-6 |
| 1 | Purview Communication Compliance — Copilot supervisory escalation | purview.microsoft.com → Communication compliance | VC-7 |
| 1 | Microsoft 365 Service Health — Microsoft-side correlation gate | admin.microsoft.com → Health → Service health | VC-8 |
| 1 | SharePoint IR list — case fields, notification matrix, legal-hold site | SharePoint admin center + IR site | VC-9 |
| 1 | Power Automate — notification flows, per-regulator timers, Teams adaptive cards | make.powerautomate.com | VC-10 |
| 1 | Materiality determination workflow (SEC 8-K Item 1.05) | Disclosure Committee — SharePoint + Teams | VC-11 |
| 1 | OFAC sanctions screening for extortion / ransom payment | GC + Treasury workflow | VC-12 |
| 1 | Tabletop exercise harness — quarterly AI-specific scenarios | Teams + SharePoint | VC-13 |
| 1 | Regulator portal references — NYDFS, EDGAR, FINRA, FTC, CISA, state AGs | External portals | VC-14 |
| 1 | Evidence capture, cadence, and examiner packet | All portals + Purview | VC-15 |

The numbering of regulator timers used throughout this walkthrough mirrors the **Regulator Notification Matrix** in [Control 3.4](../../../controls/pillar-3-reporting/3.4-incident-reporting-and-root-cause-analysis.md). Detection and triage signals upstream of §1 are owned by [Control 1.8](../../../controls/pillar-1-security/1.8-runtime-protection-and-external-threat-detection.md) and [Control 3.9](../../../controls/pillar-3-reporting/3.9-microsoft-sentinel-integration.md); supervisory escalation downstream of §1 is owned by [Control 2.12](../../../controls/pillar-2-management/2.12-supervision-and-oversight-finra-rule-3110.md).
...
- [ ] Incident severity (Sev-1 / 2 / 3) provisionally assigned per §1.3 rubric
...
Proceed to §1 (Defender XDR) after every box is ticked or a documented exception is approved by the Incident Commander.
...
### 1.2 Triage UX — column set, filters, and row actions
...
### 1.3 FSI severity rubric — beyond the Defender default

Defender XDR's default Sev (Informational / Low / Medium / High) is necessary but **not sufficient** for FSI Control 3.4. Apply the following overlay during triage; the resulting severity is what enters the SharePoint IR list and drives the §1 timer flows.
...
!!! warning "Rubric overrides require documented justification"
    A mismatch between the Defender XDR default severity and the FSI Sev assigned by the Incident Commander is permissible but must be justified in writing. The justification is captured in the incident worksheet (SharePoint IR list row), in the Defender XDR incident comment (§1.6), and in the Sentinel incident comment (§1.4).

### 1.4 MITRE ATT&CK + MITRE ATLAS dual tagging
...
### 1.5 Multi-source correlation review
...
### 1.6 Assignment, status, and incident comments
...
2. Set:
    ...
    - **Severity** — set to the **FSI Sev** per §1.3 (override the default if needed).
    - **Status** — `In progress` once triage starts; `Resolved` only at full close-out (do **not** resolve before the SharePoint IR list is closed and the §8 timers are stopped).
...
### 1.7 Linking to Microsoft Sentinel
...
## 2. Microsoft Sentinel — Incidents and SOAR (Logic Apps) Playbooks

Microsoft Sentinel extends Defender XDR triage with **automation rules**, **Logic Apps playbooks** (SOAR), **investigation graph**, and **long-window hunting**. Control 3.4 uses Sentinel as the **orchestration spine** that fans incidents out to Power Automate notification flows (§8), Teams adaptive card escalations, SharePoint IR list updates (§1), and ITSM ticketing.
...
### 2.2 Reviewing the synced incident

For an incident that originated in Defender XDR (§1):
...
### 2.3 Custom AI-agent entity in the investigation graph

Where the workspace has the optional **AI Agent custom entity** mapping configured (per [Control 3.9 §1](../../../controls/pillar-3-reporting/3.9-microsoft-sentinel-integration.md)), the investigation graph displays the agent as a first-class node alongside users and devices. Confirm:
...
### 2.4 Adding the incident comment with the Sentinel-side overlay
...
### 2.5 Automation rules — wiring incidents to playbooks
...
### 2.6 Manually invoking a playbook
...
### 2.7 Reviewing playbook run logs
...
### 2.9 Closing the Sentinel incident

Sentinel close-out lags Defender XDR close-out by design — close Sentinel **only after** the §8 timers have either reached their statutory determination point or been documented as not applicable, and the SharePoint IR list row is signed.
...
## 3. Microsoft Purview Insider Risk Management — Cases
...
### 3.2 Pseudonymization — confirm before you click
...
### 3.3 Opening a case from an alert
...
### 3.4 Case workspace — Activity, Content explorer, User activity, Alerts
...
2. **Alerts** — every IRM alert correlated with this user / case. Cross-reference each alert against the SharePoint IR list row.
...
4. **Content explorer** — preview of the actual content involved (where the user role permits content view). Treat content view as a **legal-hold-relevant action** — confirm a hold is in place (see §4) **before** previewing sensitive content.

### 3.5 AI-agent-misuse case — required steps
...
3. Where **prompt content** is needed for the investigation, request it through eDiscovery (Premium) per §4; do **not** attempt to extract prompts from IRM Content explorer alone.

### 3.6 Escalation to eDiscovery (Premium)
...
3. Confirm the **Notify** toggle is set to **On** so a hold notice is generated automatically — General Counsel reviews and releases the notice (§3.4) before it is sent.
4. Click **Escalate**.
...
## 4. Microsoft Purview eDiscovery (Premium) — Legal Hold and Regulator-Production Export
...
3. Confirm the case from §4.6 (or create a new case if the hold is not IRM-originated — proceed to §4.2).

### 4.2 Creating an eDiscovery (Premium) case directly
...
### 4.3 Adding custodians and creating a legal hold
...
### 4.4 Hold-notice acknowledgments
...
3. Click **Send**.
4. Track acknowledgment status under **Communications → <notice> → Recipients**. Non-acknowledgment after the deadline is a **finding**: route to General Counsel for follow-up; document the follow-up in the SharePoint IR list row.

### 4.5 Searches and review sets
...
### 4.6 Regulator-production export
...
5. Hash the package (SHA-256), store the hash in the SharePoint IR list row, and transfer the package to the regulator-production secure-transfer location per the firm's regulator-production procedure (out of scope here).
...
2. Toggle **Status = Off**.
3. Confirm a **Release communication** is sent to custodians (auto-generated if §4.4 was configured with **Release notice = On**).
4. Document hold-release in the SharePoint IR list row with the release timestamp and General Counsel UPN.
...
## 5. Microsoft Purview Communication Compliance — Copilot Supervisory Escalation
...
### 5.2 Reviewing a Copilot-content alert
...
4. Review the **Message content** with the analyst's pseudonymization defaults applied (same model as IRM §5.2).
5. If the content was **sent externally** (email departed the tenant), this is a **higher-severity incident** that may trigger Reg S-P customer notice (if NPI was disclosed) and / or FINRA 4530(d) customer-complaint statistical aggregation (if a complaint follows). Cross-reference into the SharePoint IR list row.

### 5.3 Disposition and escalation
...
| **Escalate for investigation** | Creates an Advanced eDiscovery case (parallel to §4) | Reviewer note + IRM / eDiscovery linkage |
| **Remove message (where supported)** | Soft-delete from the recipient mailbox | Reviewer note + General Counsel concurrence |

For Control 3.4 active incidents, the typical disposition is **Escalate for investigation** with a back-link to the SharePoint IR list row.

### 5.4 Supervisory close-out by Designated Principal
...
1. The escalator (typically the Communication Compliance Investigator) routes the escalation to the principal via Teams adaptive card (auto-generated by the §8 Power Automate flow `PB-Escalate-To-Series24`) or directly via the Purview UI.
...
## 6. Microsoft 365 Service Health — Microsoft-Side Correlation Gate
...
### 6.2 Filtering for relevant services
...
### 6.3 Recording the check as evidence
...
3. If a Microsoft-side issue plausibly correlates with the firm-side observed behavior, **pause** the clock-starting decision and document the correlation hypothesis in the SharePoint IR list row's `RootCauseStatement` field as **provisional**. Do **not** initiate regulator notifications based on a Microsoft-side issue without first ruling it out as the proximate cause.
...
## 7. SharePoint Incident Response List — Authoritative Firm Record

The **SharePoint IR list** is the firm's authoritative incident record outside the security tools. It is the source for the §8 Power Automate notification flows, and the artifact most frequently requested by examiners as the single-pane view of an incident.

### 7.1 Site setup (one-time)
...
|---|--------|------|----------|-------|
| 1 | `IncidentID` | Single line of text | Yes | Format `INC-FSI-YYYY-NNNN`; see §7.2 |
...
| 11 | `MaterialityDeterminationUTC` | Date and time | No | 8-K Item 1.05 trigger; see §9 |
...
| 26 | `RegulatorNotificationsRequired` | Multiple lines of text | No | One per line; see §7.3 |
...
### 7.3 Per-incident folder convention
...
Each sub-folder receives the screenshots and exports captured per the section's Examiner Evidence Box. The `99-final-evidence-pack/` folder is the consolidated, hashed package referenced in §1.
...
3. Confirm the site is included in the standing eDiscovery preservation per §7.1.
...
## 8. Power Automate — Notification Flows and Per-Regulator Timer Countdowns
...
The following flows are in scope. Each flow is created from a SharePoint trigger on the §7 Incidents list and emits one or more Teams adaptive cards plus list-item updates.
...
| `IR-Card-OFAC-Escalate` | List item updated where ransom-payment context tagged | Adaptive card to General Counsel + Treasury for OFAC pre-screen (see §10) | Power Platform Admin |

### 8.3 Walkthrough — `IR-Timer-NYDFS-72h` (representative pattern)
...
!!! warning "Flows do not file notices"
    The flows here are **deadline-surface** flows. They do **not** transmit anything to a regulator portal. Every regulator notice is filed by General Counsel (or designee) through the regulator's own portal (§12). Filing automation is explicitly out of scope and would create unacceptable legal exposure.

### 8.4 Run-history retention
...
## 9. Materiality Determination — SEC 8-K Item 1.05 Workflow
...
### 9.2 Disclosure Committee meeting

1. The Incident Commander emails the **Disclosure Committee chair** (typically the General Counsel or CFO) with the SharePoint IR list row link and a one-page incident summary.
...
4. The chair records the outcome in the §7 list `MaterialityOutcome` and `MaterialityDeterminationUTC` columns.
5. If the determination is **Material**, the §8 `IR-Timer-SEC-8K-4bd` flow auto-engages.

### 9.3 Drafting the 8-K Item 1.05
...
## 10. OFAC Sanctions Screening — Pre-Payment Gate for Extortion Demands
...
The §1 `IR-Card-OFAC-Escalate` flow auto-fires the moment the §1 list item is tagged with the ransom-payment context.
...
3. The determination is recorded in the §7 list `OFACScreeningOutcome` column.
...
## 11. Tabletop Exercise Harness — Quarterly AI-Specific Scenarios
...
### 11.2 AI-specific scenario library
...
| **Ransomware with extortion of agent training data** — Threat actor encrypts and threatens to leak the firm's fine-tuning corpus (which contains NPI). | NYDFS-24h-Ransom, Reg S-P, FTC, OFAC pre-screen | 1.7, 1.8, 1.9, 3.4 (incl. §10) |
...
### 11.4 After-action report template
...
Tabletop materials (pre-read, scenario script, inject log, AAR) are retained in the IR site under **Documents → Tabletops → <YYYY-QN — scenario>** and are part of the standard examiner packet (§13). Examiners frequently sample tabletop materials to assess program maturity.
...
## 12. Regulator Portal References
...
## 13. Evidence Capture, Cadence, and Examiner Packet

### 13.1 Per-incident evidence pack
...
1. The §7 list-item version-history export (CSV).
2. All §7 SharePoint IR list row contents (PDF).
3. Defender XDR incident export (§1; structured comments included).
4. Sentinel incident export and automation-rule run logs (§2).
5. IRM case open / close exports (§3).
6. eDiscovery (Premium) case summary, hold list, hold-notice acknowledgments, search statistics, export-package SHA-256 hash (§4).
7. Communication Compliance disposition records (§5).
8. Service Health screenshots at T0+1h, T0+24h, root-cause-determination (§6).
9. Power Automate flow run-history CSVs spanning the incident window (§8).
10. Disclosure Committee meeting record (§9).
11. OFAC pre-screen record (§10) where applicable.
12. Regulator-notice copies and filing receipts (§12) where applicable.
13. Lessons-learned memo (signed by Incident Commander, CISO, CCO, General Counsel).

The pack is compressed to a single zip; SHA-256 is computed and recorded in the §7 list `EvidencePackageSHA256` column. The pack is retained per the firm's retention schedule (typically 7 years for SEC / FINRA registrants, 6 years for banking, longer where litigation hold applies).

### 13.2 Standing examiner packet
...
| IR site permissions report | §7.4 | Monthly |
| SharePoint IR list full export | §7 | Monthly |
| §7 list schema export | §7.2 | Monthly + on change |
| Power Automate flow inventory | §8.2 | Monthly + on change |
| Flow run-history (28-day) for each flow | §8.4 | Monthly |
| Disclosure Committee charter and materiality framework | §9 | Annually + on change |
| OFAC sanctions-counsel relationship attestation | §10 | Annually |
| Tabletop calendar + last 12 months AARs | §11 | Quarterly |
| Regulator-portal access list (who has filer credentials) | §12 | Quarterly |
| Cross-reference matrix to Controls 1.5/1.7/1.8/1.9/1.11/1.12/2.6/2.12/2.25/3.6/3.9 | This file | Annually + on change |

### 13.3 Cadence summary

| Cadence | Activity |
|---------|----------|
| Continuous | §0 PIM activation per incident; §1 / §2 / §3 / §5 alert triage |
| Per incident | All sections relevant to the incident profile |
| Daily (during active incident) | SharePoint IR list update; §6 Service Health re-check; §8 flow-status review |
| Weekly | §8 flow run-history export |
| Monthly | §13.2 standing examiner packet refresh; §0 license-attestation refresh; §7.4 site permissions report |
| Quarterly | §11 tabletop exercise; §12 regulator-portal access review; §0 PIM eligibility review |
| Annually | §9 materiality framework review; §10 sanctions-counsel attestation; §11 annual functional exercise; full IR plan review and sign-off; regulator notification matrix review against current rules |
...
| Provision the §7 SharePoint list schema across additional environments | **PowerShell** or **PnP PowerShell** ([powershell-setup.md](./powershell-setup.md)) | Repeatable, drift-detectable |
...
- [ ] Standing eligibility reviewed quarterly; results recorded in the §13.2 examiner packet.
...
- [ ] §0.3 authoritative-source readiness confirmed for Defender XDR, Sentinel, Purview Audit, IRM, eDiscovery (Premium), Communication Compliance, Service Health, the §7 SharePoint list, and the §8 Power Automate solution.

### Configuration

- [ ] §1 Defender XDR custom view, FSI severity rubric, and structured-comment templates in place and known to SOC.
- [ ] §2 Sentinel automation rules deployed; run-history capture procedure documented.
- [ ] §3 IRM cases workspace permissioned; pseudonymization roles confirmed.
- [ ] §4 eDiscovery (Premium) standing case template configured; hold-notice template approved by General Counsel.
- [ ] §5 Communication Compliance Copilot-scoped policy fired test alert reviewed end-to-end with Series 24/66 disposition.
- [ ] §6 Service Health gate documented in IR plan as mandatory pre-clock-start step.
- [ ] §7 SharePoint IR list deployed with full schema; site is legal-hold-aware.
- [ ] §8 Power Automate flows deployed; 28-day run history exporting to the IR site.
- [ ] §9 Disclosure Committee charter and materiality framework approved; meeting template in use.
- [ ] §10 OFAC pre-screen workflow approved; sanctions-counsel relationship attested.
- [ ] §11 quarterly tabletop calendar published; AAR template in use.

### Evidence and reporting

- [ ] §12 regulator-portal reference reviewed by General Counsel within the last 12 months.
- [ ] §13 per-incident evidence-pack convention in place; standing examiner packet refreshed within the last 30 days.
- [ ] Annual board sign-off record on file.

### Cross-control alignment

- [ ] Recurring §1 / §2 false positives routed to [Control 1.8](../../../controls/pillar-1-security/1.8-runtime-protection-and-external-threat-detection.md) for tuning.
- [ ] Recurring §3 false positives routed to [Control 1.12](../../../controls/pillar-1-security/1.12-insider-risk-detection-and-response.md) for tuning.
- [ ] Recurring §5 false positives routed to [Control 2.12](../../../controls/pillar-2-management/2.12-supervision-and-oversight-finra-rule-3110.md) for tuning.
