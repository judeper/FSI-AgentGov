...
**Q6 — OCC SR 11-7 / Fed AI risk management (banks and BHCs).** Did the event surface a **model risk** issue — i.e., a defect in the agent's design, data, or controls that materially affects model output reliability or fairness?
- **Yes →** Open a model-risk finding in the MRM inventory, notify the Model Risk Officer, and queue for the next model-validation cycle. SR 11-7 does not have an external filing clock but **does** require documentation of the issue, root cause, and remediation in the model's lifecycle record.
- **No →** Continue to Q7.
...
| **E-03** | Azure AI Content Safety Prompt Shields verdict + classifier scores (jailbreak, indirect-injection, protected-material) | Prompt Shields | Defender for Cloud → Threat Protection for AI workloads → alert detail; export JSON | SR 11-7 model evidence |
| **E-04** | Defender XDR alert(s) for UPIA/XPIA on M365 Copilot, including the full alert evidence graph | Defender XDR | Security.microsoft.com → Incidents → export incident package; preserve `AlertId`, `IncidentId`, `EvidenceId` | SR 11-7, NYDFS §500.06 |
| **E-05** | Defender for Cloud Threat Protection for AI workloads alert (when Azure-hosted agent in scope) | Defender for Cloud | Defender for Cloud → Security alerts → download alert JSON | SR 11-7 |
...
| **E-07** | Sentinel incident package (analytics rule ID, query, raw events, entities, comments) | Sentinel | `Export-AzSentinelIncident` (community module) or portal "Run playbook → Export" | SR 11-7 |
...
| **E-09** | Principal context: Entra user object snapshot, group memberships, sign-in risk, MFA state at incident time | Entra ID | `Get-MgUser`, `Get-MgUserMemberOf`, Entra ID Protection risk detection export | NYDFS §500.07, SR 11-7 |
| **E-10** | Agent context: agent ID, publisher, last-modified user, knowledge-source list, plugin/connector list, Zone designation | Copilot Studio / M365 admin | `Get-CopilotAgent` (where available) + portal export of agent definition | SR 11-7 model inventory |
| **E-11** | Tenant configuration snapshot: Prompt Shields policy, Defender XDR policy, Comm Compliance policy, retention labels in scope | Multi-portal | Scripted export — see [powershell-setup.md](powershell-setup.md) §6 evidence script | SR 11-7 control evidence |
| **E-12** | Containment actions taken (account disable, agent suspend, policy tighten, session revoke), with operator identity and timestamp | IR ticket + change ticket | Manual entry; cross-reference change ticket ID | NYDFS §500.16, SR 11-7 |
...
- **§1.2 Reportability.**
  ...
  - Q6 (SR 11-7): yes — XPIA via grounded data is a **model risk** issue. MRM finding opened: "Copilot agent susceptible to white-text injection in PDF grounding source."
...
- **OCC Bulletin 2011-12 (SR 11-7) — Supervisory Guidance on Model Risk Management** — model design, validation, and ongoing monitoring; framework for AI/ML in regulated firms.
...
**Annual review:** at the 12-month mark, the Control 1.21 owner produces an **annual effectiveness statement** that summarizes trend-watch movement, anti-pattern catalog updates, and PIR root-cause distribution. This statement supports the firm's annual SR 11-7 model-risk attestation and the §1.2 Q5 books-and-records record.
