...
Do not proceed past §1 until every gate below is signed off. Each gate maps to a concrete artifact you will produce; the artifact name appears in the Evidence Pack table in §14.

### 1.1 License and capability gate
...
### 1.2 Role and PIM gate

You will need the following Entra / workload roles assigned to the named operator(s) for the duration of this walkthrough. Use canonical names from the [Role Catalog](../../../reference/role-catalog.md). Do **not** perform any §2–§10 step as Entra Global Admin; Global Admin must remain a break-glass role.

| Role | Required for | Assigned to (named operator) | Time-bound? |

| **AI Administrator** | Steps §2 (inventory reconcile), §5 (DSPM for AI), §6 (DLP-for-Copilot read), §7 (audit), agent-level configuration | _name + UPN_ | Yes — PIM eligible, activate for the change window |
| **Power Platform Admin** | Step §3 (Power Platform DLP) | _name + UPN_ | Yes — PIM eligible |
| **SharePoint Admin** | Step §4 (knowledge-source minimization, coordinate with Control 4.6); §9 SharePoint group hygiene | _name + UPN_ | Yes — PIM eligible |
| **Purview Compliance Admin** | Step §6 (DLP for M365 Copilot location), §7 (Audit search), §8 (Alert policies) | _name + UPN_ | Yes — PIM eligible |
| **Purview Data Security AI Admin** | Step §5 (DSPM for AI policies + Activity Explorer) | _name + UPN_ | Yes — PIM eligible |
| **Entra Identity Governance Admin** | Step §9 (Access Reviews) | _name + UPN_ | Yes — PIM eligible |
| **AI Governance Lead** | Approval of Zone 2 / Zone 3 scope changes; sign-off on quarterly access reviews (§10) | _name_ | Standing |
...
| Control 1.2 agent registry exists and is current within the last 30 days | Source-of-truth inventory for §2 reconciliation. | Open the registry (SharePoint List or Dataverse table) and confirm last-modified date. |
| Control 4.6 RCD/RSS posture documented | Required for §4 reconciliation. | Open the Control 4.6 evidence pack; confirm latest DAG snapshot and RCD list. |

### 1.4 Test fixtures gate (canary environment, agents, identities)

Before any production change, the following test fixtures must exist in a canary Power Platform environment. These names appear throughout §2–§8.
...
| DLP policy applied | `dlp-1.14-canary` (built in §3) |
...
| `scope-test-drift-001` | Identical to `scope-test-Z2-001` at baseline; used to deliberately add an out-of-scope knowledge source for §7 / §8 drift testing | Drift detection canary. |
...
| `svc-scope-drift-actor@<tenant>` | Account used to perform the deliberate scope-expansion change in §7.4 drift testing. |

### 1.5 Two-portal precondition

Sessions in **Copilot Studio**, **PPAC**, **Purview portal**, and **Entra admin center** must all be open and authenticated **as the named operator role** (not as Global Admin) before you start §2. The §2 inventory reconciliation depends on data from all four surfaces; do not begin if any portal is unreachable for the named operator.
...
- Operator (applies change): Named AI Administrator / Power Platform Admin / SharePoint Admin / Purview Data Security AI Admin (per the §1 roles matrix).
...
---
## §2 Step 1 — Build the agent grounding-surface inventory
...
### 2.1 Pull the three authoritative agent lists
...
### 2.2 Reconcile the three lists against Control 1.2
...
### 2.3 For each agent, capture the eleven grounding-surface fields
...
| **F7 — Public web grounding** | `Settings → Generative AI → Web search` toggle | Record state — **must be OFF for any Zone 3 NPI agent** (see §5) |
...
| **F11 — Agent identity model** | `Settings → Security → Authentication`; identify whether the agent uses delegated user identity, an authenticated connection, a managed identity, or an Entra Agent ID | Record the identity model — this drives §9 SharePoint group hygiene and §9 Access Reviews |

### 2.4 Document the business justification for each agent ↔ surface pair

For every (agent, surface) pair captured in §2.3, record the business justification on the registry row. The minimum justification fields are:
...
### 2.5 Establish the change-trigger discipline
...
5. Evidence collector captures the post-change inventory snapshot (artifact E3-update) and reconciles against §2.3.

This discipline is what makes the §7 / §8 scope-drift detection meaningful — without an approved baseline, drift detection has nothing to compare against.

### 2.6 Common pitfalls in §2
...
## §3 Step 2 — Power Platform DLP for connector classification
...
!!! note "Boundary with Control 1.4"
    General Power Platform DLP authoring (across all makers, all apps, and all flows) is owned by [Control 1.4 — Advanced Connector Policies](../../../controls/pillar-1-security/1.4-advanced-connector-policies-acp.md). Step §3 here configures only the **agent-relevant** dimension: the connectors used by Copilot Studio agents (knowledge connectors and action connectors) within the environments designated for Zone 2 / Zone 3 agent hosting. If your firm has a tenant-wide DLP policy already authored under Control 1.4, layer the agent-specific policy on top of it (DLP policies stack — most-restrictive wins per environment).
...
### 3.2 Classify the prebuilt connector list per zone
...
| Knowledge source with SharePoint and OneDrive in Copilot Studio | Business | Business | Business (allowlisted sites only — coordinate with Control 4.6 RCD posture in §4) |
| Knowledge source with public websites in Copilot Studio | Non-Business | Non-Business | **Blocked** |
| Knowledge source with documents in Copilot Studio (uploaded files) | Business | Non-Business | **Blocked** unless §6 file-upload posture allows label-gated upload |
...
**The standard connectors used as agent actions** (the exact set varies by environment — pull from §2.3 field F9):
...
### 3.3 Default group for new connectors
...
### 3.4 Scope the policy to environments
...
### 3.6 Sibling Zone 3 policy

Repeat §3.1–§3.5 with these differences for `dlp-1.14-zone-3`:
...
### 3.7 Validate against the test agents

1. Open `scope-test-Z2-001` in the Zone 2 canary environment.
2. Attempt to add a connector that is **Blocked** in `dlp-1.14-zone-2` (e.g., a consumer file-sharing connector from §3.2). Expected: Copilot Studio surfaces a "blocked by data policy" message and the connector cannot be added. Capture the screenshot as artifact E6 (DLP block — negative path).
...
- **Not reconciling against §2 inventory.** Every connector in the §2 F9 field must appear in the Business group of the agent's environment policy. Run the reconciliation after every DLP edit.

---
## §4 Step 3 — SharePoint knowledge-source minimization
...
- The reconciliation that those scopes have the §3.6 posture appropriate to the agent's zone.
...
### 4.2 Open the agent and review the current knowledge sources
...
5. Compare against the §2.3 inventory rows for this agent. Any source present in Copilot Studio but not in the registry is a finding — open a registry intake or remove the source.

### 4.3 Add the smallest-scope SharePoint source
...
### 4.4 Reconcile against the underlying SharePoint site posture

For each SharePoint source on the agent, verify that the corresponding site has the §3.6 posture for the agent's zone:
...
### 4.5 Group hygiene for service-principal-grounded agents

If §2.3 field F11 indicates the agent grounds via an authenticated connection, managed identity, or Entra Agent ID (rather than delegated user identity):
...
5. The group is reviewed quarterly under §12 (Zone 3 monthly).

If the agent grounds via delegated user identity, SG-AgentAccess groups are not the right control — instead, the user's own SharePoint permissions and the §3.6 site posture are what minimize the surface. Document the identity model on the registry row so reviewers do not chase the wrong control.

### 4.6 Positive- and negative-path validation

Using the test fixtures from §1.4:
...
4. **Negative path — image / file upload not present in source.** Upload a synthetic test PDF that contains the string `SCOPE-DRIFT-PROBE-{date}` and ask the agent to summarize it. If file upload is allowed by §6, expected: agent summarizes the uploaded file and cites it as the upload (not as a knowledge source). If file upload is blocked, expected: upload is rejected. Capture transcript as artifact E7-neg-3.

### 4.7 Common pitfalls in §4
...
- **Skipping the §3.6 RCD verification on Zone 3.** RCD is the underlying mechanism that prevents Copilot from indexing content beyond the agent's declared scope when the user has broad organization-level access. Without RCD, scoping the agent alone is insufficient.
- **Confusing OneDrive grounding with SharePoint grounding.** Both are added through the SharePoint knowledge source picker, but OneDrive grounding implicates the OneDrive owner's personal store. Capture OneDrive separately on the registry (§2.3 field F2) and require explicit owner consent for Zone 2 / Zone 3.
...
## §5 Step 4 — Disable public web grounding for Zone 3 NPI agents

**Purpose:** Ensure that no agent classified Zone 3 — and specifically no agent whose data classification (§2.4) is Regulated or NPI — can issue queries to the public web, exfiltrate prompt context to a public search index, or return public-web content as authoritative grounding to the user.

**Operator:** AI Administrator (agent author) under counter-sign by AI Governance Lead
**Estimated time:** 5 minutes per agent
**Outcome:** Every Zone 3 NPI agent has the **Web search** toggle OFF; the §3 DLP policy `dlp-1.14-zone-3` blocks the public-website knowledge connector at the environment level as a defence-in-depth backstop; the change is logged in the audit trail and on the registry.
...
2. In the same panel, also set:
   ...
   - **Image uploads** to **Off** for Regulated / NPI agents unless §6 documents an explicit business justification.
...
2. As the maker, attempt to re-enable **Web search** in the agent. The toggle may be re-enabled in the UI, but at publish time the §3 DLP policy will fail the publish if the public-website knowledge connector is referenced. Capture the failure screenshot as artifact E10.
3. Inspect the audit trail (§9) for the toggle change event under `CopilotInteractionAdminAction` (or the closest current operation name — verify per release).

### 5.5 Common pitfalls in §5

- **Toggling Web search off without the §3 DLP backstop.** A future maker can re-enable the toggle in seconds. Always pair the toggle setting with the environment-level DLP block.
...
## §6 Step 5 — File and image upload posture per zone

**Purpose:** Define and enforce, per zone, whether end users can upload files or images to an agent during a conversation, and whether sensitivity-label gating applies. Per-conversation uploads are a frequently overlooked grounding surface — they bypass §2 inventory because they are user-supplied at runtime.
...
| Audit | Standard `CopilotInteraction` event | Standard event + DSPM for AI activity capture | Standard event + DSPM for AI capture + Sentinel correlation under §9 |
...
### 6.2 Configure the agent toggles

1. In Copilot Studio → agent → **Settings → Generative AI**, set **File uploads** and **Image uploads** per the §6.1 row for the agent's zone.
...
### 6.3 Configure sensitivity-label gating

For Zone 2 and Zone 3 agents, the upload-time label check is enforced through the [Control 1.13 — Sensitive Information Types and Sensitivity Labels](../../../controls/pillar-1-security/1.13-sensitive-information-types-sits-and-pattern-recognition.md) infrastructure and the §8 Purview DLP policy targeting the **Microsoft 365 Copilot** location.

1. Verify that the relevant SITs and labels exist in Purview (Control 1.13 §2 SIT inventory).
2. In §8 below, the DLP policy authored for the Microsoft 365 Copilot location includes a rule that inspects user-supplied content for the SITs and either warns or blocks per the §6.1 row.
...
2. As `svc-scope-pos`, upload `scope-test-confidential.pdf` (a synthetic file containing the string `SCOPE-PROBE-CONFIDENTIAL` and labelled Confidential via Control 1.13) to a Zone 2 agent. Expected: per §6.1 row, either prompt or block; transcript captured as artifact E11.
...
- **Treating upload as out of scope for inventory.** Uploads bypass §2 because they are user-supplied. The compensating control is §6 + §8 — do not skip either.
...
---
## §7 Step 6 — Purview DSPM for AI policies and Activity Explorer

**Purpose:** Layer DSPM-for-AI insider risk and oversharing detections on top of the §2 inventory and §3–§6 minimization, so that scope drift, unsanctioned grounding sources, and risky prompt patterns surface as auditable signals rather than as silent regressions.
...
**Outcome:** DSPM for AI is opted in for the tenant; the recommended detection policies are enabled and tuned per zone; Activity Explorer queries return the agent / user / SIT / sensitivity-label dimensions needed for §9 scope-drift hunts.

!!! note "Boundary with Control 1.6"
    Tenant-wide DSPM-for-AI onboarding (audit ingestion, role assignment, recommendation review) is owned by [Control 1.6 — Microsoft Purview DSPM for AI](../../../controls/pillar-1-security/1.6-microsoft-purview-dspm-for-ai.md). Step §7 here configures the **agent-scope-drift-relevant** detections only. If DSPM for AI has not been onboarded under Control 1.6, stop and complete that prerequisite first.

### 7.1 Verify DSPM for AI prerequisites
...
### 7.3 Tune signal thresholds per zone
...
### 7.4 Activity Explorer queries for §9 scope-drift hunts

Navigate to **Solutions → DSPM for AI → Activity explorer** (the heading evolves; verify per release). The dimensions you need available for §9 are:
...
Pin a saved query named `q-1.14-scope-drift-baseline` filtered to the agents in the §2 inventory; this becomes the baseline view referenced in §9. Capture as artifact E14.

### 7.5 Common pitfalls in §7

- **Skipping the Control 1.6 onboarding prerequisite.** Without DSPM ingestion, the §9 scope-drift hunt has no signal source.
- **Leaving Zone 1 thresholds at Zone 3 sensitivity.** The canary will produce alert fatigue and be ignored.
- **Treating DSPM for AI as a substitute for §8 DLP enforcement.** DSPM detects; DLP enforces. You need both.

---

## §8 Step 7 — Purview DLP for the Microsoft 365 Copilot location

**Purpose:** Enforce sensitivity-aware blocks and warnings on prompts and responses that flow through Copilot Studio agents and Microsoft 365 Copilot, including the per-conversation file uploads from §6. This is **runtime** enforcement complementing §3 (inventory-time enforcement) and §7 (detection).
...
### 8.2 Choose the Custom template
...
### 8.4 Choose the Microsoft 365 Copilot location
...
- Actions: Restrict access — Block.
- User notifications: On — refer the user to the §6 file-upload policy text.

For each rule, click **Save**, then **Next**.

### 8.6 Test mode then enforce
...
## §9 Step 8 — Scope-drift detection via the Microsoft 365 Copilot audit schema

**Purpose:** Build the audit-driven hunt that detects scope drift — agents grounding from sources outside their §2 inventory baseline, prompts probing for out-of-scope content, citations resolving to unsanctioned SharePoint sites, or DLP overrides. This is the detection backbone behind §10 alerting.
...
### 9.1 Confirm the Copilot audit schema is populated
...
### 9.2 Hunt H1 — Out-of-inventory grounding source

**Question:** Did any agent in the §2 inventory cite a SharePoint URL that is **not** in the agent's approved knowledge sources?

1. Run an audit search filtered to `CopilotInteraction` for the last 7 days.
2. Export the results.
3. Join the export against the §2 inventory on `AppId` / agent name; flag rows where the cited SharePoint URL does not match any approved knowledge source URL for that agent.
...
### 9.3 Hunt H2 — Web search re-enabled on a Zone 3 NPI agent
...
### 9.4 Hunt H3 — Connector added that is Blocked under §3
...
2. Filter to the environments scoped under §3.
...
### 9.5 Hunt H4 — DLP override / business-justification override

**Question:** Did any Zone 2 user override a §8 DLP rule using business justification, and was the justification reviewed?
...
### 9.6 Hunt H5 — Sign-in anomaly on agent service principal
...
2. Filter to the SPNs listed in §2 field F11 (where applicable).
...
### 9.7 Sentinel correlation and retention
...
## §10 Step 9 — Alert policies for scope-drift events

**Purpose:** Convert the §9 hunts into named, on-call-routable alert policies so that scope-drift events page the right responder within the SLA defined in the firm's incident response plan.
...
**Outcome:** Named alert policies route §9 hunts H1–H5 to the on-call queue with severity tagged per zone; alert acknowledgement and resolution are evidenced.
...
### 10.1 Create the H1 alert (out-of-inventory grounding)
...
7. **Activity:** Select the underlying `CopilotInteraction` activity matching the H1 hunt logic; where the activity-picker does not natively support the join, configure the alert to fire on the related raw event and use the §9 Sentinel rule for join-based suppression.
...
### 10.2 Create the remaining alerts

Repeat §10.1 for H2 (`alert-1.14-H2-web-search-reenabled`, Severity High), H3 (`alert-1.14-H3-blocked-connector-added`, Severity Medium), H4 (`alert-1.14-H4-dlp-override`, Severity Medium for Zone 2 / High for Zone 3), and H5 (`alert-1.14-H5-spn-signin-anomaly`, Severity High).

### 10.3 Defender XDR cross-workload alert option
...
### 10.5 Common pitfalls in §10

- **Authoring the alert at "Audit log search → Create alert policy".** That breadcrumb is from the legacy Security & Compliance Center and does not reflect current Purview navigation. Use the breadcrumb in the §10 warning admonition.
...
## §11 Step 10 — Microsoft Entra ID Governance Access Reviews for agent identities
...
**Outcome:** Access Review templates exist for (a) SG-AgentAccess groups, (b) agent service principal owners, (c) Copilot Studio author / maker role assignments; reviewers and cadence match the §12 review process.

!!! note "Licensing prerequisite"
    Entra ID Governance Access Reviews require the Entra ID P2 license (or Microsoft Entra ID Governance SKU) on the reviewers and the reviewed principals where the principal is a user. Verify per the licensing matrix in §1.1.

### 11.1 Open Access Reviews
...
### 11.2 Review template AR1 — SG-AgentAccess groups

- **Select what to review:** Teams + Groups.
- **Review scope:** Select groups → pick all groups matching `SG-AgentAccess-*`.
- **Reviewers:** Group owners + the agent owner of record (from the §2 registry) as a fallback reviewer.
...
### 11.3 Review template AR2 — Agent service principal owners

- **Select what to review:** Applications.
- **Review scope:** Service principals tagged with `Tag = Control 1.14 agent` (apply this tag at SPN provisioning time as part of the §2 intake).
- **Reviewers:** the AI Governance Lead + the named agent owner.
- **Recurrence:** Quarterly.
- **Save** and capture as artifact E25.

### 11.4 Review template AR3 — Privileged Copilot Studio role assignments
...
## §12 Step 11 — Quarterly access review process and zone cadence

**Purpose:** Define the operational rhythm — who reviews what, when, with what evidence template — that turns the §11 Access Review machinery into a documented, repeatable governance process auditors can sample.
...
**Outcome:** A reviewed-and-signed quarterly attestation per zone, persisted in the firm's evidence repository, including the §11 review outcomes, the §2 reconciliation deltas, the §9 hunt findings, and any §10 alerts that fired and were resolved.

### 12.1 Cadence matrix

| Zone | §2 inventory reconcile | §11 Access Reviews | §9 hunts run | §10 alerts triaged | §12 attestation |
...
### 12.2 Per-cycle responsibilities

| Step | Owner | Input | Output |

| Pull §2 inventory for the cycle | AI Administrator | Lists A, B, C from §2.1 | Reconciled inventory (artifact for the cycle) |
| Run §11 Access Reviews | Reviewers per AR template | AR1 / AR2 / AR3 instances | Review decisions auto-applied |
| Run §9 hunts | Purview Compliance Admin | Saved searches H1–H5 | Hunt result CSVs |
| Triage §10 alerts | SOC + Compliance Officer | Alert queue | Resolution notes per ticket |
...
3. §2 inventory delta vs. prior cycle (added agents, removed agents, surface changes).
4. §11 review outcomes per template (approved, denied, pending; numeric counts).
5. §9 hunt result summary per H-rule.
6. §10 alert summary (count, severity distribution, mean time to resolve).
...
## §13 Step 12 — Verification handoff
...
- The `scope-test-Z*-001` and `scope-test-drift-001` test fixtures introduced in §1.4 above, with the full prompt set and expected outputs per zone.
...
- The §4.6 positive- / negative-path validation expanded to the full SIT and label matrix.
- The §5 Web search re-enable test and audit lookup.
- The §6 file-upload posture matrix tests.
- The §8 DLP runtime tests (block, warn, override).
- The §9 hunt H1–H5 synthetic trigger procedures.
- The §10 alert-fires-within-SLA verification.
- The evidence-collection checklist mapping each test to the artifact ID in §14.

Hand off to `verification-testing.md` after publishing the agents under §2–§8 and after the §9 / §10 / §11 alerting and review machinery is enabled.

---

## §14 Step 13 — Evidence pack, anti-patterns, FSI incident handling, and troubleshooting handoff

### 14.1 Evidence pack manifest (E1–E27)

The artifacts collected in §1–§13 form the Control 1.14 evidence pack. Each artifact has an ID, owner, format, retention tag, and SHA-256 checksum recorded in the manifest. Manifest generation is automated by `scripts/build-evidence-manifest.ps1` in the FSI-AgentGov-Solutions companion repo; the schema is below.

| ID | Description | Source surface | Owner | Format | Retention tag | Section |

| E1 | License inventory snapshot (Copilot, Entra ID P2 / Governance, Purview SKUs) | Microsoft 365 admin center → Billing → Licenses | AI Administrator | CSV | 7y per Control 6.x | §1.1 |
| E2 | Role assignment snapshot (Entra PIM eligibility + active assignments for the §1.2 roles) | Entra admin center → Roles and admins | Entra Identity Governance Admin | JSON | 7y | §1.2 |
| E3-A | Copilot Studio agent list per environment | Copilot Studio → admin agent inventory | AI Administrator | CSV | 7y | §2.1 |
...
| E8 | Copilot Studio Generative AI panel screenshot per Zone 3 NPI agent (Web search OFF + adjacent toggles) | Copilot Studio | AI Administrator | PNG | 7y | §5.2 |
| E9 | PPAC `dlp-1.14-zone-3` policy showing public-website connector blocked | PPAC | Power Platform Admin | PNG | 7y | §5.3 |
| E10 | Publish-time failure when web-search re-enabled on Zone 3 agent | Copilot Studio | AI Administrator | PNG + transcript | 7y | §5.4 |
| E11 | File-upload sensitivity-label gating transcript (Confidential block / warn) | Copilot Studio canary | AI Administrator | Transcript text | 7y | §6.4 |
| E12 | Cross-user upload non-leakage verification transcript | Copilot Studio canary | AI Administrator | Transcript text | 7y | §6.4 |
| E13 | DSPM for AI policy threshold tuning per zone | Purview portal | Purview Data Security AI Admin | PNG | 7y | §7.3 |
| E14 | DSPM for AI Activity Explorer saved query `q-1.14-scope-drift-baseline` | Purview portal | Purview Data Security AI Admin | KQL or saved-search export | 7y | §7.4 |
| E15 | Active `dlp-1.14-copilot-runtime` policy summary | Purview portal → DLP → Policies | Purview Compliance Admin | PNG + JSON export | 7y | §8.6 |
| E16-pos / E16-neg | Copilot DLP block (NPI) and response-suppression (Highly Confidential) transcripts | Copilot Studio canary | Purview Compliance Admin | Transcript text | 7y | §8.7 |
| E17 | H1 hunt — saved Sentinel KQL or Purview audit query for out-of-inventory grounding | Sentinel / Purview Audit | Purview Compliance Admin | KQL file | 7y | §9.2 |
| E18 | H2 hunt — Web search re-enabled on Zone 3 agent | Sentinel / Purview Audit | Purview Compliance Admin | KQL file | 7y | §9.3 |
| E19 | H3 hunt — Blocked connector added | Sentinel / Purview Audit | Purview Compliance Admin | KQL file | 7y | §9.4 |
| E20 | H4 hunt — DLP override / business-justification override | Sentinel / Purview Audit | Purview Compliance Admin | KQL file | 7y | §9.5 |
| E21 | H5 hunt — agent SPN sign-in anomaly | Sentinel / Purview Audit | Purview Compliance Admin | KQL file | 7y | §9.6 |
| E22 | `alert-1.14-H1-grounding-out-of-inventory` alert policy summary | Purview portal → Policies → Alert policies | Purview Compliance Admin | PNG | 7y | §10.1 |
| E23 | Synthetic-trigger alert validation packet (alert email + SOC ticket + resolution) | Email + ITSM | SOC + Compliance Officer | PDF bundle | 7y | §10.4 |
| E24 | Access Review template AR1 (SG-AgentAccess groups) | Entra → Identity governance → Access reviews | Entra Identity Governance Admin | PNG + JSON | 7y | §11.2 |
| E25 | Access Review template AR2 (agent SPN owners) | Entra | Entra Identity Governance Admin | PNG + JSON | 7y | §11.3 |
| E26 | Access Review template AR3 (privileged Copilot Studio role) | Entra | Entra Identity Governance Admin | PNG + JSON | 7y | §11.4 |
| E27 | Per-cycle attestation packet (signed) | Evidence repository | AI Governance Lead | PDF | 7y | §12.3 |
...
1. **Treating the Copilot Studio agent list as the system of record.** It is one of three lists. Reconcile per §2.
2. **Adding the SharePoint root site collection as a knowledge source instead of the smallest library or folder.** This silently expands scope for every future user query — see §4.3.
3. **Authoring the `dlp-1.14-zone-2` policy in the default Power Platform environment.** The default environment hosts unrelated maker activity tenant-wide; restricting connectors there produces collateral damage. Always scope to a named, governed environment — see §3.4.
4. **Disabling the **Web search** toggle on a Zone 3 agent without the §3 DLP backstop.** The toggle can be re-enabled in seconds; the DLP block at the environment level is the durable control — see §5.3.
5. **Building SG-AgentAccess groups for delegated-identity agents.** SG-AgentAccess groups apply only to authenticated-connection / managed-identity / Entra Agent ID grounded agents — see §4.1 and §4.5.
6. **Authoring alert policies at "Audit log search → Create alert policy".** That path does not exist in the current portal. Use Purview → Policies → Alert policies or Defender XDR → Email & collaboration → Policies & rules — see §10.
7. **Leaving Purview DLP for Microsoft 365 Copilot in Test mode indefinitely.** Test mode produces detections but does not enforce — see §8.6.
8. **Skipping the §11 SPN tagging step.** Without the `Control 1.14 agent` tag, AR2 cannot scope. Tag at SPN provisioning.
9. **Hard-coding Microsoft audit record-type names in saved searches.** They evolve. Re-validate every quarter — see §9.8.
10. **Treating DSPM for AI as authoritative for the agent inventory.** DSPM only sees agents that produced activity in the lookback window — see §2.6.
11. **Assuming Power Platform DLP narrows OAuth scopes.** It does not — that's Control 1.18's job — see §3 callout.
12. **Compressing the §12 review cycle when the AI Governance Lead is on PTO.** Build a deputy into the cycle calendar; cycle drift is itself a finding.

### 14.3 FSI incident handling — when scope drift becomes a notifiable event
...
For each scope-drift incident, the §12 attestation cycle should reference the incident ticket and the disposition (notified / not notifiable / under review by Legal) so that the audit trail of the decision is preserved.

### 14.4 Troubleshooting handoff

For symptom-driven recovery — "the agent stopped citing the in-scope library", "DSPM for AI shows zero rows for an agent we know is in use", "the §10 alert fired but the SOC ticket has no payload", "users are seeing a `dlp-1.14-copilot-runtime` block that we did not author" — see the companion playbook:

- [`troubleshooting.md`](troubleshooting.md)

That playbook indexes symptoms to the section in this walkthrough that owns the recovery procedure and to the artifact in §14.1 that demonstrates the recovery worked.
...
- Adjacent and depended-upon controls:
  - [Control 1.2 — Agent Registry](../../../controls/pillar-1-security/1.2-agent-registry-and-integrated-apps-management.md) (system of record for the §2 inventory)
  ...
  - [Control 1.6 — Microsoft Purview DSPM for AI](../../../controls/pillar-1-security/1.6-microsoft-purview-dspm-for-ai.md) (tenant onboarding for §7)
  - [Control 1.13 — Sensitive Information Types and Sensitivity Labels](../../../controls/pillar-1-security/1.13-sensitive-information-types-sits-and-pattern-recognition.md) (SIT and label inventory powering §6 and §8)
  - [Control 1.17 — Endpoint and Network Controls](../../../controls/pillar-1-security/1.17-endpoint-data-loss-prevention-endpoint-dlp.md) (endpoint DLP and Conditional Access for §4.5 SPN sign-in and §6 endpoint upload)
  ...
  - [Control 4.6 — Knowledge Source Restrictions and SharePoint Hygiene](../../../controls/pillar-4-sharepoint/4.6-grounding-scope-governance.md) (RCD / RSS / DAG underneath §4)

- Companion playbooks for Control 1.14:
  - [`powershell-setup.md`](powershell-setup.md) — automation of §2 inventory and §9 hunt scheduling
  - [`verification-testing.md`](verification-testing.md) — synthetic test suite per §13
  - [`troubleshooting.md`](troubleshooting.md) — symptom-to-section index per §14.4
