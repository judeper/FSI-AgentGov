...
!!! danger "READ FIRST — Scope and the central FSI failure mode"
    ...
    **The 13 surfaces (every one is configured in this playbook):**
    ...
    |---|---|---|
    | 1 | SharePoint Online sites | Purview DLP location `SharePoint sites` (§4) |
    | 2 | OneDrive for Business | Purview DLP location `OneDrive accounts` (§4) |
    | 3 | Exchange Online (mail in transit) | Purview DLP location `Exchange email` (§4) |
    | 4 | Teams chat & channel messages (incl. private channels) | Purview DLP location `Teams chat and channel messages` — extend default policy beyond credit cards (§5) |
    ...
    | 12 | Defender for Cloud Apps file policies (non-Microsoft SaaS) | Defender for Cloud Apps `Policies > File policies` (§10) |
    | 13a | Power BI / Fabric workspaces | Purview DLP location `Fabric and Power BI workspaces` (§11) |
    | 13b | On-premises file shares / on-prem SharePoint | Purview Information Protection scanner (§12) |
...
| Adaptive Protection (DLP risk-tiered enforcement) | Insider Risk Management entitlement (E5 Compliance / Purview Suite) — **Commercial only**; see §13 |
...
- Onboarded devices for the Endpoint DLP location (§6)
...
## §2 — Propagation banner (read once, applies to every step)
...
## §3 — Sensitivity labels: create, publish, and understand the boundaries (do this first)
...
### 2.1 Create labels (Purview portal)

1. Sign in to your tenant cloud's Purview portal.
...
### 2.2 Publish at least one label policy
...
### 2.3 Container labels vs. file labels

Labels applied to Microsoft 365 Groups, Teams, and SharePoint sites govern **container-level settings** (privacy, external sharing, device access, default link types). Items inside the container do **not** inherit the container label. File-level labeling requires a published file/email label policy, an auto-labeling policy (§3.4), or manual application.

### 2.4 Auto-labeling — locations and limits
...
### 2.5 Conditional Access session controls for label-restricted access
...
## §4 — Surfaces 1–3: SharePoint sites, OneDrive accounts, Exchange email

### 3.1 Create the baseline FSI Purview DLP policy
...
| `R4-Sensitivity-Label` | Sensitivity labels: Confidential, Highly Confidential | Restrict access / Block external sharing | Acts on items already labeled per §3 |
...
### 3.3 Move to enforcement after the simulation pass
...
## §5 — Surface 4: Teams chat & channel messages (extend default policy beyond credit cards)
...
5. Create rules referencing the same SITs, EDM classifiers, trainable classifiers, and sensitivity labels as §4.2. The Teams location supports both message-content and shared-file matching.
...
## §6 — Surface 5: Endpoint DLP (Devices) — Win 10/11, Win Server 2019/2022, last 3 macOS versions
...
## §7 — Surfaces 6 & 7: Microsoft 365 Copilot & Copilot Chat (block by label GA; block by SIT preview)
...
### 6.2 Create the Copilot DLP policy
...
### 6.3 Same-rule restriction — author **two separate rules** in the same policy
...
5. Configure policy tips, override-with-justification, and incident report (§4.2 conventions).
6. Save the rule.
...
### 6.4 Block-by-label scope — communicate the boundaries
...
### 6.5 Mode and propagation

- Save the policy in **Test with notifications** mode.
- Wait the **up to 4-hour** propagation window.
- Run the deterministic test in §14 before flipping to **Turn it on right away**.

---

## §8 — Surfaces 8 & 9: Power Platform data policies (Copilot Studio agents) and HTTP endpoint filtering (preview)
...
### 7.2 Surface 8 — Classify connectors (Business / Non-Business / Blocked)
...
### 7.3 Surface 9 — HTTP endpoint filtering (preview)
...
### 7.4 PAYG implications for non-Microsoft channels
...
## §9 — Surfaces 10 & 11: Edge for Business unmanaged AI (preview) and Network DLP for unmanaged AI (preview)
...
### 8.1 Surface 10 — Edge for Business unmanaged-AI inline protection (preview)
...
4. In your Endpoint DLP rules (§6), add an action targeting **Browser activities → Upload to a restricted website** for matches on FSI SITs, EDM, or labels.
5. Configure policy tips with required justification on user override; route override telemetry to Purview Audit and Sentinel as in §4.2.

### 8.2 Surface 11 — Network DLP for unmanaged AI (preview)
...
## §10 — Surface 12: Defender for Cloud Apps file policies (non-Microsoft SaaS)
...
5. **Filters:** match by file type, content inspection method (DLP using Microsoft Data Classification Service), and the same SITs / EDM / labels referenced in §4.2.
...
Reference connected-app coverage and supported governance actions in the Defender for Cloud Apps documentation linked in §16.

---

## §11 — Surface 13a: Power BI / Fabric workspaces
...
## §12 — Surface 13b: On-premises file shares / on-prem SharePoint (Purview Information Protection scanner)
...
3. Configure the scanner to use the **published label policies** from §3 and any **auto-labeling policies** from §3.4.
...
## §13 — Adaptive Protection (Commercial only — IRM-dependent)
...
3. In your DLP rules from §4/§5/§7, add a condition referencing **User's risk level for adaptive protection** = Elevated / Moderate as appropriate.
4. Save and validate behavior in a non-production tenant first.

---

## §14 — Verification: deterministic test, DLP Alerts, Activity Explorer, Audit, Sentinel
...
3. At a recorded UTC timestamp, have them perform each of the following and capture the policy-tip text the user sees (or absence thereof):
    - Paste a **synthetic** SSN from a controlled test set into a Copilot prompt → expect Rule A match (§7).
    - Reference a SharePoint file labeled **Highly Confidential** in a Copilot prompt → expect Rule B match (§7).
    - Upload a synthetic-PII file to OneDrive → expect §4 match.
    - Send a Teams message containing a synthetic ABA routing number → expect §5 match.
    - Attempt to copy a labeled file to a USB drive on an onboarded Windows endpoint → expect §6 match.
    - Attempt to paste a synthetic SSN into `chat.openai.com` from an onboarded device → expect §9 match.
    - Upload a labeled file to a connected Box/Dropbox Business app → expect §10 match.

### 13.2 Confirm in DLP Alerts
...
### 13.3 Confirm in Activity Explorer
...
### 13.4 Confirm telemetry destination
...
### 13.5 Reg S-P 2024 incident-response readiness
...
## §15 — Zone-specific configuration summary
...
Use this as the §14 sign-off checklist before any Zone 3 enforcement decision.
...
| Combined SIT + label conditions in one Copilot rule | Purview UI rejects save | Split into two rules in the same policy (§7.3) |
...
| Devices not onboarded | Endpoint DLP rules silently produce zero matches | Onboard devices via §6.1; confirm **Active** status |
| Default Teams policy left at credit-card-only | SSNs, ABA routing, MNPI in Teams chat not detected | Extend policy per §5 |
| Edge config policy missing for Chrome/Firefox users | §9 unmanaged-AI events missing for those users | Push Purview browser extension via Intune |
...
| Blocked-by-label perceived as fully removing Copilot access | Citations still appear in Copilot responses | Communicate documented scope to stakeholders (§7.4) |
...
## §20 — Authoritative Microsoft Learn references
