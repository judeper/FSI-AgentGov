...
| 4 | **Content moderation Lowest / Low / Medium / High / Highest** (per-agent; 5 positions; maps to Azure AI Content Safety severity thresholds across Hate / Sexual / Violence / Self-Harm) | GA | Copilot Studio per-agent → Settings → Generative AI → Content moderation | §4d |
...
| Copilot Studio **per-agent content moderation Lowest / Low / Medium / High / Highest** (5 positions; Azure AI Content Safety severity thresholds) | GA | GA (verify) | GA (verify) | **Verify** | [security-content-moderation](https://learn.microsoft.com/en-us/microsoft-copilot-studio/knowledge-copilot-studio#content-moderation); [content-safety/concepts/harm-categories](https://learn.microsoft.com/en-us/azure/ai-services/content-safety/concepts/harm-categories) |
