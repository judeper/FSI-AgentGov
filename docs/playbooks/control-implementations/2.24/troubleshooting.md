...
**Scope:** Diagnose, contain, and remediate failures of the agent feature catalog, three-surface enablement model (tenant Copilot hub → environment → per-agent), zone-based exposure, MCP/connector approval gates, voice/image/preview controls, and the cascading dependencies into MRM (2.6) and supervision (2.12). FINRA Rule 3110 / 3120, SEC Rule 17a-4(f), SEC Reg SCI (242.1001–242.1007), SOX §404 ITGC, GLBA Safeguards Rule, OCC Heightened Standards, **Federal Reserve SR 26-2 (formerly SR 11-7) model risk management**, and CFTC Rule 1.31.
...
Symptom reported
  ...
  │   same data flow?                                 → §9 DLP-MISMATCH
  ├─► Voice / image / multimodal feature enabled
  │   without comms-compliance recording path?        → §10 VOICE-IMAGE-GAP
  ├─► Feature change made without MRM change record
  │   (Fed SR 26-2 — formerly SR 11-7) or without model re-validation?       → §11 MRM-CASCADE-BROKEN
  └─► Supervisors unaware that a feature changed,
      WSP / supervision queue not updated?            → §12 SUPERVISOR-UNAWARE
```
...
| **Sev 2** | Control degraded but contained; cross-zone drift; MRM/supervision cascade broken but no client impact yet. | Catalog drift on a Z2 environment; MRM change record missing for a recent toggle. | 4 hrs — AI Administrator + Security Architect; notify AI Governance Lead. |
...
> **Stop-the-press rule.** If an examiner inquiry, internal-audit hold, or legal-hold notice is open against the affected agent, environment, or feature, **do not** flip toggles, delete agents, or rotate credentials before evidence is captured per §16 and §X. SEC Rule 17a-4(f) and CFTC Rule 1.31 require WORM-grade preservation of the configuration state at the time of the inquiry; remediation that overwrites that state is itself a finding.
...
These helpers wrap the underlying Microsoft Graph, Power Platform, Copilot, and Purview endpoints. Definitions live in [`./powershell-setup.md`](./powershell-setup.md).
...
| **GQ-03** | `GET /solutions/virtualAgents/bots?$expand=connectors` | Copilot Studio agents and their connector graph. | §5, §9 |
...
| **GQ-07** | `GET /informationProtection/policy/labels` | Sensitivity labels referenced by per-agent feature gates. | §9 |
| **GQ-08** | `GET /security/auditLog/queries` | Saved queries used by §16 evidence collection. | §16, §X |
...
| **KQL-06** | `CommunicationComplianceEvents` | Coverage gap: agents emitting voice/image without a matching CC policy (§9). |
...
| **E-03** | Toggle provenance log | CSV | 7 yrs | §2, §3, §9 |
| **E-04** | MCP / connector inventory | JSON | 7 yrs | §5, §9 |
| **E-05** | Reverse-flow workflow ticket bundle | PDF + JSON | 7 yrs | §7 |
| **E-06** | DLP-vs-toggle alignment report | CSV | 7 yrs | §9 |
| **E-07** | Multimodal coverage report | CSV | 7 yrs | §10 |
| **E-08** | Post-incident attestation (signed) | PDF + JSON manifest | 7 yrs | §X |
| **E-09** | Examiner / audit response bundle | PDF + manifest, WORM | Per legal-hold scope | §0.4, §13 (RB-01) |

> **Non-substitution.** Evidence in E-01 … E-09 supports examiner response and post-incident attestation. It does **not** satisfy Fed SR 26-2 (formerly SR 11-7) §V (model documentation) or FINRA 3110 supervisory-procedure documentation; those are produced under Controls 2.6 and 2.12 respectively, and must be cross-linked from E-08.
...
- §2 TOGGLE-ABSENT (sometimes a "missing toggle" is an undetected drift).
...
- Add a daily `Get-Agt224SurfaceMatrix` drift report to the §20 evidence pipeline.
...
| Cause | Evidence | Resolution |
|---|---|---|
| ACP (Control 1.4) policy not applied to the environment. | `Get-Agt224DlpAlignment` flags missing policy. | Apply ACP; rerun §9. |
...
- Cascade: confirm ACP (Control 1.4) policy denies *all* unapproved connectors at the environment boundary, not only at the per-agent surface.

### §5.5 Verification

- [ ] `Get-Agt224McpInventory` shows zero `ApprovalStatus = Unknown` rows.
- [ ] ACP policy active on every environment in scope.
- [ ] If Sev 1 was raised: Control 3.4 incident closed; E-08 attestation filed.
...
| Cause | Evidence | Resolution |
|---|---|---|
| GA promotion changed default behaviour. | GQ-05 message. | Re-validate per Control 2.6 (Fed SR 26-2 (formerly SR 11-7) model change). Update catalog. Communicate to supervisors (§9). |
...
- Cascade to §9 SUPERVISOR-UNAWARE.
...
- For emergency disables (Sev 1): use `Set-Agt224EmergencyDisable` with both AI Governance Lead and Compliance Officer co-sign captured in E-06. Backfill the workflow record within 24 hrs.
...
- §4 SURFACE-CONFUSION.
- §11 MRM-CASCADE-BROKEN.

---

## §9 DLP-MISMATCH — DLP / ACP policy and feature toggle disagree on the same data flow
...
1. `Get-Agt224DlpAlignment -OutFile evidence/2.24/E-06_dlp.csv`.
...
- Add `Get-Agt224DlpAlignment` to the §20 evidence pipeline.
...
## §10 VOICE-IMAGE-GAP — Multimodal feature enabled without comms-compliance recording path
...
1. `Get-Agt224MultimodalSurfaces -OutFile evidence/2.24/E-06_multimodal.csv`.
...
- Add multimodal coverage to the §20 evidence pipeline.
...
## §11 MRM-CASCADE-BROKEN — Feature change made without an MRM change record (SR 26-2)
...
- Cascade to §9 (supervisor re-notification once MRM record is closed).
...
## §12 SUPERVISOR-UNAWARE — Feature changed; supervisors / WSP not updated
...
| New surface (e.g. voice) live but no supervision sampling defined. | Audit. | See §9. |
...
## §13 RB-01 — Examiner / Regulator Inquiry on Agent Feature State
...
2. **T+1 hr — Capture point-in-time state.** Run `Get-Agt224FeatureSnapshot -IncludeAuditTrail` against the named agent / environment / tenant; commit to immutable storage as **E-06**.
...
**Cross-references.** §0.4, §20, [Control 2.25](../../../controls/pillar-2-management/2.25-agent-365-admin-center-governance-console.md), [Control 3.4](../../../controls/pillar-3-reporting/3.4-incident-reporting-and-root-cause-analysis.md).

---

## §14 RB-02 — Zero-Day Microsoft Service-Side Default Flip
...
4. **T+2 hr — Cascade.** If MRM-in-scope: open a 2.6 record. If supervision-in-scope: notify supervisors per §9. If multimodal: re-check §10 coverage.
5. **T+24 hr — Post-flip attestation.** File E-08 with rationale, decision, and cascade evidence.
...
**Cross-references.** §6 RETROACTIVE-PREVIEW, §11 MRM-CASCADE-BROKEN, §12 SUPERVISOR-UNAWARE.

---

## §15 RB-03 — Deprecated Preview Feature Bound to a Production Agent
...
5. **T+deprecation — Post-removal verification.** Confirm no agent retains a binding; close E-08.
...
## §16 RB-04 — SOX §404 Quarterly Attestation Deadline
...
5. **T-0 — Sign-off.** AI Governance Lead + Compliance Officer + Internal Audit sign E-08.

**Evidence bundle.** E-01 (start of quarter, end of quarter, weekly), E-02, E-03, MRM cascade report, supervisor cascade report, signed E-08.
...
## §17 RB-05 — Non-Compliant Z3 Agent Detected in Production
...
2. **T+15 min — Evidence first (per §0.4).** Capture E-01 + the relevant evidence codes (E-04 for MCP, E-06 for DLP, E-06 for multimodal) **before** any change.
...
6. **T+5 days — Post-incident review.** File E-06; update catalog, workflow, and detection to prevent recurrence.

**Cross-references.** §5, §8, §9, §9, [Control 3.4](../../../controls/pillar-3-reporting/3.4-incident-reporting-and-root-cause-analysis.md).

---

## §18 RB-06 — SEC Reg SCI Notification Path for Critical-System-Affecting Feature Change
...
2. **T+immediate — Preserve.** Apply §0.4 stop-the-press; capture E-01 + E-06 to immutable storage.
...
**Evidence bundle.** E-01 + E-06 + technical narrative + every cross-control artefact relevant to the affected SCI system.

**Cross-references.** [Control 3.4](../../../controls/pillar-3-reporting/3.4-incident-reporting-and-root-cause-analysis.md), §13 RB-01, §17 RB-05.
...
| **Microsoft Support — Tier 2 / Premier (Unified Support)** | Service-side default flip, RBAC defect, Graph endpoint defect. | AI Administrator + AI Governance Lead. | Premier ticket; reference incident ID. |
...
> **Role usage rule.** AI Administrator is the standing day-to-day role for all activity in this playbook. Entra Global Admin is **exceptional only** — used solely for break-glass actions where AI Administrator scope is insufficient, and every such use must appear in E-06.

---

## §20 Evidence Collection During Incident
...
4. **E-06** — `Get-Agt224DlpAlignment` for every affected data flow.
5. **E-06** — `Get-Agt224MultimodalSurfaces` for every affected agent.
...
## §21 Cross-References (Consolidated)
...
- [`../../_shared/powershell-baseline.md`](../../_shared/powershell-baseline.md) — helper-cmdlet baseline.
...
- [ ] All evidence in §16 captured to immutable storage with manifest.
...
- [ ] E-08 attestation signed (template below).

### §X.2 E-06 Attestation Template

```markdown
# Control 2.24 — Post-Incident Attestation (E-06)

**Incident ID:**            INC-YYYY-MMDD-NNNN
**Pillar(s) engaged:**      §N (NAME), §N (NAME)
**Runbook(s) executed:**    RB-NN
**Severity (final):**       Sev 1 | Sev 2 | Sev 3
**Tenant / Cloud:**         <tenantId> / Commercial
...
| AI Governance Lead | | |
| Compliance Officer | | |
| Security Architect (if §5/§8/§9) | | |
| Senior Officer for Reg SCI (if RB-06) | | |
```

### §X.3 Lessons-learned cadence

- AI Governance Lead reviews all closed E-06 records monthly; cross-cuts root causes across pillars.
...
- Annual: feed into the catalog re-baseline.
