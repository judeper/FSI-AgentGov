...
function Request-Feat224PimActivation {
    ...
    if ($Role -eq 'GlobalAdministrator') {
        Write-Warning "Global Administrator activation should be exceptional. Confirm a CAB ticket beyond '$JustificationTicket' is on file (Control 2.24 v1.3.3)."
    }
...
| [2.6](../../../controls/pillar-2-management/2.6-model-risk-management-alignment-with-occ-2011-12-sr-11-7.md) | Model Risk Management Alignment (OCC 2011-12 / SR 11-7) | Catalog rows with `RiskRating in {High, Critical}` require an MRM Review ID |
