...
>
> **Availability hedging (June 2026 rollout).** Microsoft's Copilot Credits consumption-billing model becomes the operative metering path for several agent surfaces from **June 16, 2026** — scheduled per Microsoft Learn partner announcements and Microsoft 365 roadmap references — the same day the Work IQ API moves to Copilot-Credits consumption billing. Tenant ceilings (PAYG **50** / credit **10**), per-feature **credit rates**, and the **25,000-credits-per-month** prepaid capacity pack are confirmed against Learn sources; dollar pricing must be confirmed from current Microsoft commercial licensing or commerce sources before relying on figures as examiner evidence. Whether a public **write API** for credit-policy / per-agent-cap enforcement exists is resolved: as of June 2026 there is **no public write API** (per-agent caps are Power Platform admin-center UI-managed, per Microsoft Learn, "manage-copilot-studio-copilot-credits-capacity"), so a cap **degrades to detect-and-alert** rather than a hard-stop; should Microsoft ship one, caps can be upgraded to a hard-stop.
>
...
6. **Re-confirm pricing and check the volatile surfaces post-switch** — the tenant ceilings (50/10), per-feature credit rates, and 25,000-credit capacity pack were verified against Learn sources, but dollar pricing is time-sensitive and must be confirmed from current Microsoft commercial licensing or commerce sources; also re-check the current portal/PPAC blade labels and the per-tenant `COPILOT` service-plan name, which may shift with the switch. (Footnotes 6 & 7 were verified June 2026.)
...
*This troubleshooting guide supports compliance with the cited regulations; it does not, on its own, satisfy any of them. The June 16 2026 Work IQ switch, Licensing Guide footnotes 6 & 7, the absence of a public cap-enforcement write API, and the tenant ceilings (50/10), per-feature credit rates, and 25,000-credit capacity pack were verified against Learn sources; dollar pricing is time-sensitive and must be confirmed from current Microsoft commercial licensing or commerce sources before treating any procedure as examiner evidence.*
