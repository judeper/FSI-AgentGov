...
Pricing context: Microsoft Learn confirms per-feature credit rates and that prepaid capacity packs provide **25,000 credits per month per pack**. Dollar pricing (for example, per-credit or per-pack monthly price) is commerce-sensitive and not restated in this playbook; confirm current pricing from Microsoft commercial licensing or commerce sources before producing examiner evidence. PAYG consumption meters against an Azure subscription with **budget alerts only — not a hard-stop**.
...
| **Quarterly** | Re-confirm pricing and ceilings against current Microsoft licensing documentation as they change (per-feature credit rates, 25,000-credit capacity pack, tenant ceilings 50/10 were verified against Learn sources — dollar pricing must come from current Microsoft commercial licensing or commerce sources), re-check current portal/PPAC blade labels and the per-tenant `COPILOT` service-plan name, and watch for a public cap-enforcement write API should Microsoft ship one (caps remain detect-and-alert until then) | AI Governance Lead | §0, §4, §7, §8, §9 |
...
*Updated: September 2026 | Version: v1.0 | UI Verification Status: Needs Review — the June 16 2026 Work IQ switch, Licensing Guide footnotes 6 & 7, the absence of a public cap/credit-policy write API, and the tenant ceilings (50/10), per-feature credit rates, and 25,000-credit capacity pack were verified against Learn sources; dollar pricing is time-sensitive and must be confirmed against current Microsoft commercial licensing or commerce sources before relying on enforcement.*
