...
>
> **Upstream dependency.** The agent dimension (`createdIn`) and usage tier (`configuredTier`) are produced by the sibling solutions `copilot-agent-inventory` (Azure Resource Graph `PowerPlatformResources`) and `work-iq-usage-detection`. Until those are catalog-registered, the entitlement engine operates on an assembled/fixture input set via `-InputPath`. Work IQ general availability and the move of the Work IQ API to Copilot Credits consumption billing are dated **June 16, 2026** 🔎 — verify before wiring enforcement.
>
> **Write-API posture.** A public **write API for credit-policy CRUD, per-agent cap enforcement, and hard-stop** may not yet exist 🔎. Where it is absent, enforcement **degrades to detect-and-alert** — the cap is recorded and breaches are surfaced; it is **not** represented as a hard-stop. The live credit-policy **read** API is likewise unproven and falls back to the reconciled Dataverse store.
...
```powershell
# Default posture: zero-rating RESOLVED per the June 2026 Licensing Guide (footnotes 6 & 7 🔎).
...
!!! warning "Entitlement-decision caveat"
    The contract resolves a single auditable decision per `(agent, user)`; it governs the **entitlement decision** and does not, on its own, satisfy any regulation. The zero-rating default reflects the June 2026 Microsoft Copilot Studio Licensing Guide (footnotes 6 & 7) 🔎 (the zero-rating substance is confirmed on [Microsoft Learn — Copilot Studio billing and licensing](https://learn.microsoft.com/en-us/microsoft-copilot-studio/billing-licensing); the footnote numbering is unverified) — a Copilot-licensed user on a Microsoft 365 surface under their own identity is **included** in the Microsoft 365 Copilot User SL at no additional charge, subject to fair-usage limits. The generative-answer-with-tenant-grounding and beyond-fair-use refinements affect credit **cost**, not this allow/deny, and should be confirmed per tenant.
...
**Because there is no public write API to set an enforceable credit cap or hard spend-stop 🔎 (the Billing Policy write API has no credit-cap field, and per-agent caps have no documented write API), automation degrades to detect-and-alert.** The helper records the cap and surfaces a month-to-date breach; it does **not** apply a programmatic hard-stop, and it does **not** report the cap as a hard-stop.
...
| **Quarterly** | Re-verify the 🔎 items (June 16 2026 switch, footnotes 6 & 7, credit rates, ceilings, write-API existence) against Microsoft documentation | AI Governance Lead | §0, §4, §7, §8, §9 |
...
*Updated: June 2026 | Version: v1.0 | UI Verification Status: Needs Review — verify the 🔎-flagged Microsoft licensing facts (June 16 2026 switch, footnotes 6 & 7, per-feature credit rates, $0.01/credit, 25,000-credit pack, tenant ceilings 50/10, and the cap/credit-policy write-API existence) before relying on enforcement.*
