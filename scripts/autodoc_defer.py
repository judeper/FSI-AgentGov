...
def pending_path(url: str, base_dir: str | Path) -> Path:
    """Return the deterministic pending blob path for a Learn URL."""
...
def is_already_pending(url: str, new_hash: str, base_dir: str | Path) -> bool:
    ...
    pending = load_pending(pending_path(url, base_dir))
