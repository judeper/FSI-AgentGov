...
4. Enforce site-dependent ordering:
   ...
   4. `python scripts/verify_doc_links.py site --json _broken-links.json`
5. Run current documentation/drift gates when applicable to changed paths:
...
6. Run code/test gates conditionally on relevant changes:
...
7. Keep these supplemental checks conditional and explicitly label them as non-authoritative CI supplements:
...
8. If sibling `FSI-AgentGov-Solutions` exists, conditionally parse `*/src/*.json` with `json.loads()` and report parse failures only.
