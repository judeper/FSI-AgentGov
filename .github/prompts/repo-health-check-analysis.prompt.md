...
<objective>
Analyze repository structural integrity and content quality through cross-file reasoning that automated scripts cannot perform. Do NOT re-check anything covered by validation scripts (language rules, control section structure/footer checks, built-site internal/anchor link validation). Report issues only, one line per finding.
</objective>
...
- Built-site internal + anchor link coverage (authoritative) → `verify_doc_links.py site --json _broken-links.json`
- Source-markdown anchor heuristic (supplemental, non-authoritative) → `validate_docs_anchors.py`
