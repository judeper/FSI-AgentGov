...
> **For each required check name whose real workflow has a `paths:` filter, add a "shim" workflow with the same `name:` triggered via `paths-ignore:` that mirrors the real workflow's `paths:` list.** The shim's job body is a one-line `echo` reporting success — there is no real work to do, because by construction the PR's diff is outside the scope the real workflow guards. The check name reports success, branch protection is satisfied, the bot PR merges. Real PRs continue to get the full real workflow.
>
> **Trigger semantics caveat.** `paths-ignore` triggers a workflow unless **all** changed files match the ignore list; `paths` triggers when **any** changed file matches. For pure bot PRs (every file is in the shim's ignore list) the two are mutually exclusive and only the shim fires. For mixed PRs that touch both ignored and non-ignored paths, **both the shim and the real workflow fire**. That's harmless — branch protection just sees two green check-runs with the same name — but it means the shim's `paths-ignore:` and the real workflow's `paths:` are not literally inverse partitions in general. When you change a real workflow's `paths:`, you should still mirror the change in the shim's `paths-ignore:` in the same commit so the no-op case (bot PRs) stays single-firing.
