...
**Auto-merge kill-switches (repo variables):** `LEARN_STATE_AUTOMERGE`, `REGULATORY_STATE_AUTOMERGE`, and `DEPENDABOT_AUTOMERGE` each gate one unattended auto-merge stream (set to `false`/unset to disable); `AUTODOC_ENABLED` gates the autodoc pipeline. All four are currently `true`. Branch protection on `main` requires 11 named checks and `strict` (up-to-date) merges, so every auto-merge still waits on green required checks plus a rebase — which also prevents the daily Learn / weekly Regulatory state-only PRs from clobbering each other's `data/monitor-state.json`.
