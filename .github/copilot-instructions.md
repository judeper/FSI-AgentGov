...
| `learn-monitor.yml`, `regulatory-monitoring.yml` | Schedule | Source-of-truth drift detection. Learn state PRs use the fail-closed, `add-paths`-restricted auto-merge path gated by `LEARN_STATE_AUTOMERGE`. Regulatory mutation and state-PR creation are default-branch-only; feature/PR refs use a separate read-only (`contents: read`, `persist-credentials: false`) validation job and never receive App/write credentials. The trusted regulatory job requests write permissions and its App token only after checkout, monitor success, and captured default-branch/state SHA CAS. Scheduled mutation requires valid regulatory source sections and `last_run`; missing/corrupt state fails closed, while baseline initialization is local-only and explicitly approved. Regulatory exit-0 runs create a clearly identified maintenance PR only when persisted state is dirty; clean runs stay silent and exit >=2 never creates a state PR. The workflow verifies the exact created head, file set, and generated blobs, then leaves the newest PR for the external guarded sweep or human merge gate. `REGULATORY_STATE_AUTOMERGE` does not authorize merging; only after successful verification does it permit consolidation of older non-`needs-review` monitor PRs. A failed or skipped verification leaves older PRs untouched, and CRITICAL/HIGH PRs retain `needs-review` and are never consolidated. |
...
**Automation switches (repo variables):** `LEARN_STATE_AUTOMERGE` and `DEPENDABOT_AUTOMERGE` gate unattended auto-merge streams (set to `false`/unset to disable). `REGULATORY_STATE_AUTOMERGE` is legacy-named and gates only superseded-PR consolidation after verified regulatory handoff; regulatory merging remains external/human guarded. `AUTODOC_ENABLED` gates the autodoc pipeline. Autodoc target-native/agreement-ledger auto-merge is disabled and observational; OceanSquad is its sole merge owner. Remote branch/ruleset state—not a committed whole-document branch-protection payload—is the source of truth for required checks. The dedicated dependency artifact requirement remains BLOCKED until its expected-source GitHub App ruleset is provisioned and read back. `mkdocs-strict` already provides deterministic internal-link and anchor validation; external-link health remains non-required.
