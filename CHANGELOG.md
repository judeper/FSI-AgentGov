...
### Added
- **Autonomous Learn Monitor documentation pipeline — workflow layer (Stage 1, opt-in).** Wires the merged scaffolding into a runnable, fail-closed, human-merge-gated pipeline, **off by default** (`AUTODOC_ENABLED` repo variable): `learn-autodoc-route.yml` + `scripts/autodoc_route.py` (classify the latest Learn report → open autodraft `squad:copilot` issues or human `escalate` issues with a machine-readable authoring contract, fingerprint-idempotent via `data/autodoc-ledger.json`); `autodoc-verify.yml` + `scripts/autodoc_verify_gate.py` (shim-aware required check running the deterministic + cross-vendor LLM verifiers, hardened `pull_request_target`, fails closed); `autodoc-fix-retry.yml` + `scripts/autodoc_retry.py` (bounded retry then human escalation). Reuses the existing `squad-issue-assign.yml` for Copilot coding-agent assignment. Maintainer operations + provisioning in `.github/AUTODOC-RUNBOOK.md`; published description updated in `docs/reference/learn-monitor-ai-enhancement.md`. The F5 baseline-deferral was deliberately deferred (silent-drop already mitigated by issue tracking; live-monitor risk). Each piece independently reviewed (cross-family owl-mode) before merge.
