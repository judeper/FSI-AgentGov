...
- **Unattended auto-merge streams + Playwright pin guard** (`dependabot-automerge.yml`, the auto-merge steps in `learn-monitor.yml` / `regulatory-monitoring.yml`, `scripts/check_playwright_pin_drift.py`): three kill-switch repo variables gate three unattended `gh pr merge --auto` streams — `DEPENDABOT_AUTOMERGE` (minor/patch + security; majors → `needs-review`), `LEARN_STATE_AUTOMERGE`, and `REGULATORY_STATE_AUTOMERGE` (state-only monitor PRs; regulatory only when **0 CRITICAL/HIGH**, via a fail-closed `automerge_eligible` signal, and consolidation never closes a `needs-review` PR). Every merge still waits on the 11 required checks + `strict` (up-to-date) rebase, which also stops the daily-Learn / weekly-Regulatory PRs from clobbering each other's `data/monitor-state.json`. Any workflow pinning a Playwright `container.image:` must keep its tag equal to `@playwright/test` in `package.json` — the required `manifest / index / nav drift` job runs `check_playwright_pin_drift.py` to enforce it (the #583 `e2e-full` skew). `e2e-full` is deliberately **not** a required check (label-gated off PRs to avoid the deadlock); `e2e.yml`'s `notify-e2e-failure` job opens a deduped `e2e-full-broken` issue on a main-push/schedule failure instead.
