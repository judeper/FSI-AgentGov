...
- **Unattended auto-merge streams + Playwright pin guard** (`dependabot-automerge.yml`, the auto-merge steps in `learn-monitor.yml` / `regulatory-monitoring.yml`, `scripts/check_playwright_pin_drift.py`): three kill-switch repo variables gate three unattended `gh pr merge --auto` streams — `DEPENDABOT_AUTOMERGE` (minor/patch + security; majors → `needs-review`), `LEARN_STATE_AUTOMERGE`, and `REGULATORY_STATE_AUTOMERGE` (successful regulatory state-maintenance PRs and findings with 0 CRITICAL/HIGH, via a fail-closed `automerge_eligible` signal; CRITICAL/HIGH findings keep `needs-review`, and consolidation never closes a `needs-review` PR). Regulatory mutation and state-PR creation are default-branch-only; feature/PR refs use a separate read-only job (`contents: read`, `persist-credentials: false`) and never receive App/write credentials. The trusted default-branch mutation job requests write permissions and its App token only after checkout, monitor success, and base/state SHA CAS. Scheduled mutation requires valid regulatory source sections and `last_run`; missing/corrupt state fails closed, while baseline initialization is local-only and explicitly approved. A successful exit-0 regulatory run creates a clearly identified PR only when persisted state is dirty; clean runs create no PR, and exit >=2 never creates a state PR. Every merge still waits on the 11 required checks + `strict` (up-to-date) rebase, which also stops the daily-Learn / weekly-Regulatory PRs from clobbering each other's `data/monitor-state.json`. Any workflow pinning a Playwright `container.image:` must keep its tag equal to `@playwright/test` in `package.json` — the required `manifest / index / nav drift` job runs `check_playwright_pin_drift.py` to enforce it (the #583 `e2e-full` skew). `e2e-full` is deliberately **not** a required check (label-gated off PRs to avoid the deadlock); on `push`/`schedule`, `e2e.yml` opens or updates one deduped, owner-assigned `e2e-full-broken` issue on failure, then comments and closes it only after a successful full run.
